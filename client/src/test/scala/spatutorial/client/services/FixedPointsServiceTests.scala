...
object FixedPointsServiceTests extends TestSuite {

  import drt.client.services.JSDateConversions._

  case class FixedPoint(name: String, terminalName: TerminalName, startDt: MilliDate, endDt: MilliDate, numberOfFixedPoints: Int) {
    def toCsv = {
      ...
      s"$name,$terminalName,$startDateString,$startTimeString,$endTimeString,$numberOfFixedPoints"
    }
  }

  case class FixedPointService(fixedPoints: Seq[FixedPoint]) {
    def fixedPointsAt(date: MilliDate): Int = fixedPoints.filter(fixedPoint =>
      fixedPoint.startDt <= date && date <= fixedPoint.endDt).map(_.numberOfFixedPoints).sum
    def terminalFixedPointsAt(terminalName: TerminalName, date: MilliDate): Int = fixedPoints.filter(fixedPoint => {
      fixedPoint.startDt <= date && date <= fixedPoint.endDt && fixedPoint.terminalName == terminalName
    }).map(_.numberOfFixedPoints).sum
  }

  object FixedPointsService {
    def apply(fixedPoint: Seq[Try[FixedPoint]]): Try[FixedPointService] = {
      if (fixedPoint.exists(_.isFailure))
        Failure(new Exception("Couldn't parse fixed points"))
      else {
        val successfulFixedPoints = fixedPoint.map { case Success(s) => s }
        Success(FixedPointService(successfulFixedPoints))
      }
    }
  }
    ...
    'StaffShifts - {
      "As an HO, either in planning or at start of shift, " +
        ...
        "Given a shift of 10 people, if we ask how many staff are available" - {
          val fixedPoints = FixedPoint("alpha", "any", SDate(2016, 12, 10, 10, 0), SDate(2016, 12, 10, 19, 0), 10)
          val shiftService = FixedPointsService(fixedPoints :: Nil)
