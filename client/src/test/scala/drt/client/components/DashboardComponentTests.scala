...
object DashboardComponentTests extends TestSuite {
  ...
  def tests = Tests {
    ...
    "Given a list of flights spanning a 3 hour period when I group by hour I should a list of touples of hour to list " +
      ...
      val flight1 = ApiFlightWithSplits(ArrivalGenerator.arrival(schDt = "2017-11-01T09:52:00"), Set())
      val flight2 = ApiFlightWithSplits(ArrivalGenerator.arrival(schDt = "2017-11-01T09:45:00"), Set())
      val flight3 = ApiFlightWithSplits(ArrivalGenerator.arrival(schDt = "2017-11-01T10:55:00"), Set())
      val flight4 = ApiFlightWithSplits(ArrivalGenerator.arrival(schDt = "2017-11-01T11:00:00"), Set())
      val flight5 = ApiFlightWithSplits(ArrivalGenerator.arrival(schDt = "2017-11-01T12:05:00"), Set())
      val flight6 = ApiFlightWithSplits(ArrivalGenerator.arrival(schDt = "2017-11-01T13:00:00"), Set())
    ...
    "When I ask for a break down of flights and queues per hour" - {
      "Given 1 flight and 1 Crunch Minute for the same period" - {
        val startDate = SDate("2017-10-30T00:00:00Z")
        val flights = List(ApiFlightWithSplits(ArrivalGenerator.arrival(schDt = "2017-10-30T00:00:00Z", passengerSources = Map(ApiFeedSource -> Passengers(actual = Option(15), transit = None))), Set()))
      ...
      "Given 2 flights and 1 Crunch Minute for the same period then we should get a summary with 2 flights in" - {
        ...
        val flights = List(
          ApiFlightWithSplits(
            ArrivalGenerator.arrival(schDt = "2017-10-30T00:00:00Z", passengerSources = Map(ApiFeedSource -> Passengers(actual = Option(15), transit = None))), Set()
          ),
          ApiFlightWithSplits(
            ArrivalGenerator.arrival(schDt = "2017-10-30T00:01:00Z", passengerSources = Map(ApiFeedSource -> Passengers(actual = Option(15), transit = None))), Set()
          )
        )
        val cms = List(CrunchMinute(T1, Queues.EeaDesk, startDate.millisSinceEpoch, 20, 0, 0, 0, None, None, None))
      ...
      "Given 2 flights and 1 Crunch Minute for different hours then we should get two hour summaries back each with one flight" - {
        ...
        val flights = List(
          ...
          ApiFlightWithSplits(
            ArrivalGenerator.arrival(schDt = "2017-10-30T01:00:00Z", passengerSources = Map(ApiFeedSource -> Passengers(actual = Option(15), transit = None))), Set()
          )
        )
        val cms = List(CrunchMinute(T1, Queues.EeaDesk, startDate.millisSinceEpoch, 20, 0, 0, 0, None, None, None))
