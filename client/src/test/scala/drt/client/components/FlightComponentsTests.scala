...
object ArrivalGenerator {
  ...
  def apiFlight(
    ...
    Arrival(
      ...
      Status = status,
      EstDT = estDt,
      Estimated = if (estDt != "") SDate.parse(estDt).millisSinceEpoch else 0L,
      ActDT = actDt,
      Actual = if (actDt != "") SDate.parse(actDt).millisSinceEpoch else 0L,
      EstChoxDT = estChoxDt,
      EstimatedChox = if (estChoxDt != "") SDate.parse(estChoxDt).millisSinceEpoch else 0L,
      ActChoxDT = actChoxDt,
      ActualChox = if (actChoxDt != "") SDate.parse(actChoxDt).millisSinceEpoch else 0L,
      Gate = gate,
      Stand = stand,
      MaxPax = maxPax,
      TranPax = tranPax,
