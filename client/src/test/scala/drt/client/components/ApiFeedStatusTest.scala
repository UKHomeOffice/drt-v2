...
class ApiFeedStatusTest extends AnyWordSpec with Matchers {
  ...
  "ApiFeedStatus" should {
    ...
    val landedWithNoSources =
      ArrivalGenerator.arrival(schDt = beforeNow, totalPax = Option(100), transPax = None, feedSource = LiveFeedSource)
    val landedWithNoPax =
      ArrivalGenerator.arrival(schDt = beforeNow, totalPax = None, transPax = None, feedSource = LiveBaseFeedSource)
    val landedWithLiveSource =
      ArrivalGenerator.arrival(schDt = beforeNow, totalPax = Option(100), transPax = None, feedSource = LiveFeedSource)
    val notLanded = ApiFlightWithSplits(
      ArrivalGenerator.arrival(schDt = afterNow, totalPax = Option(100), transPax = None, feedSource = LiveFeedSource),
      Set()
    )
    ...
    val landedWithValidApi = ApiFlightWithSplits(
      landedWithNoSources,
      Set(Splits(
        Set(ApiPaxTypeAndQueueCount(PaxTypes.B5JPlusNational, EeaDesk, 100d, None, None)),
        SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages,
        Option(DC)
      ))
    )
    val landedWithInvalidApi = ApiFlightWithSplits(
      landedWithLiveSource,
      Set(Splits(Set(), SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages, Option(DC)))
    )
    val landedWithNoActPax = ApiFlightWithSplits(
      landedWithNoPax,
      Set(Splits(
        Set(ApiPaxTypeAndQueueCount(PaxTypes.B5JPlusNational, EeaDesk, 100d, None, None)),
        SplitSources.ApiSplitsWithHistoricalEGateAndFTPercentages,
