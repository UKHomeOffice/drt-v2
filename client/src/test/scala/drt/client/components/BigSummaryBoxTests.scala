    ...
    "Summary for the next 3 hours" - {
      "Given a rootModel with flightsWithSplits with flights arriving 2017-05-01T12:01Z onwards" - {
        ...
        "Given 3 flights" - {
          ...
          val apiFlight1 = apiFlight(schDt = "2017-05-01T12:05Z", terminal = T1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(200), transit = None)))
          val apiFlight2 = apiFlight(schDt = "2017-05-01T13:05Z", terminal = T1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(300), transit = None)))
          val apiFlight3 = apiFlight(schDt = "2017-05-01T13:20Z", terminal = T1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(40), transit = None)))
        ...
        "Given 3 flights with a nonZero PcpTime we use the pcpTime " - {
          val apiFlightPcpBeforeNow = apiFlight(schDt = "2017-05-01T11:40Z", terminal = T1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(7), transit = None)), pcpTime = Option(mkMillis("2017-05-01T11:40Z")))
          val apiFlight0aPcpAfterNow = apiFlight(schDt = "2017-05-01T11:40Z", terminal = T1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(11), transit = None)), pcpTime = Option(mkMillis("2017-05-01T12:05Z")))
          val apiFlight1 = apiFlight(schDt = "2017-05-01T12:05Z", terminal = T1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(200), transit = None)), pcpTime = Option(mkMillis("2017-05-01T12:05Z")))
          val apiFlight2 = apiFlight(schDt = "2017-05-01T13:05Z", terminal = T1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(300), transit = None)), pcpTime = Option(mkMillis("2017-05-01T13:15Z")))
          val apiFlight3 = apiFlight(schDt = "2017-05-01T13:20Z", terminal = T1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(40), transit = None)), pcpTime = Option(mkMillis("2017-05-01T13:22Z")))
        ...
        "Given 3 flights with a nonZero PcpTime we use the pcpTime " - {
          "AND we can filter by Terminal - we're interested in T1" - {
            ...
            val apiFlight1 = apiFlight(schDt = "2017-05-01T12:05Z", terminal = terminal1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(200), transit = None)), pcpTime = Option(mkMillis("2017-05-01T12:05Z")))
            val apiFlight2 = apiFlight(schDt = "2017-05-01T13:05Z", terminal = terminal1, totalPax = Map(ApiFeedSource -> Passengers(actual = Option(300), transit = None)), pcpTime = Option(mkMillis("2017-05-01T13:15Z")))
            val notOurTerminal = apiFlight(schDt = "2017-05-01T13:20Z", terminal = Terminal("T4"), totalPax = Map(ApiFeedSource -> Passengers(actual = Option(40), transit = None)), pcpTime = Option(mkMillis("2017-05-01T13:22Z")))
          ...
          "And they have splits" - {
            "Then we can aggregate the splits for a graph" - {
              val apiFlight1 = apiFlight(schDt = "2017-05-01T12:05Z", terminal = terminal1,
                totalPax = Map(ApiFeedSource -> Passengers(actual = Option(200), transit = None)), pcpTime = Option(mkMillis("2017-05-01T12:05Z")))
              val apiFlight2 = apiFlight(schDt = "2017-05-01T13:05Z", terminal = terminal1,
                totalPax = Map(ApiFeedSource -> Passengers(actual = Option(300), transit = None)), pcpTime = Option(mkMillis("2017-05-01T13:15Z")))
            ...
            "Then we can aggregate the splits by multiply the % against the bestPax so that we can show them in a graph" - {
              val flights = List(
                ApiFlightWithSplits(apiFlight(schDt = "2017-05-01T12:05Z", terminal = terminal1,
                  totalPax = Map(LiveFeedSource -> Passengers(actual = Option(100), transit = None)), pcpTime = Option(mkMillis("2017-05-01T12:05Z"))),
                  Set(Splits(Set(
                ...
                ApiFlightWithSplits(apiFlight(schDt = "2017-05-01T13:05Z", terminal = terminal1,
                  totalPax = Map(LiveFeedSource -> Passengers(actual = Option(100), transit = None)), pcpTime = Option(mkMillis("2017-05-01T13:15Z"))),
                  Set(Splits(Set(
            ...
            "And they have API PaxNo splits including a transfer split" - {
              "And they have tranpax" - {
                "Then we can aggregate the splits by adding them up - we also filter out the transfer split" - {
                  val flights = List(
                    ApiFlightWithSplits(apiFlight(schDt = "2017-05-01T12:05Z", terminal = terminal1,
                      totalPax = Map(LiveFeedSource -> Passengers(actual = Option(300), transit = Option(100))), pcpTime = Option(mkMillis("2017-05-01T12:05Z"))),
                      Set(Splits(Set(
