    ...
    "Summary for the next 3 hours" - {
      "Given a rootModel with flightsWithSplits with flights arriving 2017-05-01T12:01Z onwards" - {
          ...
          "AND a current time of 2017-05-01T12:00" - {
            ...
            "Then we can get a number of flights arriving in that period" - {
              val countOfFlights = rootModel.portStatePot.map(_.flights.values.count(f => {
                ...
                now.millisSinceEpoch <= flightDt.millisSinceEpoch &&
                flightDt.millisSinceEpoch <= nowPlus3Hours.millisSinceEpoch
              }))
        ...
        "Given 3 flights" - {
          ...
          val apiFlight1 = ArrivalGenerator.arrival(
            schDt = "2017-05-01T12:05Z",
            terminal = T1,
            totalPax = Option(200),
            feedSource = LiveFeedSource
          )
          val apiFlight2 = ArrivalGenerator.arrival(
            schDt = "2017-05-01T13:05Z",
            terminal = T1,
            totalPax = Option(300),
            feedSource = LiveFeedSource
          )
          val apiFlight3 = ArrivalGenerator.arrival(
            schDt = "2017-05-01T13:20Z",
            terminal = T1,
            totalPax = Option(40),
            feedSource = LiveFeedSource
          )

          val rootModel = RootModel(portStatePot =
            Ready(PortState(
              List(
        ...
        "Given 3 flights with a nonZero PcpTime we use the pcpTime " - {
          val apiFlightPcpBeforeNow =
            ArrivalGenerator.live(schDt = "2017-05-01T11:40Z", terminal = T1, totalPax = Option(7))
              .toArrival(LiveFeedSource).copy(PcpTime = Option(mkMillis("2017-05-01T11:40Z")))
          val apiFlight0aPcpAfterNow =
            ArrivalGenerator.live(schDt = "2017-05-01T11:40Z", terminal = T1, totalPax = Option(11))
              .toArrival(LiveFeedSource).copy(PcpTime = Option(mkMillis("2017-05-01T11:40Z")))
        ...
        "Given 3 flights with a nonZero PcpTime we use the pcpTime " - {
          "AND we can filter by Terminal - we're interested in T1" - {
            ...
            val notOurTerminal = ArrivalGenerator.arrival(
              schDt = "2017-05-01T13:20Z",
              terminal = T4,
              totalPax = Option(40),
              feedSource = LiveFeedSource
          ...
          "And they have splits" - {
            "Then we can aggregate the splits for a graph" - {
              val apiFlight1 = ArrivalGenerator.arrival(
                ...
                totalPax = Option(200),
                feedSource = LiveFeedSource
              ).copy(PcpTime = Option(mkMillis("2017-05-01T12:05Z")))
              val apiFlight2 = ArrivalGenerator.arrival(
                schDt = "2017-05-01T13:05Z",
                terminal = terminal1,
                totalPax = Option(300),
                feedSource = LiveFeedSource
              ).copy(PcpTime = Option(mkMillis("2017-05-01T13:15Z")))

              val splits1 = Splits(
                Set(
                  ApiPaxTypeAndQueueCount(PaxTypes.NonVisaNational, Queues.NonEeaDesk, 41, None, None),
              ...
              val flights = List(
                ...
                ApiFlightWithSplits(
                  ArrivalGenerator.arrival(
                    ...
                    totalPax = Option(100),
                    feedSource = LiveFeedSource
                  ).copy(PcpTime = Option(mkMillis("2017-05-01T13:05Z"))),
                  Set(Splits(
                    Set(
