...
object DeskRecsChart {
  ...
  def DeskRecs(labels: IndexedSeq[String]) = ReactComponentB[ModelProxy[Pot[CrunchResult]]]("CrunchResults")
    .render_P { proxy =>
      Panel(Panel.Props("Desk Recommendations and Wait times"),
        ...
        proxy().render(chartData => {
          val sampledWaitTimes = chartData.waitTimes.grouped(15).map(_.max).map(_.toDouble).toList
  ...
  def apply(labels: IndexedSeq[String], proxy: ModelProxy[Pot[CrunchResult]]) = DeskRecs(labels)(proxy)
}
