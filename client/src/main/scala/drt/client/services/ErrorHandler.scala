...
import drt.client.logger.{ log, LoggerFactory }
...
object ErrorHandler {
  def registerGlobalErrorHandler(): Unit = {
    ...
    dom.window.onerror = (ev: Event, url: String, line: Int, col: Int, error: Any) => {
      ...
      val message =
        s"Event: $ev, Url: $url, Line: $line, Col: $col, Error: $error, Browser: ${dom.window.navigator.appVersion}"
      ...
      val reload = dom.window.confirm(
        "Sorry, we have encountered an error. The error has been logged. Would you like to reload the page?"
      )
      if (reload) {
