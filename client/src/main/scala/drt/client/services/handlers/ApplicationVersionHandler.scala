...
import drt.client.actions.Actions.{
  ...
  TriggerReload,
  UpdateServerApplicationVersion
}
...
class ApplicationVersionHandler[M](modelRW: ModelRW[M, Pot[ClientServerVersions]])
      ...
      val effect = Effect(DrtApi.get("version").map(response => {
        Try(read[BuildVersion](response.responseText)) match {
          case Success(buildInfo) =>
            value match {
              case Ready(ClientServerVersions(clientVersion, _))
                  if buildInfo.version != clientVersion && buildInfo.requiresReload =>
                TriggerReload
