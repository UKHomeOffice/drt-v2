...
class ForecastHandler[M](modelRW: ModelRW[M, Pot[ForecastPeriodWithHeadlines]]) extends LoggingActionHandler(modelRW) {
  protected def handle: PartialFunction[Any, ActionResult[M]] = {
    case action @ GetForecast(startDay, numberOfDays, terminalName, periodInterval) =>
      val apiCallEffect =
        Effect(DrtApi.get(s"forecast-summary/$terminalName/${startDay.millisSinceEpoch}/$numberOfDays/$periodInterval")
          .map { res =>
          ...
          .recoverWith {
            case t =>
              log.error(
                s"Failed to get Forecast Period: ${t.getMessage}. Re-requesting after ${PollDelay.recoveryDelay}"
              )
