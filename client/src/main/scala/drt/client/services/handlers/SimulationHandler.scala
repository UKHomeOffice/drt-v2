...
class SimulationHandler[M](simulationResult: ModelRW[M, Pot[SimulationResult]])
    extends LoggingActionHandler(simulationResult) {
  protected def handle: PartialFunction[Any, ActionResult[M]] = {
    case GetSimulation(params) =>
      updated(
        Pending(),
        Effect(DrtApi.get(s"desk-rec-simulation?${params.toQueryStringParams}")
