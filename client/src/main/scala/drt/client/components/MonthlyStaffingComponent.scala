      ...
      val modelChangeDetection = shiftAssignmentsRCP { shiftAssignmentsMP =>
        ...
        val content = for {
          ...
          if (monthOfShiftAssignments != state.shiftAssignments || state.timeSlots.isEmpty) {
            val slots = slotsFromShiftAssignments(
              monthOfShiftAssignments,
            ...
            scope.modState(state =>
              state.copy(
                shiftAssignments = monthOfShiftAssignments,
                timeSlots = Ready(slots),
                shiftsLastLoaded = Option(SDate.now().millisSinceEpoch)
              )
            ).runNow()
          }
          <.div()
                ...
                <.div(
                  ...
                  <.div(
                    ...
                    state.shiftsLastLoaded.map(lastLoaded =>
                      <.div(
                        ^.className := "staffing-table-content",
                        HotTable(HotTable.Props(
                          timeSlots,
                          colHeadings = state.colHeadings.map(h =>
                            s"<div style='text-align: left;'>${h.day}<br>${h.dayOfWeek}</div>"
                          ),
                          rowHeadings = state.rowHeadings,
                          afterChanges = (changes: Seq[(Int, Int, Any, Any)]) => {
                            tempChanges = changes.collect {
                              case (row, col, _, newVal: String) if newVal.forall(_.isDigit) =>
                                TimeSlotDay(row, col).key -> newVal.toInt
