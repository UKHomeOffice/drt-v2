...
object TerminalDashboardComponent {
  ...
  val component: Component[Props, Unit, Unit, CtorType.Props] = ScalaComponent.builder[Props]("TerminalDashboard")
    .render_P { props =>
      ...
      val pot = for {
        ...
        val currentSlotPs = portState.crunchMinutes.filter(k =>
          start.millisSinceEpoch <= k._1.minute && k._1.minute < end.millisSinceEpoch
        )
        val prevSlotPs = portState.crunchMinutes.filter(k =>
          prevSlotStart.millisSinceEpoch <= k._1.minute && k._1.minute < start.millisSinceEpoch
        )
        ...
        <.div(
          ...
          <.div(
            ...
            if (props.terminalPageTabLoc.queryParams.contains("showArrivals")) {
              ...
              <.div(
                ...
                <.div(
                  ...
                  <.div(
                    ^.className := "terminal-dashboard__arrivals_popup_table",
                    FlightTable(
                      FlightTable.Props(
                  ...
                  props.router.link(closeArrivalsPopupLink)(
                    ^.className := "close-arrivals-popup btn btn-default",
                    "close"
                  )
                )
              )
            ...
            <.div(
              ^.className := "terminal-dashboard-queues",
              <.div(^.className := "pax-bar", s"$terminalPax passengers presenting at the PCP"),
              <.div(
                ^.className := "queue-boxes",
                QueueConfig.queuesForDateAndTerminal(props.airportConfig.queuesByTerminal)(
                  startPoint.toLocalDate,
                  terminal
                ).filterNot(q => q == FastTrack || q == Transfer).map(q => {
                  ...
                  props.slaConfigs.render { slaConfigs =>
                    def slas: Map[Queue, Int] =
                      slaConfigs.configForDate(startPoint.millisSinceEpoch).getOrElse(props.airportConfig.slaByQueue)

                    <.dl(
                      ^.aria.label := s"Passengers joining queue ${Queues.displayName(q)}",
                      ^.className :=
                        s"queue-box col ${q.toString.toLowerCase} ${TerminalDesksAndQueuesRow.slaRagStatus(qWait, slas(q))}",
              ...
              <.div(
                ^.className := "tb-bar-wrapper",
                props.router.link(props.terminalPageTabLoc.
                  copy(queryParams = Map("start" -> s"$urlPrevTime")))(
                  ^.aria.label := s"View previous $slotSize minutes",
                  ^.className := "dashboard-time-switcher prev-bar col",
                  Icon.angleDoubleLeft
                ),
                <.div(
                  ^.className := "tb-bar",
                  ^.aria.label := "current display time range",
                  s"${start.prettyTime} - ${end.prettyTime}"
                ),
                props.router.link(props.terminalPageTabLoc.
                  copy(queryParams = Map("start" -> s"$urlNextTime")))(
                  ^.aria.label := s"View next $slotSize minutes",
                  ^.className := "dashboard-time-switcher next-bar col",
                  Icon.angleDoubleRight
                )
              )
            ),
            <.div(
              ^.className := "terminal-dashboard-side",
              props.router
              ...
              <.div(
                ...
                <.label(
                  ^.className := "terminal-dashboard-side__sidebar_widget__label",
                  ^.aria.label := "Select timeslot size for PCP passengers display",
                  "Time slot duration"
                ),
                <.select(
                  ^.onChange ==>
  ...
  private def cmsForTerminalAndQueue(
