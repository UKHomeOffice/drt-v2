...
object TerminalStaffingV2 {
  ...
  def staffTimeSlotSeqToStaffTimeSlots(month: SDateLike, staffDays: Seq[Seq[Int]], terminal: String, slotMinutes: Int): Seq[StaffTimeSlot] =
    staffDays.zipWithIndex.flatMap {
      case (slotDaysStaff, timeSlotIndex) =>
        slotDaysStaff.zipWithIndex.collect {
          case (staffInSlotForDay, dayIndex) if staffInSlotForDay != 0 =>
            val slotStart = month.addDays(dayIndex).addMinutes(timeSlotIndex * slotMinutes).getUtcMillis()
            StaffTimeSlot(terminal, slotStart, staffInSlotForDay, slotMinutes * 60000)
        }
