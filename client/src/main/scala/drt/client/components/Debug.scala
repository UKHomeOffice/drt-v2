...
object Debug {
  ...
  val component: Component[Props, Unit, Unit, CtorType.Props] = ScalaComponent.builder[Props]("Debug")
    .render_P { _ =>
      val staffingRCP = SPACircuit.connect(m =>
        (
          ...
          if (dom.window.hasOwnProperty("debug")) {
            <.table(
              <.tr(
                ...
                potShifts.render(s =>
                  <.td(<.pre(
                    ^.style := js.Dictionary("overflow" -> "auto", "height" -> "200px"),
                    s.assignments.map(_.toString).mkString("\n")
                  ))
                )
              ),
              <.tr(
                <.th("fixed points"),
                potFixedPoints.render(s =>
                  <.td(<.pre(
                    ^.style := js.Dictionary("overflow" -> "auto", "height" -> "200px"),
                    s.assignments.map(_.toString).mkString("\n")
                  ))
              ...
              <.tr(
                ...
                <.td(<.pre(
                  ^.style := js.Dictionary("overflow" -> "auto", "height" -> "200px"),
                  staffMovements.toString()
                ))
              ),
              <.tr(
                <.th("crunch State"),
                portState.render(s =>
                  <.td(<.pre(^.style := js.Dictionary("overflow" -> "auto", "height" -> "200px"), s.toString))
                )
              ),
