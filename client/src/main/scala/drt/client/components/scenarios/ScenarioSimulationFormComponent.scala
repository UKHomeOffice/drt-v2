...
object ScenarioSimulationFormComponent extends ScalaCssReactImplicits {
  ...
  case class Props(date: LocalDate, terminal: Terminal, airportConfig: AirportConfig, slaConfigs: SlaConfigs)
      extends UseValueEq

  val component: Component[Props, State, Unit, CtorType.Props] =
    ScalaComponent.builder[Props]("SimulationFormComponent")
      ...
      .renderPS {
        ...
        (scope, props, state) =>
          def changePassengerWeighting(e: ReactEventFromInput): Callback = {
            ...
            scope.setState(state.copy(simulationFormFields =
              state.simulationFormFields.copy(passengerWeighting = maybeValue)
            ))
          }

          def changeBankSize(bankIndex: Int)(e: ReactEventFromInput): Callback = {
            val maybeValue = Try(e.target.value.toInt).toOption
            val updatedBankSizes =
              state.simulationFormFields.eGateBankSizes.indices.zip(state.simulationFormFields.eGateBankSizes).map {
                case (idx, existingBankSize) => if (idx == bankIndex) maybeValue else existingBankSize
              }
            scope.setState(state.copy(simulationFormFields =
              state.simulationFormFields.copy(eGateBankSizes = updatedBankSizes)
            ))
          }

          def changeProcessingTimes(ptq: PaxTypeAndQueue): ReactEventFromInput => Callback =
          ...
          def processingTimesFields = {
            <.div(
              ^.style := js.Dictionary("display" -> "flex", "flexDirection" -> "column", "gap" -> "16px"),
              state.simulationFormFields.processingTimes.toList.sortBy {
                ...
                case (ptq, maybeProcTime) =>
                  ...
                  MuiTextField(
                    label =
                      s"Seconds to process ${PaxTypes.displayNameShort(ptq.passengerType, isBeforeAgeEligibilityChangeDate = false)} to $queueDisplayName".toVdom
                  )(
                    ^.key := ptq.key,
                    `type` := "number",
          ...
          def eGatesOpen: VdomElement =
            MuiFormGroup()(
              ...
              (0 to 23)
                .map(hour =>
                  <.div(
                    ^.key := s"egate-open-hour-$hour",
                    MuiInputLabel()(f"$hour%02d:00"),
                    MuiCheckbox()(
                      ^.value := hour.toString,
          ...
          <.div(
            ...
            <.div(
              ...
              MuiButton(
                ...
                ^.href :=
                  SPAMain.absoluteUrl(s"export/desk-rec-simulation?${state.simulationFormFields.toQueryStringParams}"),
                "Export"
              )
            )
          )
