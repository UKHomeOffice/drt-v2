...
object MonthlyShiftsUtil {
  ...
  case class ShiftDetails(
      shift: Shift,
      terminal: Terminal,
      shiftAssignments: ShiftAssignments
  )

  case class ShiftPeriod(
      start: SDateLike,
      end: SDateLike,
      endHour: Int,
      endMinute: Int,
  ...
  def createStaffTableEntries(
    ...
    val shiftEndsAfterMidnight = shiftEndHour < shiftStartHour ||
  ...
  private def dayRangeForView(startDate: LocalDate, daysCount: Int, shift: Shift): Range.Inclusive = {
    ...
    val daysInMonth: Int =
      if (
        shift.endDate.exists(ed => startDate.month == ed.month && startDate.year == ed.year && ed.day >= startDate.day)
      )
  ...
  def staffTableEntriesForShift(
    ...
    val dayAssignments = shiftDetails.shiftAssignments.assignments
      .filter(assignment =>
        assignment.start >= shiftPeriod.start.millisSinceEpoch && assignment.end <= shiftPeriod.end.millisSinceEpoch
      )
      .map { a =>
    ...
    Iterator
      ...
      .takeWhile(_ < shiftPeriod.end).toSeq.zipWithIndex.map { case (slotStart, index) =>
        ...
        findAndCreateDayTableAssignment(
          shiftPeriod,
          shiftDetails.shift,
          staffRec,
          maybeAssignment,
  ...
  private def nextSlotTime(shiftPeriod: ShiftPeriod, slotTime: SDateLike): SDateLike =
    ...
    if (
      (slotTime.getMinutes == 30 && shiftPeriod.intervalMinutes == 60) ||
  ...
  private def findAndCreateDayTableAssignment(
      shiftPeriod: ShiftPeriod,
      shift: Shift,
    ...
    StaffTableEntry(
      ...
      endTime =
        ShiftDateTime(nextTime.getFullYear, nextTime.getMonth, nextTime.getDate, nextTime.getHours, nextTime.getMinutes)
    )
  }

  def generateShiftSummaries(
    ...
    shifts.sortBy(_.startTime).zipWithIndex.map { case (shift, index) =>
      ...
      val summary =
        ShiftSummary(shift.shiftName, shift.staffNumber, shift.startTime, shift.endTime, startDate, maybeEndDate)
  ...
  def updateShiftSummaries(
      shifts: Seq[ShiftSummaryStaffing],
      changedAssignments: Seq[StaffTableEntry],
      slotMinutes: Int
  ): Seq[ShiftSummaryStaffing] = {
    val changedAssignmentsWithSlotMap: Seq[StaffTableEntry] =
      changedAssignments.flatMap(a => StaffTableEntry.splitIntoSlots(a, slotMinutes))
    val changedAssignmentsMap: Map[String, StaffTableEntry] =
      changedAssignmentsWithSlotMap.map(a => ShiftDateTime.toString(a.startTime) -> a).toMap
