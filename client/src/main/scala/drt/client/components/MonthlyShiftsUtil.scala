...
object MonthlyShiftsUtil {
  ...
  def generateShiftData(viewingDate: SDateLike, dayRange: String, terminal: Terminal, staffShifts: Seq[StaffShift], shifts: ShiftAssignments, interval: Int): Seq[ShiftData] = {
    ...
    val numberOfDaysInMonth = daysInMonth(viewingDate.getFullYear, viewingDate.getMonth)

    val daysCount = dayRange match {
      case "weekly" => 7
      case "daily" => 1
      case _ => numberOfDaysInMonth
    }

    val firstDay = dayRange match {
      case "weekly" =>
    ...
    def assignmentsForShift(s: StaffShift): Seq[ShiftAssignment] = {
    ...
    staffShifts.zipWithIndex.map { case (s, index) =>
      ShiftData(
        ...
        assignments = assignmentsForShift(s)
      )
    }
  }
