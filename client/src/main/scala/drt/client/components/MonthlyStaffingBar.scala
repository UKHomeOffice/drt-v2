...
case class ConfirmAndSaveForMonthlyStaffing(
    viewingDate: SDateLike,
    timeSlots: Seq[Seq[Any]],
    props: MonthlyStaffingComponent.Props,
    state: MonthlyStaffingComponent.State,
    scope: BackendScope[MonthlyStaffingComponent.Props, MonthlyStaffingComponent.State]
) extends ConfirmAndSave {
  override def apply(): ReactEventFromInput => Callback = _ =>
    Callback {
      val initialTimeSlots: Seq[Seq[Any]] = slotsFromShiftAssignments(
      ...
      if (
        confirm(
          s"You have updated staff for ${dateListToString(changedDays.map(_.day))} $updatedMonth - do you want to save these changes?"
        )
      ) {
        GoogleEventTracker.sendEvent(
          s"${props.terminalPageTab.terminal}",
...
case class ConfirmAndSaveForMonthlyShifts(
  ...
  override def apply(): ReactEventFromInput => Callback = _ =>
    Callback {
      val changedShifts: Seq[StaffAssignment] = shiftsData
        .flatMap(_.staffTableEntries.toSeq.map(ShiftAssignmentConverter.toStaffAssignment(
          _,
          props.terminalPageTab.terminal
        )))
      ...
      if (
        confirm(
          s"You have updated staff for ${MonthlyStaffingBar.whatDayChanged(changedAssignments)} $updatedMonth - do you want to save these changes?"
        )
      ) {
        GoogleEventTracker.sendEvent(
          s"${props.terminalPageTab.terminal}",
...
object MonthlyStaffingBar {
  ...
  private def navigationArrows(props: Props, previousWeekDate: SDateLike, nextWeekDate: SDateLike) = {
    <.div(
      MuiButton(
        ...
        variant = "contained",
        sx = SxProps(Map("height" -> "40px", "backgroundColor" -> "white"))
      )(
        MuiIcons(ChevronLeft)(fontSize = "small"),
        ^.onClick --> props.router.set(
          props.terminalPageTab.withUrlParameters(UrlDateParameter(Some(previousWeekDate.toISODateOnly)))
        )
      ),
      MuiButton(
        color = Color.secondary,
        ...
        ^.onClick -->
          props.router.set(props.terminalPageTab.withUrlParameters(UrlDateParameter(Some(nextWeekDate.toISODateOnly))))
      )
    )
  }

  def handleNavigation(props: Props, viewingDate: SDateLike): VdomTagOf[Div] = {
    val (previousDate, nextDate) =
      navigationDates(viewingDate, props.terminalPageTab.dayRangeType.getOrElse("monthly"), () => SDate.now())
  ...
  case class Props(
    ...
    def timeSlotMinutes: Int =
      Try(terminalPageTab.queryParams("timeInterval").toInt).toOption.getOrElse(defaultTimeSlotMinutes)
    ...
    .render_P { props =>
      def handleShiftViewToggle(): Callback = {
        Callback(GoogleEventTracker.sendEvent(
          s"${props.terminalPageTab.terminal}",
          "Shift View Toggle",
          s"${!props.userPreferences.showStaffingShiftView}"
        )) >>
          Callback(SPACircuit.dispatch(UpdateUserPreferences(props.userPreferences.copy(showStaffingShiftView =
            !props.userPreferences.showStaffingShiftView
          ))))
      }

      <.div(
        ^.className := "staffing-bar",
        <.div(
          ^.className := "staffing-controls-save",
          <.div(
            <.div(
              ^.className := "staffing-controls-title",
              <.h3("Set time filter")
            ),
            <.div(
              ^.style :=
                js.Dictionary("display" -> "flex", "justifyContent" -> "spaceBetween", "alignItems" -> "center"),
              <.span(
                ^.className := "staffing-controls-title-options",
                <.div(
                    ...
                    drawSelect(
                      ...
                      callback = (e: ReactEventFromInput) => {
                        props.router.set(props.terminalPageTab.withUrlParameters(
                          UrlDateParameter(Option(SDate(e.target.value).toISODateOnly))
                        ))
                      }
                    )
                  )
                ...
                <.div(
                  ^.className := "staffing-controls-navigation ",
                ...
                <.div(
                  ...
                  drawSelect(
                    ...
                    callback = (e: ReactEventFromInput) =>
                      props.router.set(
                        props.terminalPageTab.withUrlParameters(UrlTimeInterval(Some(s"${e.target.value}")))
                      )
                  )
                )
              ),
        ...
        <.div(
          ...
          if (props.isShiftFeatureEnabled && props.isShiftsEmpty)
            MuiButton(color = Color.secondary, variant = "contained")(
              ...
              ^.onClick -->
                props.router.set(TerminalPageTabLoc(props.terminalPageTab.terminalName, "shifts", "createShifts"))
            )
          else if (props.isShiftFeatureEnabled && !props.isShiftsEmpty)
            MuiButton(color = Color.secondary, variant = "contained")(
              <.span("Add shift"),
              ^.onClick -->
                props.router.set(TerminalPageTabLoc(props.terminalPageTab.terminalName, "shifts", "addShift"))
            )
          else
            EmptyVdom,
