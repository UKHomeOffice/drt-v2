...
object FlightTableContent {
  ...
  class Backend {
    ...
    private val handleTogglePaxSourceIcon = (e: ReactEventFromInput, userPreferences: UserPreferences) =>
      Callback {
        e.preventDefault()
        SPACircuit.dispatch(UpdateUserPreferences(userPreferences.copy(hidePaxDataSourceDescription =
          !e.target.checked
        )))
      }
    ...
    def render(props: Props): VdomElement = {
      val flightDisplayFilter = props.airportConfig.portCode match {
        case PortCode("LHR") => LhrFlightDisplayFilter(
            props.redListUpdates,
            (portCode, _, _) =>
              props.redListPorts.contains(portCode),
            LhrTerminalTypes(LhrRedListDatesImpl)
          )
        ...
        <.div(
          if (sortedFlights.nonEmpty) {
            ...
            <.div(
              ...
              <.div(^.style :=
                js.Dictionary("display" -> "flex", "justifyContent" -> "space-between", "alignItems" -> "center"))(
                <.div {
                  val flaggerInUse = props.flightHighlight.selectedNationalities.nonEmpty || ageGroups.nonEmpty ||
                    props.flightHighlight.showNumberOfVisaNationals
                  val flightCounts = if (flaggerInUse && props.flightHighlight.showOnlyHighlightedRows)
                    <.span(
                      s"$highlightedFlightsCount flight${pluraliseString(highlightedFlightsCount)} shown and highlighted"
                    )
                  else if (flaggerInUse)
                    <.span(
                      s"${sortedFlights.length} flight${pluraliseString(sortedFlights.length)} shown",
                      " | ",
                      <.b(s"$highlightedFlightsCount flight${pluraliseString(highlightedFlightsCount)} highlighted")
                    )
                  else
                ...
                <.div(
                  ...
                  MuiFormControl()(
                    MuiSwitch(defaultChecked = !props.userPreferences.hidePaxDataSourceDescription)(^.onChange ==>
                      ((e: ReactEventFromInput) => handleTogglePaxSourceIcon(e, props.userPreferences)))
                  ),
                  MuiTypography(sx =
                    SxProps(Map("paddingRight" -> "10px", "marginBottom" -> "0 !important"))
                  )(if (!props.userPreferences.hidePaxDataSourceDescription) "On" else "Off")
                )
              ),
              <.div(
                <.table(
                  ...
                  tableHead(
                    ...
                    props.shortLabel,
                    showFlagger,
                    props.userPreferences.hidePaxDataSourceDescription
                  ),
                  <.tbody(
                    ^.id := "sticky-body",
                    sortedFlights.flatMap {
                      case (flightWithSplits, codeShares) =>
                        ...
                        val directRedListFlight = DirectRedListFlight(
                          props.viewMode.dayEnd.millisSinceEpoch,
                          props.portCode,
                          props.terminal,
                        ...
                        val maybeManifestSummary =
                          props.flightManifestSummaries.get(ManifestKey(flightWithSplits.apiFlight))
                        ...
                        FlightHighlighter.highlightedFlight(
                          maybeManifestSummary,
                          props.flightHighlight.selectedNationalities,
                          ageGroups,
                          props.flightHighlight.showNumberOfVisaNationals,
                          props.flightHighlight.showOnlyHighlightedRows,
            ...
            ^.style := js.Dictionary("paddingTop" -> "16px", "paddingBottom" -> "16px"),
            if (flights.isEmpty) {
              <.div(
                ^.style := js.Dictionary(),
                MuiAlert(variant = MuiAlert.Variant.standard, color = "info", severity = "info")(MuiTypography(sx =
                  SxProps(Map("fontWeight" -> "bold"))
                )("No flights to display."))
              )
            } else {
              <.div(
                ^.style := js.Dictionary("border" -> "1px solid #99001E"),
                MuiAlert(variant = MuiAlert.Variant.standard, color = "error", severity = "error")(
                  MuiTypography(sx = SxProps(Map("fontWeight" -> "bold")))("No flights found."),
                  "Check the flight number or time period."
                )
              )
            }
          )
  ...
  def tableHead(
    ...
    val portColumnThs = columnHeadersWithClasses(
      columns,
      props.hasEstChox,
    ...
    val queueDisplayNames = <.th(
      <.span(
        ^.className := "flex-uniform-size",
        (Seq(if (hidePaxDataSourceDescription) <.div("", "", ^.className := "icon-header-data-quality")
        else EmptyVdom) ++
          queues.map(q =>
            <.div(Queues.displayName(q), " ", ^.className := "arrivals_table__splits__queue-pax flex-horizontally")
          )).toTagMod
      ),
  ...
  private def columnHeadersWithClasses(
      columns: Seq[(String, Option[String])],
      hasEstChox: Boolean,
      displayRedListInfo: Boolean,
      redListPaxExist: Boolean,
      redListHeading: String
  ): Seq[VdomTagOf[TableCell]] = {
    ...
    columns
      ...
      .map {
        case (label, Some(className)) if label == "Est PCP Pax" =>
          <.th(
            ^.className := "arrivals__table__flight__pcp-pax__header_column",
            <.div(^.cls := className, label)
          )
        case (label, None) if label == "Expected" || label == "Exp" =>
          <.th(
            <.div(^.className := "flex-horizonally", label, " ", expTimeTooltip)
          )
        case (label, None) if label == "Flight" =>
          <.th(
            <.div(^.cls := "arrivals__table__flight-code-wrapper", label, " ", wbrFlightColorTooltip)
          )
        case (label, None)                                      => <.th(label)
        case (label, Some(className)) if className == "country" =>
          <.th(label, " ", countryTooltip, ^.className := className)
        case (label, Some(className)) => <.th(label, ^.className := className)
      }
  }

  private def columnHeaders(
      shortLabel: Boolean,
      redListHeading: String,
      isMobile: Boolean,
      showFlagger: Boolean
    ...
    List(
      Option((
        "Flight",
        if (showFlagger) Option("arrivals__table__flight-code-with-highlight")
        else Option("arrivals__table__flight-code")
      )),
