...
import drt.client.components.FlightComponents.{ paxFeedSourceClass, SplitsDataQuality }
...
import uk.gov.homeoffice.drt.models.{ FlightManifestSummary, PaxAgeRange }
...
object FlightTableRow {
  ...
  class Backend {
    def render(props: Props): VdomTagOf[TableRow] = {
      ...
      val timeIndicatorClass =
        if (flight.PcpTime.getOrElse(0L) < SDate.now().millisSinceEpoch) "before-now" else "from-now"
      ...
      val highlighterIsActive = props.flaggedNationalities.nonEmpty || props.flaggedAgeGroups.nonEmpty ||
        props.showNumberOfVisaNationals

      val highlightPaxExists: Boolean = FlightHighlighter.highlightedFlight(
      ...
      def flightCodeElement(
          flightCodes: String,
          outgoingDiversion: Boolean,
          incomingDiversion: Boolean
      ): VdomTagOf[Span] =
          ...
          <.span(
            ...
            ^.onClick --> Callback(SPACircuit.dispatch {
              GoogleEventTracker.sendEvent(
                props.flightWithSplits.apiFlight.Terminal.toString,
                "flight feed source",
                props.flightWithSplits.unique.stringValue
              )
              props.viewMode match {
                case vm: ViewDay if vm.isHistoric(SDate.now()) && vm.timeMachineDate.isEmpty =>
                  GetArrivalSourcesForPointInTime(
                    SDate(props.viewMode.localDate).addHours(28),
                    props.flightWithSplits.unique
                  )
      ...
      val timesPopUp = arrivalTimes.map { case (label, time) =>
        <.div(
          ...
          <.span(^.display := "inline-block", ^.width := "120px", label),
          <.span(SDate(time).toLocalDateTimeString.takeRight(5))
        )
      }.toTagMod
      ...
      val charts = (flightWithSplits.hasApi, props.maybeManifestSummary) match {
        case (true, Some(manifestSummary)) =>
          ...
          if (maybePaxDiffAndPct.isEmpty || maybePaxDiffAndPct.exists(_._2 <= 1.05)) {
            val cls = if (props.showHighLighted) "arrivals__table__flight-code__info-highlighted"
            else "arrivals__table__flight-code__info"
            <.div(
              ^.className := cls,
      ...
      val firstCells = List[TagMod](
        <.td(
          ...
          <.div(
            ^.cls := s"$highlighterClass $isHighlightedClass arrivals__table__flight-code-wrapper",
            flightCodeElement(flightCodes, outgoingDiversion, props.directRedListFlight.incomingDiversion),
            charts
          )
        ),
        ...
        <.td(TerminalContentComponent.airportWrapper(flight.Origin) { airportInfoPot: ModelProxy[Pot[AirportInfo]] =>
          <.span(
            ...
            airportInfoPot().render { ai =>
              val redListCountry = props.indirectRedListPax.isEnabled &&
                isRedListCountry(ai.country, props.viewMode.dayEnd, props.redListUpdates)
              val style: html_<^.TagMod = if (redListCountry)
        ...
        <.td(gateOrStand(
          flight,
          props.airportConfig.defaultWalkTimeMillis(flight.Terminal),
          props.directRedListFlight.paxDiversion,
          props.walkTimes
        )),
        <.td(
          ^.className := "no-wrap",
          if (isMobile) flight.displayStatusMobile.description else flight.displayStatus.description
        ),
        <.td(maybeLocalTimeWithPopup(Option(flight.Scheduled))),
        <.td(expectedContent)
      )
      val pcpPaxDataQuality = paxFeedSourceClass(
        flightWithSplits.apiFlight.bestPaxEstimate(props.paxFeedSourceOrder),
        flight.Origin.isDomesticOrCta
      )
      val lastCells = List[TagMod](
        <.td(
          if (flight.Origin.isDomesticOrCta) EmptyVdom
          else pcpTimeRange(
            flightWithSplits,
            props.airportConfig.firstPaxOffMillis,
            props.walkTimes,
        ...
        <.td(
          pcpPaxDataQuality.map(dq =>
            if (props.hidePaxDataSourceDescription) {
              <.div(
                ^.className := s"pcp-icon-data-quality pax-rag-${dq.`type`}",
                PaxDatasourceComponent(IPaxDatasource(dq.text)),
                FlightComponents.paxComp(
                  flightWithSplits,
                  props.directRedListFlight,
                  flight.Origin.isDomesticOrCta,
                  props.paxFeedSourceOrder
                )
              )
      ...
      <.tr(
        ...
        if (props.hasTransfer)
          <.td(
            ^.className := "arrivals__table__flight_transfer-pax",
            FlightComponents.paxTransferComponent(flight, props.paxFeedSourceOrder)
          )
        else EmptyVdom
      )
    }
  ...
  private def gateOrStand(
      arrival: Arrival,
      terminalWalkTime: Long,
      paxAreDiverted: Boolean,
      walkTimes: WalkTimes
  ): VdomTagOf[Span] = {
    ...
    <.span(
      ^.className := "no-wrap",
      ^.key := "walk-time",
      Tippy.interactive("walk-time", <.span(walkTimeString), gateOrStand)
    )
  }
