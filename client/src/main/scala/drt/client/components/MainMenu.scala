...
object MainMenu {
  ...
  case class MenuItem(
      ...
      icon: Icon,
      location: Loc,
      tabClasses: Iterable[String] = List(),
      linkClasses: Iterable[String] = List()
  )
  ...
  private val portConfigMenuItem: Int => MenuItem =
    (position: Int) => MenuItem(position, "Port Config", Icon.cogs, PortConfigLoc)

  private val forecastUploadFile: Int => MenuItem =
    (position: Int) => MenuItem(position, "Forecast Upload", Icon.upload, ForecastFileUploadLoc)

  private def feedsRag(feeds: Seq[FeedSourceStatuses]): String = {
    val statuses = feeds.map(f =>
      FeedStatuses.ragStatus(SDate.now().millisSinceEpoch, f.feedSource.maybeLastUpdateThreshold, f.feedStatuses)
    )
      ...
      val targetLoc = currentLoc match {
        ...
        case tptl: TerminalPageTabLoc =>
          TerminalPageTabLoc(
            ...
            tptl.mode,
            tptl.subMode,
            tptl.withUrlParameters(
              UrlDateParameter(tptl.maybeViewDate.map(_.toISOString)),
              UrlTimeRangeStart(tptl.timeRangeStartString),
              UrlTimeRangeEnd(tptl.timeRangeEndString)
            ).queryParams
          )
      ...
      (
        ...
        (offset: Int) =>
          MenuItem(
            ...
            Icon.calculator,
            targetLoc,
            linkClasses = Seq(s"terminal-${terminalName.toLowerCase}")
          )
      )
    }

    val restrictedMenuItems: List[(Role, Int => MenuItem)] = addFileUpload ++ terminalDepsMenuItem :+
      ((ViewConfig, portConfigMenuItem))

    val nonTerminalUnrestrictedMenuItems = dashboardMenuItem :: Nil

    val itemsForLoggedInUser: List[MenuItem] =
      restrictedMenuItemsForRole(restrictedMenuItems, userRoles, nonTerminalUnrestrictedMenuItems.length)
  ...
  private def restrictedMenuItemsForRole(
      restrictedMenuItems: List[(Role, Int => MenuItem)],
      roles: Set[Role],
      startIndex: Int
  ): List[MenuItem] = {
  ...
  private class Backend() {
    def render(props: Props): VdomTagOf[Div] = {
      val children =
        for (item <- menuItems(props.airportConfig, props.currentLoc, props.user.roles, props.feeds)) yield {
          ...
          <.div(
            ^.key := item.idx,
            ^.classSet(classes: _*),
            ^.className := item.tabClasses.mkString(" "),
  ...
  def apply(
      ...
      airportConfig: AirportConfig,
      user: LoggedInUser
  ): VdomElement = component(Props(ctl, currentLoc, feeds, airportConfig, user))
}
...
object PortSwitcher {
      ...
      .renderPS((scope, props, state) => {
        ...
        if (ports.size == 2) {
          ...
          <.span(
            ...
            if (state.showDropDown) <.div(^.className := "menu-overlay", ^.onClick --> scope.modState(_ => State()))
            else "",
            <.ul(
              ^.className := s"main-menu__port-switcher dropdown-menu $showClass",
  ...
  private def listPortsByRegion(
      regions: List[(String, Set[PortCode])],
      currentPort: PortCode
  ): List[html_<^.VdomTagOf[LI]] =
    regions.flatMap { case (region, regionPorts) =>
      val regionLi =
        <.li(^.className := "dropdown-item", <.span(region, ^.disabled := true, ^.className := "non-selectable region"))
