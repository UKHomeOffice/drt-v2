...
object Layout {
  ...
  private case class LayoutModelItems(
    ...
    .render_P { props: Props =>
      def onClickAccessibilityStatement(): Unit = {
        GoogleEventTracker.sendEvent(
          props.currentLoc.page.portCodeStr,
          "Accessibility",
          "Accessibility statement clicked"
        )
      }
              ...
              <.div(
                ...
                <.div(
                    ...
                    MuiPaper(sx =
                      ...
                      MuiGrid(container = true, alignItems = "center")(
                        ...
                        MuiGrid(item = true, xs = gridItem2, sx = SxProps(Map("flexGrow" -> "1")))(
                          MuiTypography(
                            variant = "h5",
                            sx = SxProps(Map("color" -> "white", "padding" -> "3px 10px"))
                        ...
                        MuiGrid(item = true, xs = gridItem3)(
                          MuiButton(
                            ...
                            "Give feedback",
                            ^.onClick -->
                              Callback(dom.window.open(s"${SPAMain.urls.rootUrl}/feedback/banner/$aORbTest", "_blank"))
                          )
                        ),
                        MuiGrid(item = true, xs = 1)(
                          <.div(
                            ...
                            MuiIconButton(sx =
                              ...
                              ^.onClick --> Callback(SPACircuit.dispatch(CloseBanner())),
                              ^.aria.label := "Close",
                              Icon.close
                            )
                          )
                  ...
                  <.div(
                    ...
                    <.div(
                      ^.className := "contact",
                      ^.style := js.Dictionary("display" -> "flex", "alignItems" -> "center", "paddingRight" -> "20px"),
                      <.span(
                        "Contact: ",
                        <.a(^.href := s"mailto:$email", ^.target := "_blank", ^.textDecoration := "underline", email)
                      )
                    )
                  ),
                  <.div(
                ...
                <.footer(
                  BottomBarComponent(
                    BottomBarProps(
                      ...
                      s"${SPAMain.urls.rootUrl}/feedback/banner/$aORbTest"
                    )
                  )
                )
              )
