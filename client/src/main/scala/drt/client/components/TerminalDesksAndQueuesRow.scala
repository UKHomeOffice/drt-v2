...
object TerminalDesksAndQueuesRow {
  ...
  val component: Component[Props, Unit, Unit, CtorType.Props] =
    ScalaComponent.builder[Props]("TerminalDesksAndQueuesRow")
      .render_P(props => {
        val crunchMinutesByQueue =
          props.queueMinutes.filter(qm => props.queues.contains(qm.queue)).map(qm => Tuple2(qm.queue, qm)).toMap
        ...
        val queueTds = crunchMinutesByQueue.flatMap {
          case (queue, cm) =>
            ...
            def deployDeskTd: VdomTagOf[TableCell] = <.td(
              ...
              Tippy.interactive(
                "deploy-desk",
                <.span(s"Suggested deployments with available staff: ${cm.deployedDesks.getOrElse("-")}"),
                s"${cm.deskRec}"
              )
            )
              ...
              case Deployments =>
                if (props.showWaitColumn)
                  List(
                    paxLoadTd,
                    deployRecsDeskTd,
                    withSlaClass(cm.deployedWait.getOrElse(0))(ragClass =>
                      <.td(
              ...
              case Recommended =>
                if (props.showWaitColumn)
                  List(
                    paxLoadTd,
                    deployDeskTd,
                    withSlaClass(cm.waitTime)(ragClass =>
                      <.td(
                        ^.className := s"${queueColour(queue)} $ragClass",
                        Tippy.interactive(
                          "ideal-wait-time",
                          <.span(s"Suggested deployments with available staff: ${cm.waitTime}"),
        ...
        val pcpTds: Seq[VdomTagOf[TableCell]] = List(
          ...
          if (allowAdjustments)
            <.td(
              ^.className := s"total-deployed staff-adjustments",
              ^.colSpan := 2,
              <.span(minus, <.span(^.className := "deployed", available), plus)
            )
          else
            <.td(
              ^.className := s"total-deployed staff-adjustments",
              ^.colSpan := 2,
              <.span(^.className := "deployed", available)
            )
        )
  ...
  def adjustmentLink(props: Props, action: String): TagOf[html.Div] = {
    ...
    <.div(
      ^.className := "staff-deployment-adjustment-container",
      <.div(^.className := "popover-trigger", action, ^.onClick --> initialiseDialogue)
    )
  }
