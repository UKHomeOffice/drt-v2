...
object ApiFeedStatus {
  def apply(
    ...
    val withNonZeroPax =
      international.filter(fws => fws.apiFlight.bestPcpPaxEstimate(paxFeedSourceOrder).exists(_ > 0) || !hasLiveFeed)
...
object ApiStatusComponent {
  ...
  val component: Component[Props, Unit, Unit, CtorType.Props] = ScalaComponent.builder[Props]("ApiStatus")
    .render_P { props =>
      case class FlightsAndPaxFeedOrder(
          flights: Pot[Iterable[ApiFlightWithSplits]],
          paxFeedSourceOrder: List[FeedSource]
      )
      val terminalFlightsProxy = SPACircuit.connect { m =>
        FlightsAndPaxFeedOrder(
          m.portStatePot.map(_.flights.values.filter(_.apiFlight.Terminal == props.terminal)),
          m.paxFeedSourceOrder
        )
      }
      ...
      terminalFlightsProxy { terminalFlightsPot =>
        <.div(
          terminalFlightsPot().flights.renderReady { flights =>
            val apiFeedStatus = ApiFeedStatus(
              flights,
              SDate.now().millisSinceEpoch,
              props.canValidate,
              terminalFlightsPot().paxFeedSourceOrder
            )
            ...
            <.div(
              ...
              DataQualityIndicator(
                ApiDataQuality(apiFeedStatus.receivedPct.map(_.round.toInt), "Received", receivedInfoText),
                props.terminal,
                "api-received",
                icon = true
              ),
              if (props.canValidate)
                DataQualityIndicator(
                  ApiDataQuality(apiFeedStatus.validPct.map(_.round.toInt), "Valid", validatedInfoText),
                  props.terminal,
                  "api-valid",
                  icon = true
                )
          ...
          terminalFlightsPot().flights.renderPending(_ =>
            <.div(
              ...
              "API (Advance Passenger Information)",
              DataQualityIndicator(
                ApiDataQuality(None, "Received", receivedInfoText),
                props.terminal,
                "api-received",
                icon = true
              )
