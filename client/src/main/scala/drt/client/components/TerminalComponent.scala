...
object TerminalComponent {
  ...
  private case class TerminalModel(
  ...
  class Backend {
    def render(props: Props): VdomElement = {
      ...
      <.div(
        ^.className := s"${props.terminalPageTab.mode.asString}-tab",
        dialogueStateRCP(dialogueStateMP =>
          <.div(dialogueStateMP().map(dialogueState => StaffAdjustmentDialogue(dialogueState)()).whenDefined)
        ),
        modelRCP(modelMP =>
          {
            ...
            for {
              ...
              val timeRangeHours: TimeRangeHours =
                if (terminalModel.viewMode == ViewLive) CurrentWindow() else WholeDayWindow()
              ...
              <.div(
                <.div(
                  ^.className := "terminal-nav-wrapper",
                  terminalTabs(
                    props,
                    loggedInUser,
                    airportConfig,
                    terminalModel.timeMachineEnabled,
                    featureFlags.enableShiftPlanningChange
                  )
                ),
                <.div(
                  ^.className := "tab-content", {
                    ...
                    rcp { mp =>
                      ...
                      val queuesPot = terminalModel.airportConfigPot.map { ac =>
                        QueueConfig.queuesForDateRangeAndTerminal(ac.queuesByTerminal)(
                          viewStart.toLocalDate,
                          viewEnd.toLocalDate,
                          terminal
                        )
                      ...
                      val codeShares = CodeShares.uniqueFlightsWithCodeShares(
                        terminalModel.paxFeedSourceOrder,
                        terminalModel.codeShareExceptions
                      )

                      props.terminalPageTab.mode match {
                        case Current =>
                          val headerClass =
                            if (terminalModel.timeMachineEnabled) "terminal-content-header__time-machine" else ""
                          ...
                          def filterFlights(
                            ...
                            flights.filter(f =>
                              f.apiFlight.flightCodeString.toLowerCase.contains(filter.toLowerCase)
                                || f.apiFlight.Origin.iata.toLowerCase.contains(filter.toLowerCase)
                                || airportInfos.get(f.apiFlight.Origin).exists(airportInfo =>
                                  airportInfo
                          ...
                          def flightsForWindow(start: SDateLike, end: SDateLike): Pot[List[ApiFlightWithSplits]] =
                            ps.map { ps =>
                          ...
                          val windowCrunchSummaries = queuesPot.flatMap(queues =>
                            ps.map(ps => ps.crunchSummary(viewStart, hoursToView * 4, viewInterval, terminal, queues))
                          )
                          val windowStaffRecs: Pot[SortedMap[Long, Int]] = queuesPot.flatMap(queues =>
                            ps.map(ps =>
                              ps.queueRecStaffSummary(viewStart, hoursToView * 4, viewInterval, terminal, queues)
                            )
                          )
                          val windowStaffDeps: Pot[SortedMap[Long, Option[Int]]] = queuesPot.flatMap(queues =>
                            ps.map(ps =>
                              ps.queueDepStaffSummary(viewStart, hoursToView * 4, viewInterval, terminal, queues)
                            )
                          )
                          val dayCrunchSummaries: Pot[SortedMap[Long, Map[Queues.Queue, CrunchMinute]]] =
                            queuesPot.flatMap(queues =>
                              ps.map(_.crunchSummary(
                                viewStart.getLocalLastMidnight,
                                96 * 4,
                                viewInterval,
                                terminal,
                                queues
                          ...
                          val windowStaffSummaries =
                            ps.map(_.staffSummary(viewStart, hoursToView * 4, viewInterval, terminal).toMap)
                          ...
                          <.div(
                            ...
                            <.div(
                              ^.className := s"terminal-content-header $headerClass",
                        ...
                        case Planning =>
                          <.div(terminalModel.userPreferencesPot.render { userPreferences =>
                            TerminalPlanningComponent(TerminalPlanningComponent.Props(
                        ...
                        case Shifts
                            if loggedInUser.roles.contains(StaffEdit) &&
                              props.terminalPageTab.subMode == "editShifts" =>
                          <.div(EditShiftsComponent(
                            terminal,
                        ...
                        case Shifts
                            if loggedInUser.roles.contains(StaffEdit) &&
                              props.terminalPageTab.subMode == "removeShift" =>
                          <.div(RemoveShiftsComponent(
                        ...
                        case Shifts
                            if loggedInUser.roles.contains(StaffEdit) &&
                              Seq("createShifts", "addShift").contains(props.terminalPageTab.subMode) =>
                          <.div(ShiftsComponent(
                            terminal,
                            props.terminalPageTab.portCodeStr,
                            userPreferences,
                        ...
                        case Staffing
                            if loggedInUser.roles.contains(StaffEdit) && !featureFlags.enableShiftPlanningChange =>
                          <.div(MonthlyStaffingComponent(
                            props.terminalPageTab,
                            props.router,
                            airportConfig,
                            showShiftsStaffing = false,
                            userPreferences,
                            shifts.isEmpty,
                            false,
                            featureFlags.enableShiftPlanningChange
                        ...
                        case Shifts
                          ...
                          if (!userPreferences.showStaffingShiftView)
                            ...
                            val slotStaffRecs = for {
                              ...
                              val cs = portState.crunchSummary(
                                viewStart.getLocalLastMidnight,
                                1440 / viewInterval,
                                viewInterval,
                                terminal,
                            ...
                            val staffWarningsEnabled =
                              terminalModel.featureFlagsPot.map(_.enableStaffingPageWarnings).getOrElse(false)
                            MonthlyShifts(
                              terminalPageTab = props.terminalPageTab,
                              router = props.router,
                              airportConfig = airportConfig,
                              ...
                              shiftCreated = Seq(
                                "created",
                                "removed"
                              ).contains(props.terminalPageTab.queryParams.getOrElse("shifts", "")),
                        ...
                        case Shifts if loggedInUser.roles.contains(StaffEdit) && shifts.isEmpty =>
                          <.div(
                            ^.className := "staffing-container-empty",
                            "No staff shifts are currently available. Please visit the ",
                            <.strong("Staffing"),
                            " tab to create new shifts or select a different tab." +
  ...
  val component: Component[Props, Unit, Backend, CtorType.Props] = ScalaComponent.builder[Props]("Loader")
    ...
    .componentDidMount { m =>
      Callback(SPACircuit.dispatch(
        GetShifts(
          m.props.terminalPageTab.terminal.toString,
          m.props.terminalPageTab.queryParams.get("date"),
          m.props.terminalPageTab.queryParams.get("dayRange")
        )
  ...
  private def terminalTabs(
    ...
    <.ul(
      ...
      <.li(
        ^.className := tabClass(Current) + " " + timeMachineClass,
      ...
      <.li(
        ...
        props.router.link(props.terminalPageTab.update(
          mode = Planning,
          subMode = subMode,
          queryParams = props.terminalPageTab.withUrlParameters(
            UrlDateParameter(None),
            UrlTimeMachineDateParameter(None)
          ).queryParams
        ))(^.id := "planning-tab", VdomAttr("data-toggle") := "tab", "Planning")
      ),
      if (loggedInUser.roles.contains(StaffEdit) && !enableShiftPlanningChange)
        <.li(
          ^.className := tabClass(Staffing),
          props.router.link(props.terminalPageTab.update(
            mode = Staffing,
