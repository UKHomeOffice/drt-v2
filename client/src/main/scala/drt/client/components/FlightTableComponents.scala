...
object FlightTableComponents {
  ...
  def maybeLocalTimeWithPopup(
      dt: Option[MillisSinceEpoch],
      maybeToolTip: Option[TagMod] = None,
      maybeInfo: Option[String] = None
  ): TagMod = {
    dt match {
      case Some(millis) =>
        ...
        maybeInfo match {
          ...
          case Some(info) => <.span(
              ...
              ^.flexWrap := "nowrap",
              timeElement,
              <.span(^.marginLeft := "5px", Tippy.info("time-element-info", info))
            )
        }
      case None => <.span()
    }
  ...
  def pcpTimeRange(
      ...
      firstPaxOffMillis: MillisSinceEpoch,
      walkTimes: WalkTimes,
    ...
    fws.apiFlight.PcpTime.map { pcpTime: MillisSinceEpoch =>
      ...
      val sdateTo =
        SDate(MilliDate(pcpTime + millisToDisembark(fws.apiFlight.bestPcpPaxEstimate(paxFeedSourceOrder).getOrElse(0))))
      val predictedWalkTime = fws.apiFlight.Predictions.predictions.get(WalkTimeModelAndFeatures.targetName).map(c =>
        s"${c / 60}m"
      ).getOrElse("-")
      val gateOrStandWalkTime = if (fws.apiFlight.Gate.isDefined || fws.apiFlight.Stand.isDefined) {
        val wtMillis =
          walkTimes.walkTimeMillisForArrival(0L)(fws.apiFlight.Gate, fws.apiFlight.Stand, fws.apiFlight.Terminal)
        if (wtMillis != 0L) s"${(wtMillis / oneMinuteMillis).toString}m" else "-"
      } else "n/a"

      val postTouchdownTimes = <.span(
        <.h3("Minutes to chocks from touchdown"),
        s"DRT predicted: ${fws.apiFlight.Predictions.predictions.get(ToChoxModelAndFeatures.targetName).map(c =>
            s"${c.toString}m"
          ).getOrElse("-")}",
        <.br(),
        s"Feed estimated: ${estMinutesToChox(fws.apiFlight).map(c => s"${c.toString}m").getOrElse("-")}",
        <.br(),
        s"Feed actual: ${actualMinutesToChox(fws.apiFlight).map(c => s"${c.toString}m").getOrElse("-")}",
        <.br(),
        <.h3("Other times"),
        s"Chocks to doors open: ${firstPaxOffMillis / oneMinuteMillis}m",
        <.br(),
        s"Predicted walk time: $predictedWalkTime",
        <.br(),
        s"Actual walk time from gate to arrivals hall: $gateOrStandWalkTime",
        <.br()
      )
      val content = <.div(
        ^.display := "grid",
