...
object MonthlyShifts {
  ...
  class Backend(scope: BackendScope[Props, State]) {
    ...
    def populateShiftSummaries(
        recs: Map[MillisSinceEpoch, Int],
    ...
    def render(props: Props, state: State): VdomTagOf[Div] = {
      val handleShiftEditForm = (e: ReactEventFromInput) =>
        Callback {
          e.preventDefault()
      ...
      def confirmAndSaveShifts(
          shiftsData: Seq[ShiftSummaryStaffing],
          changedAssignments: Seq[StaffTableEntry],
          props: MonthlyShifts.Props,
          state: MonthlyShifts.State,
          scope: BackendScope[MonthlyShifts.Props, MonthlyShifts.State]
          ...
          <.div {
            for {
              ...
              if (state.shiftSummariesPot.isEmpty || state.intervalMinutes != forecast.intervalMinutes) {
                scope.modState { s =>
                  val recommendationsBySlotTime =
                    forecast.days.values.flatMap(_.map(slot => (slot.startMillis, slot.required))).toMap
                  val staffings =
                    populateShiftSummaries(recommendationsBySlotTime, props, viewingDate, shifts, shiftAssignments)
                  val belowRecommendedExists = staffings.flatMap(_.staffTableEntries).exists { entry =>
                    entry.staffNumber < entry.staffRecommendation
                  }
                  s.copy(
                    shiftSummariesPot = Ready(staffings),
                    intervalMinutes = forecast.intervalMinutes,
                    isBelowRecommended = belowRecommendedExists
                  )
                }.runNow()
              }
            }
                ...
                <.div(
                  <.div(
                    if (state.showStaffSuccess)
                      StaffUpdateSuccess(IStaffUpdateSuccess(
                        0,
                        "The staff numbers have been successfully updated for your chosen dates and times",
                        () => {
                    ...
                    if (shifts.isEmpty && !props.viewMode) {
                      ...
                      <.div(
                        ...
                        maybeClockChangeDate(viewingDate).map { clockChangeDate =>
                          ...
                          <.div(
                            ^.className := "staff-daylight-month-warning",
                            MuiGrid(container = true, direction = "column", spacing = 1)(
                              MuiGrid(item = true)(<.span(
                                s"BST is changing to GMT on $prettyDate",
                                ^.style := js.Dictionary("fontWeight" -> "bold")
                              )),
                              MuiGrid(item =
                                true
                              )(<.span("Please ensure no staff are entered in the cells with a dash '-'. They are there to enable you to " +
                                s"allocate staff in the additional hour on $prettyDate.")),
                              MuiGrid(item = true)(<.span(
                                "If pasting from TAMS, " +
                                  "one solution is to first paste into a separate spreadsheet, then copy and paste the first 2 hours, and " +
                                  "then the rest of the hours in 2 separate steps",
                                ^.style := js.Dictionary("marginBottom" -> "15px", "display" -> "block")
                              ))
                            )
                          )
                        },
                        MonthlyStaffingBar(
                          ...
                          confirmAndSave = ConfirmAndSaveForMonthlyShifts(
                            state.shiftSummariesPot.getOrElse(Seq.empty),
                            state.changedAssignments,
                            props,
                            state,
                            scope
                          ),
                        ...
                        MuiSwipeableDrawer(
                          ...
                          <.div(UpdateStaffForTimeRangeForm(IUpdateStaffForTimeRangeForm(
                            ...
                            handleSubmit = (ssf: IUpdateStaffForTimeRangeData) => {
                              SPACircuit.dispatch(UpdateShiftAssignments(staffAssignmentsFromForm(
                                ssf,
                                props.terminalPageTab.terminal
                              )))
                        ...
                        state.shiftSummariesPot.renderPending { _ =>
                          <.div(
                            ^.style :=
                              js.Dictionary("display" -> "flex", "justifyContent" -> "center", "paddingTop" -> "50px"),
                            MuiCircularProgress()()
                          )
                        },
                        <.div(
                        ...
                        state.shiftSummariesPot.renderReady { shiftSummaries =>
                          <.div(
                            ...
                            <.div(
                              ^.className := "shifts-table-content",
                              ShiftHotTableViewComponent(ShiftHotTableViewProps(
                                shiftDate = ShiftDate(
                                  year = viewingDate.getFullYear,
                                  month = viewingDate.getMonth,
                                  day = viewingDate.getDate
                                ),
                                viewPeriod = props.terminalPageTab.dayRangeType.getOrElse("monthly"),
                                interval = props.timeSlotMinutes,
                                shiftSummaries = shiftSummaries,
                                handleSaveChanges =
                                  (shifts: Seq[ShiftSummaryStaffing], changedAssignments: Seq[StaffTableEntry]) => {
                                    val updatedTableEntries =
                                      updateTableEntries(state.changedAssignments, changedAssignments)
                                ...
                                handleEditShift = (shiftSummary: ShiftSummary) => {
                                  props.router.set(props.terminalPageTab.copy(
                                    ...
                                    queryParams = props.terminalPageTab.queryParams ++ Map(
                                      ...
                                      "shiftDate" ->
                                        s"${shiftSummary.startDate.year}-${shiftSummary.startDate.month}-${shiftSummary.startDate.day}"
                                    )
                                  )).runNow()
                                },
                                handleRemoveShift = (shiftSummary: ShiftSummary) => {
                                  props.router.set(props.terminalPageTab.copy(
                                    ...
                                    queryParams = props.terminalPageTab.queryParams ++
                                      shiftQueryParameters(shiftSummary)
                                  )).runNow()
                                },
                            ...
                            <.div(
                              ^.className := "terminal-staffing-content-footer",
                              ...
                              if (shifts.isEmpty)
                                MuiButton(color = Color.secondary, variant = "contained")(
                                  ...
                                  ^.onClick --> props.router.set(TerminalPageTabLoc(
                                    props.terminalPageTab.terminalName,
                                    "shifts",
                                    "createShifts"
                                  ))
                                )
                              ...
                              MuiButton(color = Color.primary, variant = "contained")(
                                ...
                                ^.onClick ==>
                                  confirmAndSaveShifts(shiftSummaries, state.changedAssignments, props, state, scope)
                              )
                            )
                          )
                        }
    ...
    () =>
      {
        if (props.userPreferences.showStaffingShiftView) {
          SPACircuit.dispatch(UpdateUserPreferences(props.userPreferences.copy(showStaffingShiftView =
            !props.userPreferences.showStaffingShiftView
          )))
        }
  ...
  private def staffAssignmentsFromForm(ssf: IUpdateStaffForTimeRangeData, terminal: Terminal): Seq[StaffAssignment] = {
    ...
    DateRange(startDayLocal, endDayLocal).map { date =>
      ...
      StaffAssignment(
        ...
        terminal,
        startDateTime.millisSinceEpoch,
        endDateTime.millisSinceEpoch,
        ssf.actualStaff.toInt,
        None
      )
    }
  }
  ...
  val component: Component[Props, State, Backend, CtorType.Props] = ScalaComponent.builder[Props]("ShiftStaffing")
    ...
    .componentDidMount { m =>
      ...
      Callback(SPACircuit.dispatch(GetForecast(
        m.props.terminalPageTab.dateFromUrlOrNow,
        maxDaysInMonth,
        Terminal(m.props.terminalPageTab.terminalName),
        intervalMinutes
      )))
    }
    .build

  def updatedConvertedShiftAssignments(
      changes: Seq[StaffAssignment],
