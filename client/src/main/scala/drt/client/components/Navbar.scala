...
object Navbar {
  def apply(ctl: RouterCtl[Loc],
            ...
            applicationConfig: ApplicationConfig): VdomTagOf[html.Element] = {
    ...
    <.nav(^.className := "navbar navbar-default",
      feedStatusesRCP(feedStatusesPotMP => {
        ...
        <.div(^.className := "navbar-drt",
          ...
          <.div(^.className := "collapse navbar-collapse", MainMenu(ctl, page, feedStatusesPot.getOrElse(Seq()), airportConfig, applicationConfig, loggedInUser),
