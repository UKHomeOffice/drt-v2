...
case class NavbarModel(
    feedStatuses: Pot[Seq[FeedSourceStatuses]],
    ...
    val rcp: ReactConnectProxy[NavbarModel] = SPACircuit
      .connect(m =>
        NavbarModel(
          ...
          m.snackbarMessage,
          m.featureGuides,
          m.showNewFeatureGuideOnLogin,
          m.featureGuideViewedIds,
          m.dropIns
        )
      )
            ...
            if (isLargeDisplay || state.showDropDown)
              <.div(
                ...
                MainMenu(
                  props.ctl,
                  props.page,
                  navbarModel.feedStatuses.getOrElse(Seq()),
                  props.airportConfig,
                  props.loggedInUser
                ),
                  ...
                  navbarModel.featureGuideViewIds.renderReady { userFeatureViewCount =>
                    <.div(
                      ...
                      <.div(if (trainingDataTemplates.nonEmpty) {
                        val badgeCount = calculateBadgeCount(
                          userFeatureViewCount,
                          trainingDataTemplates.map(_.id.map(_.toString).getOrElse("0"))
                        )
                        <.a(
                          MuiBadge(
                            badgeContent = <.span(^.className := "badge-font", badgeCount),
                            invisible = badgeCount == 0,
                            color = "primary"
                          )(Icon.laptopLg),
                          <.span(if (badgeCount != 0) ^.className := "badge-text" else " ", "What's new"),
                          ^.onClick ==> handleOpenDialog
                        )
                      } else EmptyVdom),
                      navbarModel.featureGuides.renderReady { trainingDataTemplates =>
                        navbarModel.showNewFeatureGuideOnLogin.renderReady { showNewFeatureGuideOnLogin =>
                          FeatureGuideModalComponent(
                            state.toggleDialog || showNewFeatureGuideOnLogin,
                            handleDialogClose,
                      ...
                      <.div(
                        ^.className := "training-hub-link",
                        props.ctl.link(TrainingHubLoc())(Icon.calendarO, " ", "Training Hub")
                      ),
                      <.div(<.a(
                        Icon.signOut,
                        "Log Out",
                        ...
                        ^.onClick --> Callback(GoogleEventTracker.sendEvent(
                          props.airportConfig.portCode.toString,
                          "Log Out",
