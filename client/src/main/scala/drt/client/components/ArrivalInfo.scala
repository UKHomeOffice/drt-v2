...
object FeedSourceRow {
  ...
  val component: Component[Props, Unit, Unit, CtorType.Props] = ScalaComponent.builder[Props]("TableRow")
    .render_P { props =>
      ...
      val paxTotal: String = arrival.bestPcpPaxEstimate.passengers.actual.map(_.toString).getOrElse("-")
      val paxTrans: String = arrival.bestPcpPaxEstimate.passengers.transit.map(_.toString).getOrElse("-")
