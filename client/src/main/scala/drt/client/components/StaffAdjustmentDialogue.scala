...
object StaffAdjustmentDialogue {
  ...
  def apply(state: StaffAdjustmentDialogueState): Component[Unit, StaffAdjustmentDialogueState, Unit, CtorType.Nullary] =
    ScalaComponent.builder[Unit]("staffMovementPopover")
      ...
      .renderS { (scope, state) =>
        ...
        def timeSelector(): VdomTagOf[Div] = popoverFormRow(
          ...
          selectFromRange(timesBy15Minutes(0),
            f"${state.startTimeHours}%02d:${state.startTimeMinutes}%02d",
            "staff-adjustment--select-start-time", adjustStartAndEndTimeFromString),
          " for ",
          selectFromRange(0 until 1440 by 15 map (m => s"$m"),
            s"${state.lengthOfTimeMinutes}",
            "staff-adjustment--select-time-length", adjustLengthFromString),
          " minutes"
        )
        ...
        def endTimeSelector(): VdomTagOf[Div] = {
          ...
          popoverFormRow(
            ...
            selectFromRange(timesBy15Minutes(state.startTimeHours),
