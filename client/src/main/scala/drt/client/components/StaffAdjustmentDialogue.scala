...
import drt.client.actions.Actions.{
  AddStaffMovements,
  RecordClientSideStaffMovement,
  UpdateStaffAdjustmentDialogueState
}
...
case class StaffAdjustmentDialogueState(
    action: String,
...
object StaffAdjustmentDialogue {
  ...
  def apply(state: StaffAdjustmentDialogueState)
      : Component[Unit, StaffAdjustmentDialogueState, Unit, CtorType.Nullary] =
    ScalaComponent.builder[Unit]("staffMovementPopover")
      .initialState(state)
      .renderS { (scope, state) =>
        ...
        def trySaveMovement(): Unit = {
          ...
          StaffAssignmentHelper
            .tryStaffAssignment(
              ...
              state.date,
              startTime,
              state.lengthOfTimeMinutes,
              numberOfStaff,
              createdBy = Option(state.loggedInUser.email)
            ) match {
            case Success(staffAssignment) =>
              val movementsToAdd =
                for (movement <- StaffMovements.assignmentsToMovements(Seq(staffAssignment))) yield movement
              SPACircuit.dispatch(AddStaffMovements(movementsToAdd))
              SPACircuit.dispatch(RecordClientSideStaffMovement(
                state.terminal,
                staffAssignment.start,
                staffAssignment.end,
                staffAssignment.numberOfStaff
              ))
              GoogleEventTracker.sendEvent(
                state.terminal.toString,
                "Add StaffMovement",
                staffAssignment.copy(createdBy = None).toString
              )
              killPopover()
            case _ =>
          }
        ...
        val adjustStartAndEndTimeFromString: String => StaffAdjustmentDialogueState => StaffAdjustmentDialogueState =
          (v: String) =>
            (s: StaffAdjustmentDialogueState) => {
              val Array(hours, minutes) = v.split(":").map(_.toInt)
              s.copy(startTimeHours = hours, startTimeMinutes = minutes)
            }

        val adjustLengthFromString = (v: String) =>
          (s: StaffAdjustmentDialogueState) => {
        ...
        def timeSelector(): VdomTagOf[Div] = popoverFormRow(
          ...
          selectFromRange(
            timesBy15Minutes(0),
            f"${state.startTimeHours}%02d:${state.startTimeMinutes}%02d",
            "staff-adjustment--select-start-time",
            adjustStartAndEndTimeFromString
          ),
          " for ",
          selectFromRange(
            0 until 1440 by 15 map (m => s"$m"),
            s"${state.lengthOfTimeMinutes}",
            "staff-adjustment--select-time-length",
            adjustLengthFromString
          ),
          " minutes"
        )

        val adjustLengthFromEndTime: String => StaffAdjustmentDialogueState => StaffAdjustmentDialogueState =
          (v: String) =>
            (s: StaffAdjustmentDialogueState) => {
              val Array(hours, minutes) = v.split(":").map(_.toInt)
        ...
        def endTimeSelector(): VdomTagOf[Div] = {
          ...
          popoverFormRow(
            ...
            selectFromRange(
              timesBy15Minutes(state.startTimeHours),
              f"$endHours%02d:$endMinutes%02d",
              "staff-adjustment--select-end-time",
              adjustLengthFromEndTime
            )
          )
        }
        ...
        def reasonSelector(): VdomTagOf[Div] = popoverFormRow(
          ...
          selectFromRange(
            movementReasons,
            state.reason,
            "staff-adjustment--select-reason",
            (v: String) => (s: StaffAdjustmentDialogueState) => s.copy(reason = v),
            autoFocus = true
          )
        )
  ...
  def saveAndCancel(killPopover: () => Unit, trySaveMovement: () => Unit): VdomTagOf[Div] = {
    <.div(
      ...
      <.div(
        ^.className := "col-sm-8 btn-toolbar",
        <.button(
          "Save",
          ^.className := "btn btn-primary staff-adjustment--save-cancel",
          ^.onClick --> Callback(trySaveMovement())
        ),
        <.button(
          "Cancel",
          ^.className := "btn btn-default staff-adjustment--save-cancel",
          ^.onClick --> Callback(killPopover())
        )
      )
    )
  }
