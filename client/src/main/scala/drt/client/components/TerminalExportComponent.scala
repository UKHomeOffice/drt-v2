...
object TerminalExportComponent extends WithScalaCssImplicits {
  ...
  val component: Component[Props, State, Unit, CtorType.Props] =
    ScalaComponent.builder[Props]("TerminalExportComponent")
      ...
      .renderPS((scope, props, state) => {
        ...
        val title = if (props.terminals.size > 1) s"Export $exportNameLC for ${props.terminal} or all terminals"
        else s"${props.exportName} for ${props.terminal.toString}"
        ...
        <.div(
          ...
          <.div(
            ^.className := s"terminals-export modal " + showClass,
            ^.id := s"#terminals-export",
            ^.tabIndex := -1,
            ^.role := "dialog",
            <.div(
  ...
  private val defaultDeploymentExport: (Terminal, SDateLike, LoggedInUser, ViewMode, String) => VdomElement =
    (term, date, _, viewMode, exportName) => {
  ...
  def componentFactory(
      terminals: Iterable[Terminal],
      exportName: String
  ): (Terminal, SDateLike, LoggedInUser, ViewMode) => VdomElement =
