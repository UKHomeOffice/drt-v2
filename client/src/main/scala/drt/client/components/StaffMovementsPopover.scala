  ...
  def apply(terminalNames: Seq[TerminalName], page: Loc, trigger: String, reason: String, startDate: SDateLike, endDate: SDateLike, popoverPosition: String) = ScalaComponent.builder[Unit]("staffMovementPopover")
    ...
    def popoverFormRow(label: String, xs: TagMod*) = {
      <.div(^.className := "form-group row",
        ...
        <.div(^.className := "col-sm-10", xs.toTagMod))
    }
    ...
    def hoveredComponent: TagMod = if (state.hovered) {
      val popoverChildren = <.div(^.className := "container", ^.key := "IS81",
      ...
      val props = PopoverWrapper.props(trigger = trigger, className = "staff-movement-popover", position = popoverPosition)
      PopoverWrapper.component(props)(popoverChildren)
    } else {
