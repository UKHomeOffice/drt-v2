...
case class FlightsProvider(flightsRouterActor: ActorRef)(implicit timeout: Timeout) {
  def terminalDateScheduledOrPcp(implicit
      mat: Materializer
  ): Terminal => (LocalDate, Option[Long]) => Future[Seq[ApiFlightWithSplits]] =
    terminal =>
      (localDate, maybePointInTime) => {
  ...
  def terminalDateScheduled(implicit
      mat: Materializer,
      ec: ExecutionContext
  ): Terminal => (LocalDate, Option[Long]) => Future[Seq[ApiFlightWithSplits]] =
    terminal =>
  ...
  def terminalScheduledOn: Terminal => UtcDate => Source[Seq[ApiFlightWithSplits], NotUsed] =
    terminal =>
      date => {
        ...
        val request =
          PartitionedPortStateActor.GetFlightsForTerminals(start.millisSinceEpoch, end.millisSinceEpoch, Seq(terminal))
