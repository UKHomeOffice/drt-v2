...
object MinutesProvider {
  def singleTerminal[A, B <: WithTimeAccessor](minutesRouterActor: ActorRef)(implicit
      timeout: Timeout
  ): Terminal => (UtcDate, UtcDate) => Source[(UtcDate, Seq[A]), NotUsed] =
    terminal =>
      (start, end) => {
  ...
  def allTerminals[A, B <: WithTimeAccessor](minutesRouterActor: ActorRef)(implicit
      timeout: Timeout
  ): (UtcDate, UtcDate) => Source[(UtcDate, Seq[A]), NotUsed] =
    (start, end) => {
      val request = actors.routing.minutes.GetStreamingMinutesForDateRange(start, end)
      minutesByUtcDate(minutesRouterActor, request)
    }

  private def minutesByUtcDate[B <: WithTimeAccessor, A](
      staffMinutesRouterActor: ActorRef,
      request: UtcDateRangeLike
