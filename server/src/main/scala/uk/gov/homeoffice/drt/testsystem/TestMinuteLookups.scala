...
case class TestMinuteLookups(
  ...
  override val requestAndTerminateActor: ActorRef =
    system.actorOf(Props(new RequestAndTerminateActor()), "test-minutes-lookup-kill-actor")

  private val resetQueuesData: (Terminal, MillisSinceEpoch) => Future[Any] =
    (terminal: Terminal, millis: MillisSinceEpoch) => {
  ...
  private val resetStaffData: (Terminal, MillisSinceEpoch) => Future[Any] =
    (terminal: Terminal, millis: MillisSinceEpoch) => {
  ...
  override val queueLoadsMinutesActor: ActorRef =
    system.actorOf(Props(new TestQueueLoadsMinutesActor(
  ...
  override val queueMinutesRouterActor: ActorRef =
    system.actorOf(Props(new TestQueueMinutesRouterActor(
      terminalsForDateRange,
  ...
  override val staffMinutesRouterActor: ActorRef =
    system.actorOf(Props(new TestStaffMinutesRouterActor(
      terminalsForDateRange,
      staffLookup,
      updateStaffMinutes,
      resetStaffData
