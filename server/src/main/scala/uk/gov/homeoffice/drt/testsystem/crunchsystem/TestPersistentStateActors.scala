...
case class TestPersistentStateActors(
    system: ActorSystem,
    now: () => SDateLike,
    minutesToCrunch: Int,
    offsetMinutes: Int,
  ...
  override val manifestsRouterActor: ActorRef =
    system.actorOf(
      Props(new TestVoyageManifestsActor(
  ...
  override val mergeArrivalsQueueActor: ActorRef =
    system.actorOf(
      Props(new TestMergeArrivalsQueueActor(now = () => SDate.now(), terminals)),
      "merge-arrivals-queue-actor"
    )
