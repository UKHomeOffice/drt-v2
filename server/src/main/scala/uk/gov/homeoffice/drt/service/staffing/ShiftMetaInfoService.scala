...
case class ShiftMetaInfoServiceImpl(shiftMetaInfoDao: ShiftMetaInfoDao)(implicit ex: ExecutionContext)
    extends ShiftMetaInfoService {
  ...
  override def getShiftMetaInfo(port: String, terminal: String): Future[Option[ShiftMeta]] =
    shiftMetaInfoDao.getShiftMetaInfo(port, terminal)
      .map(_.map(shiftMetaRow =>
        ShiftMeta(shiftMetaRow.port, shiftMetaRow.terminal, shiftMetaRow.shiftAssignmentsMigratedAt)
      ))
