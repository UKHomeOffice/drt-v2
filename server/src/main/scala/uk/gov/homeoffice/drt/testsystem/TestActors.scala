...
import actors.persistent.staffing.{
  FixedPointsActorLike,
  ShiftsActorLike,
  StaffMovementsActorLike,
  StaffMovementsState
}
...
object TestActors {
  ...
  private class TestStreamingUpdatesActor[T, S](
  ...
  ) extends StreamingUpdatesActor[T, S](
        persistenceId,
        journalType,
        initialState,
  ...
  object TestShiftsActor extends ShiftsActorLike {
    override def streamingUpdatesProps(
        persistenceId: String,
        journalType: StreamingJournalLike,
        now: () => SDateLike
    ): Props =
  ...
  class TestStaffMinutesRouterActor(
      ...
      val resetData: (Terminal, MillisSinceEpoch) => Future[Any]
  ) extends StaffMinutesRouterActor(terminalsForDateRange, lookup, updateMinutes)
      with TestMinuteActorLike[StaffMinute, TM] {
    override def receive: Receive = resetReceive orElse super.receive
  }

  class TestQueueMinutesRouterActor(
      terminalsForDateRange: (LocalDate, LocalDate) => Seq[Terminal],
      lookup: MinutesLookup[CrunchMinute, TQM],
      updateMinutes: MinutesUpdate[CrunchMinute, TQM, TerminalUpdateRequest],
      val resetData: (Terminal, MillisSinceEpoch) => Future[Any]
  ) extends QueueMinutesRouterActor(terminalsForDateRange, lookup, updateMinutes)
      with TestMinuteActorLike2[CrunchMinute, TQM] {
    override def receive: Receive = resetReceive orElse super.receive
  }

  class TestQueueLoadsMinutesActor(
      terminalsForDateRange: (LocalDate, LocalDate) => Seq[Terminal],
      lookup: MinutesLookup[PassengersMinute, TQM],
      updateMinutes: MinutesUpdate[PassengersMinute, TQM, TerminalUpdateRequest],
      val resetData: (Terminal, MillisSinceEpoch) => Future[Any]
  ) extends QueueLoadsMinutesActor(terminalsForDateRange, lookup, updateMinutes)
      with TestMinuteActorLike2[PassengersMinute, TQM] {
    override def receive: Receive = resetReceive orElse super.receive
  }
  ...
  class TestFeedArrivalsRouterActor(
      allTerminals: (LocalDate, LocalDate) => Iterable[Terminal],
  ...
  class TestPartitionedPortStateActor(
  ...
  class TestTerminalDayQueuesActor(
      date: UtcDate,
      terminal: Terminal,
  ...
  class TestTerminalDayStaffActor(
      utcDate: UtcDate,
      terminal: Terminal,
      now: () => SDateLike
  ...
  class TestTerminalDayFlightActor(
      year: Int,
      month: Int,
      day: Int,
      terminal: Terminal,
  ...
  class QueueTestUpdatesSupervisor(
      now: () => SDateLike,
  ...
  class StaffTestUpdatesSupervisor(
      now: () => SDateLike,
      terminals: LocalDate => Seq[Terminal],
      updatesActorFactory: (Terminal, SDateLike) => Props
  ) extends TestUpdatesSupervisor[StaffMinute, TM](now, terminals, updatesActorFactory)

  abstract class TestUpdatesSupervisor[A, B <: WithTimeAccessor](
      now: () => SDateLike,
      terminals: LocalDate => Seq[Terminal],
      updatesActorFactory: (Terminal, SDateLike) => Props
  ...
  class TestFlightUpdatesSupervisor(
      now: () => SDateLike,
      terminals: LocalDate => Seq[Terminal],
      updatesActorFactory: (Terminal, SDateLike) => Props
