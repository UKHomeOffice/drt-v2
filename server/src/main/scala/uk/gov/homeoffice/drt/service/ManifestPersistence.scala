...
object ManifestPersistence {
  ...
  private def persistSplitsFromManifest(
      flightsForDate: UtcDate => Source[(UtcDate, Seq[ApiFlightWithSplits]), NotUsed],
    ...
    keys =>
      Source(keys.groupBy(_.scheduled.toUtcDate))
        .flatMapConcat {
          case (date, manifests) =>
  ...
  private def manifestsFeedResponsePersistor(manifestsRouterActor: ActorRef)(implicit
      timeout: Timeout,
      ec: ExecutionContext
  ): ManifestsFeedResponse => Future[Done] =
    response => manifestsRouterActor.ask(response).map(_ => Done)

  private def manifestsToSplitsPersistor(
  ...
  def processManifestFeedResponse(
      maybeManifestsRouterActor: Option[ActorRef],
      flightsRouterActor: ActorRef,
      splitsForManifest: (ManifestLike, Terminal) => Splits
