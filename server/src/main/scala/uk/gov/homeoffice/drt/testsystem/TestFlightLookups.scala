...
case class TestFlightLookups(system: ActorSystem,
  ...
  val resetFlightsData: (Terminal, UtcDate) => Future[Any] = (terminal: Terminal, date: UtcDate) => {
    val actor = system.actorOf(Props(new TestTerminalDayFlightActor(date.year, date.month, date.day, terminal, now, paxFeedSourceOrder, None, None, processingRequests)))
  ...
  override val flightsRouterActor: ActorRef = system.actorOf(
    Props(
      new TestFlightsRouterActor(
        ...
        updateFlights(None, processingRequests),
        resetFlightsData,
        paxFeedSourceOrder,
      )))
}
