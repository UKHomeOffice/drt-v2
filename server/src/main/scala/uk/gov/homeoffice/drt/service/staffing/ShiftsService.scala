...
trait ShiftsService {
  ...
  def latestStaffShiftForADate(port: String, terminal: String, startDate: LocalDate, startTime: String)(implicit
      ec: ExecutionContext
  ): Future[Option[Shift]]
  ...
  def getActiveShiftsForViewRange(
      port: String,
      terminal: String,
...
case class ShiftsServiceImpl(staffShiftsDao: StaffShiftsDao)(implicit val ec: ExecutionContext) extends ShiftsService {
  ...
  override def deleteShift(shift: Shift): Future[Shift] = staffShiftsDao.deleteStaffShift(
    shift.port,
    shift.terminal,
    shift.shiftName,
    shift.startDate,
    ...
    getShifts(port, terminal).map { shifts =>
      ...
      dayRange.map(_.toLowerCase) match {
        case Some("weekly") =>
          ...
          shifts.filter(shift =>
            (shift.startDate.compare(weekEnd) <= 0) &&
              (shift.endDate.exists(ed => ed.compare(weekStart) >= 0) || shift.endDate.isEmpty)
          )
        case Some("daily") => shifts.filter(shift =>
            shift.startDate.compare(localDateFromString(date)) <= 0 &&
              (shift.endDate.isEmpty || shift.endDate.exists(_.compare(localDateFromString(date)) >= 0))
          )
        case _ => shifts.filter(shift =>
            shift.startDate.compare(LocalDate(year = viewDate.year, month = viewDate.month + 1, day = 1)) < 0 &&
              (shift.endDate.isEmpty || shift.endDate.exists(ed =>
                ed.compare(LocalDate(year = viewDate.year, month = viewDate.month, day = 1)) >= 0
              ))
          )
      }
    }

  override def updateShift(previousShift: Shift, shift: Shift): Future[Shift] =
    staffShiftsDao.updateStaffShift(previousShift, shift)
