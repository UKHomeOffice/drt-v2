...
case class TestFeedService(
  ...
  def resetFlightsData(
    ...
    val actor =
      system.actorOf(props(date.year, date.month, date.day, terminal, source, None, nowMillis, expireAfterMillis))
  ...
  private def feedArrivalsRouter(
    ...
    system.actorOf(
      Props(new TestFeedArrivalsRouterActor(
