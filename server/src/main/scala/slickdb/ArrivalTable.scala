...
case class ArrivalTable(portCode: PortCode, tables: Tables) extends ArrivalTableLike {
  ...
  def arrivalRow(f: shared.api.Arrival): tables.ArrivalRow = {
    ...
    ArrivalRow(f.flightCodeString, f.VoyageNumber.numeric, portCode.iata, f.Origin.toString, f.Terminal.toString, f.Gate, f.Stand, f.Status.description, sch, est, act, estChox, actChox, pcp, f.ActPax, pcpPax, f.ServiceType, f.LoadFactor)
  }
}
