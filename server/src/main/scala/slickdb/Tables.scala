...
trait Tables {
  ...
  implicit def GetResultArrivalRow(implicit e0: GR[String], e1: GR[Int], e2: GR[Option[String]], e3: GR[java.sql.Timestamp], e4: GR[Option[java.sql.Timestamp]], e5: GR[Option[Int]]): GR[ArrivalRow] = GR{
    prs => import prs._
      ArrivalRow.tupled((<<[String], <<[Int], <<[String], <<[String], <<[String], <<?[String], <<?[String], <<[String], <<[java.sql.Timestamp], <<?[java.sql.Timestamp], <<?[java.sql.Timestamp], <<?[java.sql.Timestamp], <<?[java.sql.Timestamp], <<[java.sql.Timestamp], <<?[Int], <<?[Int],<<?[String],<<?[Double]))
  }
  ...
  class Arrival(_tableTag: Tag) extends {
    ...
    def * = (code, number, destination, origin, terminal, gate, stand, status, scheduled, estimated, actual, estimatedchox, actualchox, pcp, totalpassengers, pcppassengers,servicetype,loadfactor) <> (ArrivalRow.tupled, ArrivalRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(code), Rep.Some(number), Rep.Some(destination), Rep.Some(origin), Rep.Some(terminal), gate, stand, Rep.Some(status), Rep.Some(scheduled), estimated, actual, estimatedchox, actualchox, Rep.Some(pcp), totalpassengers, pcppassengers,servicetype,loadfactor).shaped.<>({r=>import r._; _1.map(_=> ArrivalRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6, _7, _8.get, _9.get, _10, _11, _12, _13, _14.get, _15, _16,_17,_18)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Database column pcppassengers SqlType(int4), Default(None) */
    val servicetype: Rep[Option[String]] = column[Option[String]]("servicetype")
    /** Database column pcppassengers SqlType(int4), Default(None) */
    val loadfactor: Rep[Option[Double]] = column[Option[Double]]("loadfactor")
