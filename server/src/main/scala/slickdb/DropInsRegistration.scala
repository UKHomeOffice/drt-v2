...
case class DropInsRegistrationRow(
    email: String,
    dropInId: Int,
    registeredAt: Timestamp,
    emailSentAt: Option[Timestamp]
) {
  def toDropInRegistration: DropInRegistration =
    DropInRegistration(email, dropInId, registeredAt.getTime, emailSentAt.map(_.getTime))
}
...
class DropInsRegistration(tag: Tag) extends Table[DropInsRegistrationRow](tag, "drop_in_registration") {
  ...
  def * : ProvenShape[DropInsRegistrationRow] =
    (email, dropInId, registeredAt, emailSentAt).mapTo[DropInsRegistrationRow]
...
case class DropInsRegistrationTable(tables: AggregatedDbTables) extends DropInsRegistrationTableLike {
  ...
  def createDropInRegistration(email: String, id: String)(implicit ex: ExecutionContext): Future[Int] = {
    val insertAction = dropInsRegistrationTable +=
      DropInsRegistrationRow(email, id.toInt, getCurrentTime, Some(getCurrentTime))
