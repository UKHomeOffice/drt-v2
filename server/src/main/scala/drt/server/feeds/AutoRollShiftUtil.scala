...
object AutoRollShiftUtil {
  ...
  def existingCheckAndUpdate(
    ...
    if (monthsToAdd > 0) shiftStaffRollingService.latestShiftStaffRolling(port, terminal.toString).flatMap { _ =>
        ...
        if (shifts.nonEmpty) {
          shiftStaffRollingService.upsertShiftStaffRolling(
            ShiftStaffRolling(
              ...
              rollingStartDate =
                SDate(startRollingDate.year, startRollingDate.month, startRollingDate.day).millisSinceEpoch,
          ...
          log.info(
            s"updateShiftsStaffingToAssignments :AutoShiftStaffing updated shifts for $port from ${startRollingDate.toISOString} to ${endRollingDate.toISOString} for terminal $terminal"
          )
        }
        updatedAssignments
      }
      ...
      log.info(
        s"updateShiftsStaffingToAssignments :AutoShiftStaffing no update needed for $port for terminal $terminal as monthsToAdd is $monthsToAdd"
      )
      Future.successful(ShiftAssignments(Seq.empty[StaffAssignmentLike]))
    }
  }
