...
object LHRForecastXLSExtractor {
  ...
  def apply(xlsFilePath: String) = {
    ...
    val arrivals = for {
      ...
      direct <- row.cells.find(cell => cell.index == 6 && cell.isInstanceOf[NumericCell]).map(c => c.asInstanceOf[NumericCell].data)
      transfer <- row.cells.find(cell => cell.index == 7 && cell.isInstanceOf[NumericCell]).map(c => c.asInstanceOf[NumericCell].data)
    } yield LHRForecastFlightRow(scheduledDate = SDate(DateUtil.getJavaDate(flightDate).getTime), flightCode = number, origin = airport, internationalDomestic = internationalDomestic, totalPax = total.toInt, transferPax = transfer.toInt, terminal)
