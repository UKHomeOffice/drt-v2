...
object StreamingChromaFlow {
  ...
  def chromaPollingSource[X <: ChromaFlightLike](
      chromaFetcher: ChromaFetcher[X],
  ...
  def liveChromaToFeedArrival(chromaArrivals: Seq[ChromaLiveFlight]): List[FeedArrival] = {
    chromaArrivals.map(flight => {
      ...
      val (carrierCode, voyageNumber, maybeSuffix) =
        FlightCode.flightCodeToParts(if (flight.ICAO != "") flight.ICAO else flight.IATA)
      LiveArrival(
