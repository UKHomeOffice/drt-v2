...
import uk.gov.homeoffice.drt.services.exports.{
  AdminExportImpl,
...
class FlightsExportController @Inject() (cc: ControllerComponents, ctrl: DrtSystemInterface)
    extends AuthController(cc, ctrl) {

  def exportFlightsWithSplitsTerminalsForDayAtPointInTimeCSV(
      localDateString: String,
      pointInTime: MillisSinceEpoch
  ): Action[AnyContent] = {
    val terminals = ctrl.airportConfig.terminalsForDate(
      LocalDate.parse(localDateString).getOrElse(throw new Exception(s"Could not parse local date '$localDateString''"))
    )
    doExportForPointInTime(localDateString, pointInTime, terminals.toSeq, exportForUser)
  }

  def exportFlightsWithSplitsForDayAtPointInTimeCSV(
      localDateString: String,
      pointInTime: MillisSinceEpoch,
      terminalName: String
  ): Action[AnyContent] =
  ...
  private def doExportForPointInTime(
      localDateString: String,
      pointInTime: MillisSinceEpoch,
      terminals: Seq[Terminal],
      exportTerminalDateRange: (
          LoggedInUser,
          PortCode,
          RedListUpdates
        ...
        maybeDate match {
          case Some(localDate) =>
            ctrl.applicationService.redListUpdatesActor.ask(GetState).mapTo[RedListUpdates].flatMap { redListUpdates =>
              requestToCsvStream(
                Option(pointInTime),
                exportTerminalDateRange(user, airportConfig.portCode, redListUpdates)(localDate, localDate, terminals)
              )
  ...
  def exportTerminalFlightsWithSplitsForDateRangeCSV(
  ...
  def exportFlightsWithSplitsForDateRangeCSV(
      startLocalDateString: String,
      endLocalDateString: String
  ): Action[AnyContent] = authByRole(ArrivalsAndSplitsView) {
    val start = LocalDate.parse(
      startLocalDateString
    ).getOrElse(throw new Exception(s"Could not parse local date '$startLocalDateString''"))
    val end = LocalDate.parse(
      endLocalDateString
    ).getOrElse(throw new Exception(s"Could not parse local date '$endLocalDateString''"))
  ...
  private def doExportForDateRange(
      startLocalDateString: String,
      endLocalDateString: String,
      terminals: Seq[Terminal],
      exportTerminalDateRange: (
        ...
        (LocalDate.parse(startLocalDateString), LocalDate.parse(endLocalDateString)) match {
          case (Some(start), Some(end)) =>
            ctrl.applicationService.redListUpdatesActor.ask(GetState).mapTo[RedListUpdates].flatMap { redListUpdates =>
              requestToCsvStream(
                None,
                exportTerminalDateRange(user, airportConfig.portCode, redListUpdates)(start, end, terminals)
              )
  ...
  private def requestToCsvStream(
      maybePointInTime: Option[MillisSinceEpoch],
      `export`: FlightsWithSplitsExport
  ): Future[Source[String, NotUsed]] = {
    val eventualFlightsByDate: Future[Source[(UtcDate, Iterable[ApiFlightWithSplits]), NotUsed]] =
  ...
  private val exportForUser
      : (LoggedInUser, PortCode, RedListUpdates) => (LocalDate, LocalDate, Seq[Terminal]) => FlightsWithSplitsExport =
    (user, _, _) =>
      (start, end, terminals) =>
