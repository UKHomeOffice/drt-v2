...
class ABFeatureController @Inject() (cc: ControllerComponents, ctrl: DrtSystemInterface)
  ...
  def getABFeature(functionName: String): Action[AnyContent] = Action.async { implicit request =>
    ...
    val abFeatureRowsF: Future[Seq[ABFeatureRow]] =
      ctrl.abFeatureService.getABFeaturesByEmailForFunction(userEmail, functionName)
      ...
      abFeatureRows.size match {
        case 0 =>
          ...
          ctrl.abFeatureService.insertOrUpdate(row).map { _ =>
            ...
            case e =>
              log.warning(s"Error while db insert for ab feature: ${e.getMessage}")
