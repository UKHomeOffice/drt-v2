...
import uk.gov.homeoffice.drt.db.serialisers.{
  EgateEligibility,
  EgateSimulation,
  EgateSimulationRequest,
  EgateSimulationResponse
}
...
class SimulationsController @Inject() (cc: ControllerComponents, ctrl: DrtSystemInterface)
  ...
  def doEgateSimulation(
      terminalName: String,
      startDate: String,
      endDate: String,
      uptakePercentage: Double
  ): Action[AnyContent] = authByRole(SuperAdmin) {
    Action.async { request =>
      ...
      val start =
        UtcDate.parse(startDate).getOrElse(throw new IllegalArgumentException(s"Invalid start date: $startDate"))
      val end = UtcDate.parse(endDate).getOrElse(throw new IllegalArgumentException(s"Invalid end date: $endDate"))

      val egateSimulationRequest =
        EgateSimulationRequest(ctrl.airportConfig.portCode, terminal, start, end, uptakePercentage, childParentRatio)
      val dao = EgateSimulationDao()

      ctrl.aggregatedDb.run(dao.get(egateSimulationRequest)).flatMap {
        case Some(existingSimulation) =>
          log.info(
            s"Egate uptake simulation already exists for request: $egateSimulationRequest with UUID: ${existingSimulation.uuid}"
          )
        ...
        case None =>
          ...
          ctrl.aggregatedDb.run(dao.insertOrUpdate(egateSimulation)).map { _ =>
            log.info(
              s"Starting egate uptake simulation for terminal $terminal from $start to $end with uptake percentage $uptakePercentage%"
            )
                ...
                bxAndDrtStatsForDate(date, terminal).map {
                  case (bxTotalPax, bxPercentage, bxUptakePct, drtTotalPax, drtPercentage) =>
                    ...
                    ctrl.aggregatedDb.run(
                      dao.insertOrUpdate(egateSimulation.copy(status =
                        s"processing - ${progressPercentage.toInt}% complete"
                      ))
                    )
            ...
            stream
              ...
              .map { rows =>
                ...
                ctrl.aggregatedDb.run(dao.insertOrUpdate(egateSimulation.copy(
                  status = "completed",
  ...
  def optimiseEgates(terminalName: String, startDate: String, endDate: String): Action[AnyContent] =
      ...
      Action {
        ...
        def uptakeStdDev(x: Double): Double = {
          ...
          if (ys.size < (dates.size.toDouble * 0.85)) {
            ...
            log.info(
              s"Uptake percentages for adult-child ratio $x ($x: ${ys.size} out of ${dates.size}): ${ys.mkString(", ")}"
            )
        ...
        log.info(
          s"Optimised egate simulation for terminal $terminal from $startDate to $endDate: Adult-Child Ratio: $bestAdultChildRatio, Uptake Percentage: $bestUptakePct"
        )
  ...
  private def optimiseWithBounds(
  ...
  private def bxAndDrtStatsForUptakeAndAdultChildRatio()
      : (Double, Double) => (UtcDate, Terminal) => Future[(Int, Double, Double, Int, Double)] = {
    val historicManifest: UniqueArrivalKey => Future[Option[BestAvailableManifest]] =
      (ua: UniqueArrivalKey) =>
        ctrl.applicationService.manifestLookupService.maybeBestAvailableManifest(
          ua.arrivalPort,
          ua.departurePort,
          ua.voyageNumber,
          ua.scheduled
    ...
    val flightsWithPcpStartDuringDate: (UtcDate, Terminal) => Future[Seq[Arrival]] =
      (d: UtcDate, t: Terminal) => {
        ...
        ctrl
          ...
          .map { case (_, flights) =>
            flights.collect {
              case fws if fws.apiFlight.hasPcpDuring(start, end, ctrl.feedService.paxFeedSourceOrder) =>
                fws.unique -> fws.apiFlight
            }.toMap.values
          }
          .runWith(Sink.seq)
          .map(_.flatten)
    ...
    val storeEgateEligibility: (UtcDate, Terminal, Int, Double, Double) => Unit =
      (date, terminal, tot, el, ua) =>
        ctrl.aggregatedDb.run(egateEligibilityDao.insertOrUpdate(EgateEligibility(
      ...
      (uptakePercentage, adultChildRatio) =>
        EgateSimulations.drtTotalAndEgateEligibleAndActualPercentageForDateAndTerminal(uptakePercentage)(
          cachedEgateEligibleAndUnderAgeForDate =
            (terminal, date) => ctrl.aggregatedDb.run(egateEligibilityDao.get(airportConfig.portCode, date, terminal)),
    ...
    (up: Double, acr: Double) =>
      EgateSimulations.bxAndDrtStatsForDate(
        bxEgatePercentage,
        drtEgatePercentage(up, acr),
        EgateSimulations.bxUptakePct
      )
  }
  ...
  private def deskRecsForSimulation(
      simulationParams: SimulationParams,
      simulationConfig: AirportConfig
  ): Future[DeskRecMinutes] = {
    implicit val timeout: Timeout = new Timeout(10 minutes)

    val date = SDate(simulationParams.date)
    val eventualFlightsWithSplitsStream =
      (ctrl.actorService.portStateActor ? GetFlightsForTerminalDateRange(
    ...
    FlightsRouterActor.runAndCombine(eventualFlightsWithSplitsStream).map { fws =>
      ...
      val deskLimits = PortDeskLimits.flexed(
        simulationConfig,
        terminalEgateBanksFromParams(simulationParams),
        ctrl.applicationService.paxForQueue
      )
  ...
  private def portEgateBanksFromParams(simulationParams: SimulationParams): () => Future[PortEgateBanksUpdates] =
    () =>
      terminalEgateBanksFromParams(simulationParams)(simulationParams.terminal).map {
        case EgateBanksUpdates(updates) =>
          PortEgateBanksUpdates(Map(simulationParams.terminal -> EgateBanksUpdates(updates)))
      }
  ...
  private def simulationResultAsCsv(
      simulationParams: SimulationParams,
      terminal: Terminal,
      futureDeskRecMinutes: Future[DeskRecMinutes]
  ): Future[Result] = {

    val date = SDate(simulationParams.date)

    futureDeskRecMinutes.map(deskRecMinutes => {
      val crunchMinutes: SortedMap[TQM, CrunchMinute] = SortedMap[TQM, CrunchMinute]() ++
        deskRecMinutes
          .minutes
          .map(dr => dr.key -> dr.toMinute).toMap

      val fileName = CsvFileStreaming.makeFileName(
        s"simulation-${simulationParams.passengerWeighting}",
