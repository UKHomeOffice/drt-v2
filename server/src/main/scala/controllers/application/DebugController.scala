...
class DebugController @Inject() (cc: ControllerComponents, ctrl: DrtSystemInterface) extends AuthController(cc, ctrl) {
  ...
  private case class ActorTraceDiagnostics(
  ...
  private def fetchFlightDiffMessagesWithDiagnostics(
      persistenceId: String,
      pit: SDateLike,
      messages: Int
  ): Future[ActorTraceDiagnostics] = {
  ...
  private def feedDiffEventsFromMessage(
    ...
    if (!isFeedDiff) Seq.empty
      ...
      if ((containsFlightCode || containsCarrierAndVoyage) && matchesScheduled) {
        ...
        val estimated =
          firstMatch(estimatedRegex, proto).flatMap(v => Try(v.toLong).toOption).map(v => SDate(v).toISOString)
        val actualChox =
          firstMatch(actualChoxRegex, proto).flatMap(v => Try(v.toLong).toOption).map(v => SDate(v).toISOString)
  ...
  private def traceEventsFromFlightsDiff(
      diff: FlightsWithSplitsDiffMessage,
      persistenceId: String,
      flightCodeUpper: String,
    ...
    val updates: Seq[(Long, String, JsObject)] = diff.updates.collect {
      case update
          if update.getFlight.getIATA.toUpperCase == flightCodeUpper &&
            scheduledMatches(maybeScheduledMillis, update.getFlight.getScheduled) =>
        (
          createdAt,
          persistenceId,
          Json.obj(
    ...
    val removals: Seq[(Long, String, JsObject)] = diff.removals.collect {
      case removal
          if flightNumberOnly.nonEmpty && removal.getNumber.toString == flightNumberOnly &&
            scheduledMatches(maybeScheduledMillis, removal.getScheduled) =>
        (
          createdAt,
          persistenceId,
          Json.obj(
  ...
  private def actorDiagnosticsJson(
      actorData: Seq[ActorTraceDiagnostics],
      actorMatchCounts: Map[String, Int]
  ): Seq[JsObject] =
    actorData.map { data =>
  ...
  def getFlightTraceAtTime(flightCode: String, dateString: String, messages: Int): Action[AnyContent] =
    authByRole(Debug) {
      Action.async { request =>
        ...
        Future.sequence(actorIds.map(id => fetchFlightDiffMessagesWithDiagnostics(id, pit, messages))).map {
          actorData =>
