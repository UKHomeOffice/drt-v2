...
class SummariesController @Inject() (cc: ControllerComponents, ctrl: DrtSystemInterface)
  ...
  def exportPassengersByTerminalForDateRangeApi(
      startLocalDateString: String,
  ...
  def exportPassengersByPortForDateRangeApi(
      startLocalDateString: String,
      endLocalDateString: String
  ): Action[AnyContent] =
    auth(Action {
  ...
  private def exportPassengersCsv(
      startLocalDateString: String,
      endLocalDateString: String,
    ...
    (LocalDate.parse(startLocalDateString), LocalDate.parse(endLocalDateString)) match {
      case (Some(start), Some(end)) =>
        val fileName =
          makeFileName("passengers", maybeTerminal.toSeq, SDate(start), SDate(end), airportConfig.portCode) + ".csv"
  ...
  private def streamForGranularity(
      maybeTerminal: Option[Terminal],
    ...
    (start, end) => {
      ...
      val queueTotals =
        PassengersHourlyDao.queueTotalsForPortAndDate(ctrl.airportConfig.portCode.iata, maybeTerminal.map(_.toString))
      val queueTotalsQueryForDate: LocalDate => Future[Map[Queue, Int]] =
        date => ctrl.aggregatedDb.run(queueTotals(date))
      val capacityTotals =
        CapacityHourlyDao.totalForPortAndDate(ctrl.airportConfig.portCode.iata, maybeTerminal.map(_.toString))
      ...
      val stream = granularity match {
        case Some("hourly") =>
          val hourlyQueueTotalsQuery =
            PassengersHourlyDao.hourlyForPortAndDate(ctrl.airportConfig.portCode.iata, maybeTerminal.map(_.toString))
          ...
          val hourlyCapacityTotalsQuery =
            CapacityHourlyDao.hourlyForPortAndDate(ctrl.airportConfig.portCode.iata, maybeTerminal.map(_.toString))
  ...
  private val hourlyStream: (LocalDate => Future[Map[Long, Map[Queue, Int]]], LocalDate => Future[Map[Long, Int]]) => (
      LocalDate,
      LocalDate
  ) => Source[(Map[Queue, Int], Int, Option[Long]), NotUsed] =
    (queueTotalsForDate, hourlyCapacityTotalsForDate) =>
      (start, end) =>
  ...
  private val dailyStream: (LocalDate => Future[Map[Queue, Int]], LocalDate => Future[Int]) => (
      LocalDate,
      LocalDate
  ) => Source[(Map[Queue, Int], Int, Option[LocalDate]), NotUsed] =
    (queueTotalsForDate, capacityTotalForDate) =>
      (start, end) =>
        Source(DateRange(start, end))
  ...
  private val totalsStream: (LocalDate => Future[Map[Queue, Int]], LocalDate => Future[Int]) => (
      LocalDate,
      LocalDate
  ) => Source[(Map[Queue, Int], Int, Option[LocalDate]), NotUsed] =
    (queueTotalsForDate, capacityTotalForDate) =>
  ...
  ): (Map[Queue, Int], Int, Option[T]) => String =
