...
class DropInsController @Inject() (
    cc: ControllerComponents,
    ctrl: DrtSystemInterface,
    govNotifyEmail: GovNotifyEmail
) extends AuthController(cc, ctrl) {

  private lazy val dropInRegistrationTemplateId =
    ctrl.config.get[String]("notifications.dropIn-registration-templateId")

  private lazy val dropInRegistrationHostTemplateId =
    ctrl.config.get[String]("notifications.dropIn-registration-host-templateId")

  private lazy val dropInHostEmail = config.get[String]("notifications.dropIn-host-email")
          ...
          Try(content.parseJson.convertTo[String])
            .map { id =>
              ctrl.dropInRegistrationService.createDropInRegistration(userEmail, id).map { _ =>
                ...
                case e =>
                  log.warning(s"Error while db insert for drop-in registration: ${e.getMessage}")
                  BadRequest(s"Failed to register drop-ins for user $userEmail")
              }
              Ok("Successfully registered drop-ins")
            }.recover {
              case e =>
                log.warning(s"Error while drop-in registration: ${e.getMessage}")
                BadRequest(s"Failed to register drop-ins for user $userEmail")
  ...
  private def sendDropInRegistrationEmails(email: String, dropIns: Seq[DropInRow]): Seq[Try[Any]] = {
    ...
    dropIns.map { dropIn =>
      ...
      govNotifyEmail.sendRequest(
        govNotifyReference,
        email,
        dropInRegistrationTemplateId,
        personalisation
      ).recover {
        case e => log.error(s"Error sending drop-in registration email to user $email: ${e.getMessage}")
      }
      ...
      govNotifyEmail.sendRequest(
        ...
        case e => log.error(s"Error sending drop-in registration email to host $email: ${e.getMessage}")
      }
