...
class ForecastAccuracyController @Inject() (cc: ControllerComponents, ctrl: DrtSystemInterface)
    ...
    Action.async { _ =>
      val maybeResponse = for {
        ...
        ForecastAccuracyCalculator(
          date,
          daysToCalculate,
          ctrl.feedService.actualPaxNos,
          ctrl.feedService.forecastPaxNos,
          ctrl.now().toLocalDate
        )
      }
      maybeResponse match {
  ...
  def forecastAccuracyExport(daysForComparison: Int, daysAhead: Int): Action[AnyContent] = auth {
    Action {
      val stream = ForecastAccuracyCalculator
        .predictionsVsLegacyForecast(
          daysForComparison,
          daysAhead,
          ctrl.feedService.actualArrivals,
          ctrl.feedService.forecastArrivals,
          ctrl.now().toLocalDate
        )
        .map {
  ...
  def forecastModelComparison(
      modelNames: String,
      terminalName: String,
      startDateStr: String,
      endDateStr: String
  ): Action[AnyContent] = auth {
    Action {
        ...
        .flatMapConcat { models =>
          ...
          val headerRow =
            (Seq(
              ...
              "Forecast capacity",
              "Capacity change %"
            ) ++ paxHeaders ++ loadHeaders).mkString(",") + "\n"
  ...
  private def streamDateRange(
    ...
    val terminalFlights: (LocalDate, Option[Long]) => Future[Seq[ApiFlightWithSplits]] = {
      (date, maybePit) =>
        ctrl.applicationService.flightsProvider.terminalDateScheduled(ctrl.materializer, ctrl.ec)(terminal)(
          date,
          maybePit
        )
  ...
  private def updateCachedStats(
    ...
    val modelFcstRows = modelFcsts.map(mf =>
      ArrivalStatsRow(
        portCode = airportConfig.portCode.iata,
        terminal = terminalName,
        date = localDate.toISOString,
    ...
    ctrl.applicationService.arrivalStatsDao.addOrUpdate(actRow).flatMap(_ =>
      ctrl.applicationService.arrivalStatsDao.addOrUpdate(fcstRow).flatMap(_ =>
        Future.sequence(modelFcstRows.map(mf => ctrl.applicationService.arrivalStatsDao.addOrUpdate(mf)))
      )
    )
  }

  private def actualsStats(
      localDate: LocalDate,
  ...
  private def modelsFromCache(
    ...
    for {
      ...
      forecasts <-
        ctrl.applicationService.arrivalStatsDao.get(terminalName, localDate.toISOString, daysAhead, "forecast")
  ...
  private def generateForecastStats(
    ...
    val modelForecasts: Seq[ModelForecast] =
      sortedModelsForDate.collect { case (modelName, model: ArrivalModelAndFeatures) =>
  ...
  private def csvRow(
      ...
      actuals: Actuals,
      forecast: Forecast,
      modelForecasts: Seq[ModelForecast],
      expectedModelCount: Int
  ): String = {
    ...
    (Seq(
      actuals.date.toISOString,
      actuals.flights,
      forecast.flights,
      unscheduledFlightsPct,
      actuals.capacity,
      forecast.capacity,
      capacityChange
    ) ++ paxCells ++ loadCells).mkString(",") + "\n"
  }

  private def percentOrNa(numerator: Int, denominator: Int): String =
    percentOrNa(numerator.toDouble, denominator.toDouble)
  ...
  private def predictedPaxTotal(
      model: ArrivalModelAndFeatures,
      localDate: LocalDate,
      arrivals: Seq[ApiFlightWithSplits]
  ): Int =
    arrivals.map { fws =>
      fws.apiFlight.MaxPax.map(mp =>
        (model.prediction(fws.apiFlight).getOrElse(defaultLoadPct).toDouble * mp / 100).round.toInt
      ).getOrElse {
  ...
  private def predictedLoadPctTotal(
      model: ArrivalModelAndFeatures,
      localDate: LocalDate,
      arrivals: Seq[ApiFlightWithSplits]
  ): Double =
  ...
  private def feedPaxTotal(
      localDate: LocalDate,
      arrivals: Seq[ApiFlightWithSplits],
      feedsPreference: Seq[FeedSource]
  ): Int =
    arrivals
      .map { fws =>
        fws.apiFlight.bestPcpPaxEstimate(feedsPreference).getOrElse {
          log.warning(
            s"No ${feedsPreference.map(_.name).mkString(", ")} for ${fws.apiFlight.unique} on $localDate. Using 0 for a default"
          )
          0
        }
      }.sum
