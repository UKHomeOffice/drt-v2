...
class DataRetentionController @Inject() (cc: ControllerComponents, ctrl: DrtSystemInterface)
  ...
  def purge(fromStr: String): Action[AnyContent] = authByRole(SuperAdmin) {
    Action {
      ...
      if (ctrl.applicationService.dateIsSafeToPurge(fromDate)) {
        ...
        BadRequest(
          s"Cannot purge data from ${fromDate.toISOString} as it is within the retention period (${toDate.toISOString} onwards)"
        )
      }
    }
  }
