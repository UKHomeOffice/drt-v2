...
object PaxDeltas {
  ...
  private def maybePctDeltasRecursive(
      maxDays: Int,
      averageDays: Int,
    ...
    if (resultsCount == averageDays) Nil
      ...
      maybeDeltaForDays(dailyPaxNosByDay, day, dayBefore) match {
        case Some(delta) =>
          Option(delta) ::
            maybePctDeltasRecursive(maxDays, averageDays, dailyPaxNosByDay, startDay, dayOffset + 1, resultsCount + 1)
        case None =>
          maybePctDeltasRecursive(maxDays, averageDays, dailyPaxNosByDay, startDay, dayOffset + 1, resultsCount)
      }
    }

  private def maybeDeltaForDays(
      dailyPaxNosByDay: Map[(MillisSinceEpoch, MillisSinceEpoch), Int],
      day: SDateLike,
      dayBefore: SDateLike
  ): Option[Double] = {
  ...
  def applyAdjustmentsToArrivals(
      passengersActorProvider: () => ActorRef,
      numDaysInAverage: Int
  )(arrivals: List[FeedArrival])(implicit mat: Materializer, ec: ExecutionContext): Future[List[FeedArrival]] = {
    val paxActor = passengersActorProvider()
    val updatedArrivalsSource = Source(arrivals)
      .mapAsync(1) { arrival =>
        val request =
          GetAverageAdjustment(OriginAndTerminal(PortCode(arrival.origin), arrival.terminal), numDaysInAverage)
  ...
  private def lookupAndApplyAdjustment(
      paxActor: ActorRef,
