...
object Forecast {
  ...
  def forecastPeriod(
    ...
    val queues = QueueConfig.queuesForDateRangeAndTerminal(airportConfig.queuesByTerminal)(
      startOfForecast.toLocalDate,
      endOfForecast.toLocalDate,
      terminal
    )
    val crunchSummary15Mins =
      portState.crunchSummary(startOfForecast, periods.toInt, intervalMinutes, terminal, queues.toList)
    val timeSlotsByDay = Forecast.rollUpForWeek(crunchSummary15Mins, staffSummary)
    ForecastPeriod(intervalMinutes, timeSlotsByDay)
  }

  def rollUpForWeek(
      crunchSummary: Map[MillisSinceEpoch, Map[Queue, CrunchMinute]],
