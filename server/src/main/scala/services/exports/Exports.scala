...
import actors.PartitionedPortStateActor.{DateRangeLike, GetFlightsForTerminalEffectingRange, GetMinutesForTerminalDateRange}
import akka.NotUsed
...
object Exports {
  ...
  def flightSummariesFromPortState(terminalFlightsSummaryGenerator: TerminalFlightsSummaryLikeGenerator)
    ...
    flightsProvider(GetFlightsForTerminalEffectingRange(from.millisSinceEpoch, to.millisSinceEpoch, terminal)).map {
