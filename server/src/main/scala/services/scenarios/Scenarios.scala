  ...
  def simulationResult(
    ...
    val paxLoadsProducer = DynamicRunnablePassengerLoads.crunchRequestsToQueueMinutes(
      ...
      dynamicQueueStatusProvider = () =>
        egateBanksProvider().map { ep =>
          DynamicQueueStatusProvider(
            queuesForDateAndTerminal,
            simulationAirportConfig.maxDesksByTerminalAndQueue24Hrs,
            ep
          )
        },
    ...
    val desksProducer: Flow[TerminalUpdateRequest, DeskRecMinutes, NotUsed] = paxLoadsProducer
      .mapAsync(1)(loads =>
        portDesksAndWaitsProvider.terminalLoadsToDesks(
          request.minutesInMillis,
          loads.indexed,
          deskLimitsProviders(simulationParams.terminal),
          "scenarios",
    ...
    val crunchGraphSource =
      new SortedActorRefSource(dummyPersistentActor, SortedSet.empty[TerminalUpdateRequest], "sim-desks")
    val (crunchRequestQueue, deskRecsKillSwitch) =
      QueuedRequestProcessing.createGraph(crunchGraphSource, portStateActor, desksProducer, "sim-desks").run()
