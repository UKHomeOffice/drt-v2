...
object RunnableHistoricPax extends RunnableGraphLike {
  private def arrivalsToHistoricPax(
      maybeHistoricPax: UniqueArrival => Future[Option[(Int, Int)]],
        ...
        Source(arrivalKeys.toList)
          .mapAsync(1) { arrival =>
            maybeHistoricPax(arrival).map(_.map { case (total, transit) =>
              (
                arrival,
                Map[FeedSource, Passengers](HistoricApiFeedSource -> Passengers(Option(total), Option(transit)))
              )
            })
          }
          .collect {
  ...
  private def maybeHistoricPax(maybeManifestPaxCount: UniqueArrival => Future[Option[ManifestPaxCount]])(implicit
      ec: ExecutionContext
  ): UniqueArrival => Future[Option[(Int, Int)]] =
    uniqueArrival =>
  ...
  def apply(
    ...
    val persistPax: PaxForArrivals => Future[Done] =
      paxForArrivals => flightsRouterActor.ask(paxForArrivals).map(_ => Done)
