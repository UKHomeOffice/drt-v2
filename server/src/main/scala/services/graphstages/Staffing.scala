...
object Staffing {
  ...
  def staffAvailableByTerminalAndQueue(
      dropBeforeMillis: MillisSinceEpoch,
  ...
  def terminalStaffAt(
      shifts: ShiftAssignments,
      fixedPoints: FixedPointAssignments,
      movements: StaffMovementsCalculator
  ): (MillisSinceEpoch, Terminal, MillisSinceEpoch => SDateLike) => Int =
...
object StaffDeploymentCalculator {
  ...
  ): Map[TQM, CrunchMinute] = crunchMinutes
    .values
    .groupBy(_.terminal)
    .flatMap {
        ...
        val terminalByMinute: Map[TQM, CrunchMinute] = tCrunchMinutes
          ...
          .flatMap {
            case (minute, mCrunchMinutes) =>
              val deskRecAndQueueNames: Seq[(Queue, Int)] =
                mCrunchMinutes.map(cm => (cm.queue, cm.deskRec)).toSeq.sortBy(_._1)
              ...
              val deploymentsAndQueueNames: Map[Queue, Int] =
                deployer(deskRecAndQueueNames, available, minMaxByQueue).toMap
              mCrunchMinutes.map(cm =>
                (cm.key, cm.copy(deployedDesks = Option(deploymentsAndQueueNames(cm.queue))))
              ).toMap
          }
        terminalByMinute
    }
  ...
  def queueRecsToDeployments(round: Double => Int)(
    ...
    queueRecsCorrected.foldLeft(List[(Queue, Int)]()) {
      case (agg, (queue, deskRec)) if agg.length < queueRecsCorrected.length - 1 =>
        ...
        val dr =
          deploymentWithinBounds(minMaxDesks(queue)._1, minMaxDesks(queue)._2, ideal, staffAvailable - totalRecommended)
        agg :+ Tuple2(queue, dr)
      case (agg, (queue, _)) =>
        val totalRecommended = agg.map(_._2).sum
...
case class StaffSources(
    shifts: StaffAssignmentsLike,
