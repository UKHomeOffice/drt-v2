...
trait WorkloadCalculatorLike {
  ...
  def flightsWithPcpWorkload(
      flights: Iterable[ApiFlightWithSplits],
      redListUpdates: RedListUpdates
  ): Iterable[ApiFlightWithSplits] =
    flights.filter(fws =>
      terminalProcTimes.contains(fws.apiFlight.Terminal) && flightHasWorkload.apply(fws, redListUpdates)
    )

}

case class DynamicWorkloadCalculator(
    terminalProcTimes: Map[Terminal, Map[PaxTypeAndQueue, Double]],
  ...
  override def flightLoadMinutes(
      minuteMillis: NumericRange[MillisSinceEpoch],
      flights: FlightsWithSplits,
      redListUpdates: RedListUpdates,
      terminalQueueStatuses: Terminal => (Queue, MillisSinceEpoch) => QueueStatus,
    ...
    val procTimes = (terminal: Terminal) =>
      (paxType: PaxType, queue: Queue) =>
        terminalProcTimes
    ...
    val loadMinutes = relevantFlights
      ...
      .flatMap { case (terminal, flightsForTerminal) =>
        val distributePaxOverQueuesAndMinutes = WholePassengerQueueSplits.wholePaxLoadsPerQueuePerMinute(
          minuteMillis,
          procTimes(terminal),
          terminalQueueStatuses(terminal)
        )
