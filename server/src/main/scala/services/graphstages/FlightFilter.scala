...
object FlightFilter {
  ...
  def validTerminalFilter(validTerminals: List[Terminal]): FlightFilter =
    FlightFilter((fws, _) => validTerminals.contains(fws.apiFlight.Terminal))
  ...
  val lhrRedListFilter: FlightFilter = FlightFilter { (fws, redListUpdates) =>
    val isGreenOnlyTerminal =
      terminalTypes.lhrNonRedListTerminalsForDate(fws.apiFlight.Scheduled).contains(fws.apiFlight.Terminal)
