...
object PortDeskLimits {
  def fixed(
    ...
    (
      for {
        ...
        val queuesForDate =
          (date: LocalDate) => QueueConfig.queuesForDateAndTerminal(airportConfig.queuesByTerminal)(date, terminal)
        val minDesksForDate =
          (date: LocalDate) => minDesksByQueue24Hrs.filter { case (queue, _) => queuesForDate(date).contains(queue) }

        (
          terminal,
          FixedTerminalDeskLimits(
            minDesksForDate,
            capacityProviders(maxDesksByQueue24Hrs, () => egatesProvider(terminal)),
            paxForQueue(terminal)
          )
        )
      }
    ).toMap
  ...
  def flexedByAvailableStaff(
      airportConfig: AirportConfig,
      egatesProvider: Terminal => Future[EgateBanksUpdates],
      paxForQueue: Terminal => (NumericRange[Long], Queue) => Future[Seq[Int]]
  )(terminal: Terminal, terminalStaffByMinute: List[Int])(implicit
      ec: ExecutionContext
  ...
  private def capacityProviders(
      maxDesks: Map[Queues.Queue, IndexedSeq[Int]],
      egatesProvider: () => Future[EgateBanksUpdates]
