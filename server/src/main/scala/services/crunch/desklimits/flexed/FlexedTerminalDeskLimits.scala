...
trait FlexedTerminalDeskLimitsLike extends TerminalDeskLimitsLike {
  ...
  def maxProcessors(
    ...
    if (flexedQueues.contains(queue)) {
      ...
      val remainingMinDesks =
        DeskRecs.desksByMinuteForQueues(minDesksByQueue24Hrs(date), minuteMillis, remainingFlexedQueues).values.toList
      val terminalDesksForPeriod: immutable.Seq[Int] = List.fill(minuteMillis.length)(terminalDesks)
      val desksByMinute: Iterable[Int] =
        reduceIterables[Int](terminalDesksForPeriod :: totalDeployed :: remainingMinDesks)(_ - _)
      Future.successful(WorkloadProcessorsProvider(desksByMinute.map(d => Seq.fill(d)(Desk))))
    } else {
      capacityByQueue.getOrElse(
        queue, {
...
case class FlexedTerminalDeskLimits(
    terminalDesks: Int,
