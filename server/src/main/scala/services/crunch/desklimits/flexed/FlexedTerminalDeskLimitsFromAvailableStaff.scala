...
case class FlexedTerminalDeskLimitsFromAvailableStaff(
    totalStaffByMinute: List[Int],
    terminalDesks: Int,
  ...
  override def maxDesksForMinutes(
    ...
    eventualMaxProcessorsProvider.map { desksProvider =>
      ...
      val availableDesks: Iterable[Int] =
        Crunch.reduceIterables[Int](List(desksByMinute, availableStaffByMinute))(Math.min)
  ...
  private def availableStaffForMinutes(
      minuteMillis: NumericRange[Long],
      queue: Queue,
      allocatedDesks: Map[Queue, List[Int]]
  ): Iterable[Int] = {
    ...
    val minDesksForRemainingQueuesByMinute =
      DeskRecs.desksByMinuteForQueues(minDesksByQueue24Hrs(date), minuteMillis, remainingQueues).values.toList
    val minimumPromisedStaffByMinute = if (minDesksForRemainingQueuesByMinute.nonEmpty)
      Crunch.reduceIterables[Int](minDesksForRemainingQueuesByMinute)(_ + _)
    else List()
