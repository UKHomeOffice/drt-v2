...
case class FixedTerminalDeskLimits(minDesksByQueue24Hrs: Map[Queue, IndexedSeq[Int]],
                                   maxDesksByQueue24Hrs: Map[Queue, IndexedSeq[Int]],
                                  ) extends TerminalDeskLimitsLike {
  def maxDesksForMinutes(minuteMillis: NumericRange[Long],
                         queue: Queue,
                         allocatedDesks: Map[Queue, List[Int]]): Future[List[Int]] =
    Future.successful(DeskRecs.desksForMillis(minuteMillis, maxDesksByQueue24Hrs.getOrElse(queue, IndexedSeq.fill(24)(0))).toList)
}
