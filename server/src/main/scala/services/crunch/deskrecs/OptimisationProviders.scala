...
object OptimisationProviders {
  ...
  def flightsWithSplitsProvider(arrivalsActor: ActorRef)(processingRequest: TerminalUpdateRequest)(implicit
      timeout: Timeout,
      ec: ExecutionContext
  ): Future[Source[List[ApiFlightWithSplits], NotUsed]] =
    arrivalsActor
      .ask(GetFlightsForTerminalDateRange(
        processingRequest.start.millisSinceEpoch,
        processingRequest.end.millisSinceEpoch,
        processingRequest.terminal
      ))
      .mapTo[Source[(UtcDate, FlightsWithSplits), NotUsed]]
      .map(_.map(_._2.flights.values.toList))

  def passengersProvider(passengersActor: ActorRef)(implicit
      timeout: Timeout,
      ec: ExecutionContext
  ): (SDateLike, SDateLike, Terminal) => Future[Map[TQM, PassengersMinute]] =
    (start, end, terminal) =>
      passengersActor
  ...
  def staffMinutesProvider(staffActor: ActorRef)(implicit
      timeout: Timeout,
      ec: ExecutionContext
  ): (SDateLike, SDateLike, Terminal) => Future[List[Int]] =
    (start, end, terminal) => {
      staffActor
        .ask(GetStateForTerminalDateRange(start.millisSinceEpoch, end.millisSinceEpoch, terminal))
  ...
  private def allMinutesForPeriod(
      terminal: Terminal,
      firstMillis: MillisSinceEpoch,
      lastMillis: MillisSinceEpoch,
      indexedStaff: Map[TM, StaffMinute]
  ): Iterable[StaffMinute] = (firstMillis to lastMillis by MilliTimes.oneMinuteMillis).map(minute =>
