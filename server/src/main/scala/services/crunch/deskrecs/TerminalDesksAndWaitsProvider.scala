          ...
          for {
            ...
            queueWork match {
              ...
              case someWork =>
                ...
                val optimisedDesks = cruncher(someWork, minDesks.toSeq, processorsProvider.processorsByMinute.map(_.processors.size).take(minuteMillis.length), OptimiserConfig(slas(queue), processorsProvider)) match {
