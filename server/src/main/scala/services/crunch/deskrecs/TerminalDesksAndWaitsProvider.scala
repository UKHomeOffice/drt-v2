...
case class TerminalDesksAndWaitsProvider(slas: Map[Queue, Int],
                                         ...
                                         cruncher: TryCrunch,
                                         banksProvider: () => Future[PortEgateBanksUpdates]) {
  ...
  def workToDeskRecs(terminal: Terminal,
    ...
    val terminalEgatesProvider: () => Future[EgateBanksUpdates] = () => banksProvider().map(_.updatesByTerminal(terminal))
    desksAndWaits(minuteMillis, terminalWork, deskLimitsProvider, terminalEgatesProvider).map { queueDesksAndWaits =>
  ...
  def desksAndWaits(minuteMillis: NumericRange[MillisSinceEpoch],
                    ...
                    deskLimitsProvider: TerminalDeskLimitsLike,
                    egateBanksProvider: () => Future[EgateBanksUpdates])
