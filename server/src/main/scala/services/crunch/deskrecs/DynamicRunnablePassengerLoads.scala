...
object DynamicRunnablePassengerLoads extends DrtRunnableGraph {
  ...
  def crunchRequestsToQueueMinutes(
      arrivalsProvider: TerminalUpdateRequest => Future[Source[List[ApiFlightWithSplits], NotUsed]],
    ...
    Flow[TerminalUpdateRequest]
      ...
      .wireTap(crWithFlights =>
        log.info(
          s"${crWithFlights._1} crunch request - found ${crWithFlights._2.size} arrivals with ${crWithFlights._2.map(_.apiFlight.bestPcpPaxEstimate(paxFeedSourceOrder).getOrElse(0)).sum} passengers"
        )
      )
      .via(toPassengerLoads(
        portDesksAndWaitsProvider,
        redListUpdatesProvider,
      ...
      .wireTap { updateRequestWithMinutes =>
        log.info(
          s"${updateRequestWithMinutes._1} crunch request - ${updateRequestWithMinutes._2.minutes.size} minutes of passenger loads with ${updateRequestWithMinutes._2.minutes.map(_.toMinute.passengers.size).sum} passengers"
        )
  ...
  private def toPassengerLoads(
    ...
    (TerminalUpdateRequest, Iterable[ApiFlightWithSplits]),
    (TerminalUpdateRequest, MinutesContainer[PassengersMinute, TQM]),
    NotUsed
  ] = {
    Flow[(TerminalUpdateRequest, Iterable[ApiFlightWithSplits])]
      .mapAsync(1) {
        case (request, flights) =>
          log.info(
            s"Passenger load calculation starting: ${request.date.toISOString}, ${request.terminal}, ${flights.size} flights"
          )
          ...
          val eventualDeskRecs = for {
            ...
            val flightsPax = portDesksAndWaitsProvider.flightsToLoads(
              request.minutesInMillis,
              FlightsWithSplits(flights),
              redListUpdates,
              statusProvider.queueStatus,
              terminalSplits
            )
            val paxMinutesForCrunchPeriod = for {
              ...
              flightsPax.getOrElse(
                TQM(request.terminal, queue, minute),
                PassengersMinute(request.terminal, queue, minute, Seq(), Option(SDate.now().millisSinceEpoch))
              )
            }

            log.info(
              s"Passenger load calculation finished: (${request.start.toISOString} to ${request.end.toISOString})"
            )
  ...
  private def addArrivals(flightsProvider: TerminalUpdateRequest => Future[Source[
    List[ApiFlightWithSplits],
    NotUsed
  ]])(implicit
      ec: ExecutionContext
  ): Flow[TerminalUpdateRequest, (TerminalUpdateRequest, List[ApiFlightWithSplits]), NotUsed] =
    Flow[TerminalUpdateRequest]
      ...
      .flatMapConcat {
        case (crunchRequest, flightsStream, startTime) =>
          ...
          log.info(s"DynamicRunnableDeskRecs ${crunchRequest.date}: addArrivals took ${SDate.now().millisSinceEpoch -
              startTime.millisSinceEpoch} ms")
          requestWithArrivals
      }
}
