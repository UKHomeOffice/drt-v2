...
object QueuedRequestProcessing {
  ...
  def createGraph[A, B](
      processingRequestSource: SortedActorRefSource,
      sinkActor: ActorRef,
      processor: Flow[B, A, NotUsed],
      graphName: String
  ): RunnableGraph[(ActorRef, UniqueKillSwitch)] = {
    val deskRecsSink =
      Sink.actorRefWithAck(sinkActor, StreamInitialized, StatusReply.Ack, StreamCompleted, StreamFailure)
    val ks = KillSwitches.single[A]

    val graph = GraphDSL.createGraph(processingRequestSource, ks)((_, _)) {
      implicit builder => (requests, killSwitch) =>
