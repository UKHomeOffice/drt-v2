...
case class PortDesksAndWaitsProvider(
    queuesByTerminal: (LocalDate, LocalDate, Terminal) => Seq[Queue],
  ...
  override def loadsToSimulations(
    ...
    terminalLoadsToDesks(minuteMillis, passengersByQueue, deskLimitProviders, description, terminal).map {
      deskRecMinutes =>
        val simMinutes = deskRecsToSimulations(deskRecMinutes.minutes).values.toSeq
        log.info(
          s"Deployments & waits calculated for ${SDate(minuteMillis.min).toISOString} to ${SDate(minuteMillis.max).toISOString}"
        )
        SimulationMinutes(simMinutes)
    }
  }

  private def deskRecsToSimulations(terminalQueueDeskRecs: Iterable[DeskRecMinute]): Map[TQM, SimulationMinute] =
    terminalQueueDeskRecs
      .map {
  ...
  private def terminalWorkLoadsByQueue(
      terminal: Terminal,
      minuteMillis: NumericRange[MillisSinceEpoch],
    ...
    queuesByTerminal(SDate(minuteMillis.min).toLocalDate, SDate(minuteMillis.max).toLocalDate, terminal)
      ...
      .map { queue =>
        val lms = minuteMillis.map(minute =>
          loadMinutes.getOrElse(
            TQM(terminal, queue, minute),
            PassengersMinute(terminal, queue, minute, Seq(), None)
          ).passengers.sum
        )
        (queue, lms)
      }
  ...
  private def terminalPassengersByQueue(
      terminal: Terminal,
      minuteMillis: NumericRange[MillisSinceEpoch],
      loadMinutes: Map[TQM, PassengersMinute]
  ...
  override def terminalLoadsToDesks(
      minuteMillis: NumericRange[MillisSinceEpoch],
