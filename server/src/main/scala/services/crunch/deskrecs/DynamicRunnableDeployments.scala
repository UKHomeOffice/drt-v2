...
object DynamicRunnableDeployments extends DrtRunnableGraph {
  ...
  type PassengersToQueueMinutes =
    (
        ...
        String,
        Terminal
    ) => Future[PortStateQueueMinutes]

  def crunchRequestsToDeployments(
      loadsProvider: (SDateLike, SDateLike, Terminal) => Future[Map[TQM, PassengersMinute]],
      staffMinutesProvider: (SDateLike, SDateLike, Terminal) => Future[List[Int]],
      staffToDeskLimits: (Terminal, List[Int]) => FlexedTerminalDeskLimitsFromAvailableStaff,
      ...
      ac: AirportConfig
  ): Flow[TerminalUpdateRequest, MinutesContainer[CrunchMinute, TQM], NotUsed] = {
    Flow[TerminalUpdateRequest]
      ...
      .mapAsync(1) {
        case (request: TerminalUpdateRequest, loads, deskLimitsByTerminal) =>
          ...
          loadsToQueueMinutes(
            request.minutesInMillis(ac.crunchOffsetMinutes),
            loads,
            deskLimitsByTerminal,
            "deployments",
            request.terminal
          )
            .map { minutes =>
