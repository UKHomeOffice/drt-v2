...
case class ApiValidityReporter(flightsActor: ActorRef)(implicit
    timeout: Timeout,
    mat: Materializer,
    ec: ExecutionContext
) extends Runnable {
  def run(): Unit = {
    val today = SDate.now()
    val request =
      GetStateForDateRange(today.getLocalLastMidnight.millisSinceEpoch, today.getLocalNextMidnight.millisSinceEpoch)
    flightsActor
      .ask(request)
