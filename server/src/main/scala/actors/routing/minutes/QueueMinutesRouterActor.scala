...
object QueueMinutesRouterActor {
  def splitByResource(request: MinutesContainer[CrunchMinute, TQM])
      : Map[(Terminal, UtcDate), MinutesContainer[CrunchMinute, TQM]] = {
  ...
  def sendIfDeskRec(request: MinutesContainer[CrunchMinute, TQM]): Boolean =
    request.minutes.exists(_.isInstanceOf[DeskRecMinute])
}

class QueueMinutesRouterActor(
    ...
    updateMinutes: MinutesUpdate[CrunchMinute, TQM, TerminalUpdateRequest]
      ...
      lookup,
      updateMinutes,
      QueueMinutesRouterActor.splitByResource,
      QueueMinutesRouterActor.sendIfDeskRec
    )
    with RouterActorLikeWithSubscriber2[
      MinutesContainer[CrunchMinute, TQM],
      (Terminal, UtcDate),
