...
case class GetStreamingMinutesForTerminalDateRange(terminal: Terminal, start: UtcDate, end: UtcDate)
    extends UtcDateRangeLike

case class GetStreamingMinutesForDateRange(start: UtcDate, end: UtcDate) extends UtcDateRangeLike

object MinutesActorLike {
  type MinutesLookup[A, B <: WithTimeAccessor] =
    ((Terminals.Terminal, UtcDate), Option[MillisSinceEpoch]) => Future[Option[MinutesContainer[A, B]]]
  ...
  type MinutesUpdate[A, B <: WithTimeAccessor, U] =
    ((Terminals.Terminal, UtcDate), MinutesContainer[A, B]) => Future[Set[U]]
...
object MinutesActorLikeCommon {
  ...
  def handleLookup[A, B <: WithTimeAccessor](eventualMaybeResult: Future[Option[MinutesContainer[A, B]]])(implicit
      ec: ExecutionContext
  ): Future[Option[MinutesContainer[A, B]]] =
  ...
  def retrieveTerminalMinutesWithinRangeAsStream[A, B <: WithTimeAccessor](
  ...
  def handleLookups[A, B <: WithTimeAccessor](
      lookup: MinutesLookup[A, B],
      terminal: Terminal,
      start: SDateLike,
      end: SDateLike,
  ...
  def retrieveTerminalMinutesDateRangeAsStream[A, B <: WithTimeAccessor](
      lookup: MinutesLookup[A, B],
      terminal: Terminal,
      start: UtcDate,
      end: UtcDate,
  ...
  def handleAllTerminalLookupsStreamMinutesContainer[A, B <: WithTimeAccessor](
      lookup: MinutesLookup[A, B],
  ...
  def retrieveMinutesDateRangeAsStream[A, B <: WithTimeAccessor](
      lookup: MinutesLookup[A, B],
      terminals: Iterable[Terminal],
      start: UtcDate,
      end: UtcDate,
    ...
    Source(DateRange(start, end))
      .mapAsync(1) { day =>
        val terminalContainers =
          terminals.map(terminal => MinutesActorLikeCommon.handleLookup(lookup((terminal, day), maybePointInTime)))
...
abstract class MinutesActorLike[A, B <: WithTimeAccessor, U](
  ...
  val sequentialUpdatesActor: ActorRef =
    context.actorOf(Props(new SequentialAccessActor(updateMinutes, splitByResource)))
  ...
  override def receiveQueries: Receive = {
    case PointInTimeQuery(pit, GetStateForDateRange(startMillis, endMillis)) =>
      ...
      MinutesActorLikeCommon.handleAllTerminalLookupsStreamMinutesContainer(
        lookup,
        terminals,
        startMillis,
        endMillis,
    ...
    case PointInTimeQuery(pit, request: DateRangeMillisLike with TerminalRequest) =>
      val replyTo = sender()
      MinutesActorLikeCommon.handleLookups(
        lookup,
        request.terminal,
        SDate(request.from),
        SDate(request.to),
  ...
  override def partitionUpdates
      : PartialFunction[MinutesContainer[A, B], Map[(Terminal, UtcDate), MinutesContainer[A, B]]] = {
...
abstract class MinutesActorLike2[A, B <: WithTimeAccessor, U](
    terminalsForDateRange: (LocalDate, LocalDate) => Iterable[Terminal],
    lookup: MinutesLookup[A, B],
    updateMinutes: MinutesUpdate[A, B, U],
  ...
  override val sequentialUpdatesActor: ActorRef =
    context.actorOf(Props(new SequentialAccessActor(updateMinutes, splitByResource) {
