...
object QueueLoadsMinutesActor {
  def splitByResource(request: MinutesContainer[PassengersMinute, TQM])
      : Map[(Terminal, UtcDate), MinutesContainer[PassengersMinute, TQM]] = {
...
class QueueLoadsMinutesActor(
    ...
    with RouterActorLikeWithSubscriber2[
      MinutesContainer[PassengersMinute, TQM],
      (Terminal, UtcDate),
