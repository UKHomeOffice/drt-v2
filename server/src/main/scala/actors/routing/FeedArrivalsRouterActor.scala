...
object FeedArrivalsRouterActor {
  ...
  case class GetStateForDateRangeAndTerminal(start: UtcDate, end: UtcDate, terminal: Terminal) extends query
      with DateRangeMillisLike {
    override val from: MillisSinceEpoch = SDate(start).millisSinceEpoch
    override val to: MillisSinceEpoch = SDate(end).millisSinceEpoch
  }
  ...
  def updateArrivals(
      ...
      system: ActorSystem,
      timeout: Timeout
  ): ((Terminals.Terminal, UtcDate), Seq[FeedArrival]) => Future[Boolean] =
  ...
  private def multiTerminalArrivalsByDaySource(
      flightsLookupByDayAndTerminal: Option[MillisSinceEpoch] => UtcDate => Terminal => Future[Seq[FeedArrival]]
  )(
      start: UtcDate,
      end: UtcDate,
    ...
    val reduceAndSort =
      SourceUtils.reduceFutureIterables(terminals, (s: Iterable[Seq[FeedArrival]]) => s.reduce(_ ++ _))
...
class FeedArrivalsRouterActor(
  ...
  private val flightsLookupService: (UtcDate, UtcDate, Iterable[Terminal], Option[MillisSinceEpoch]) => Source[
    (UtcDate, Seq[FeedArrival]),
    NotUsed
  ] =
