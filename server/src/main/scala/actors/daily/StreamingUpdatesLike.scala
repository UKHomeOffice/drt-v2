...
trait StreamingUpdatesLike[A <: MinuteLike[A, B], B <: WithTimeAccessor] extends PersistentActor {
  ...
  val startUpdatesStream: MillisSinceEpoch => Unit = (nr: MillisSinceEpoch) =>
    if (maybeKillSwitch.isEmpty) {
  ...
  def updatesSince(sinceMillis: MillisSinceEpoch): MinutesContainer[A, B] =
    updates.values.filter(_.lastUpdated.getOrElse(0L) > sinceMillis) match {
