...
class TerminalDayQueuesUpdatesActor(
    year: Int,
    month: Int,
    day: Int,
    terminal: Terminal,
...
) extends StreamingUpdatesLike[CrunchMinute, TQM] {
  ...
  override def removalsFromMessages(removalMessages: Seq[GeneratedMessage]): Seq[TQM] =
    removalMessages.map {
      case msg: CrunchMinuteRemovalMessage =>
        TQM(terminal, Queue(msg.getQueueName), SDate(msg.getMinute).millisSinceEpoch)
