...
import uk.gov.homeoffice.drt.protobuf.messages.CrunchState.{
  ...
  PassengersMinuteRemovalMessage,
...
class TerminalDayQueueLoadsActor(
    utcDate: UtcDate,
    terminal: Terminal,
    queuesForDateAndTerminal: (LocalDate, Terminal) => Seq[Queue],
    val now: () => SDateLike,
    maybePointInTime: Option[MillisSinceEpoch]
) extends TerminalDayLikeActor[PassengersMinute, TQM, PassengersMinuteMessage, PassengersMinuteRemovalMessage](
      utcDate,
      terminal,
      now,
      maybePointInTime
  ...
  override def shouldSendEffectsToSubscriber(container: CrunchApi.MinutesContainer[PassengersMinute, TQM]): Boolean =
    true
  ...
  override val msgLastUpdated: PassengersMinuteMessage => MillisSinceEpoch =
    (msg: PassengersMinuteMessage) => msg.getLastUpdated
