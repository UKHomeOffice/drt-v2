...
object CrunchManagerActor {
  ...
  def missingHistoricSplitsArrivalKeysForDate(allTerminalsFlights: (
      UtcDate,
      UtcDate
  ) => Source[(UtcDate, Seq[ApiFlightWithSplits]), NotUsed])(implicit
      mat: Materializer
  ): UtcDate => Future[Seq[UniqueArrival]] =
    date =>
      allTerminalsFlights(date, date)
        .map { case (_, arrivals) =>
          arrivals
  ...
  def historicSplitsArrivalKeysForDate(allTerminalsFlights: (
      UtcDate,
      UtcDate
  ) => Source[(UtcDate, Seq[ApiFlightWithSplits]), NotUsed])(implicit
      mat: Materializer,
  ...
  def missingPaxArrivalKeysForDate(allTerminalsFlights: (
      UtcDate,
      UtcDate
  ) => Source[(UtcDate, Seq[ApiFlightWithSplits]), NotUsed])(implicit
      mat: Materializer
  ...
  def liveSplitsArrivalKeysForDate(allTerminalsFlights: (
      UtcDate,
      UtcDate
  ) => Source[(UtcDate, Seq[ApiFlightWithSplits]), NotUsed])(implicit
      mat: Materializer
  ...
  def props(allTerminalsFlights: (UtcDate, UtcDate) => Source[(UtcDate, Seq[ApiFlightWithSplits]), NotUsed])(implicit
      ec: ExecutionContext,
      mat: Materializer,
      ac: AirportConfig
  ): Props = {
    val missingHistoricSplitsArrivalKeysForDate =
      CrunchManagerActor.missingHistoricSplitsArrivalKeysForDate(allTerminalsFlights)
    ...
    Props(new CrunchManagerActor(
      missingHistoricSplitsArrivalKeysForDate,
      historicSplitsArrivalKeysForDate,
      missingPaxArrivalKeysForDate
    ))
  }
}

class CrunchManagerActor(
    missingHistoricManifestArrivalKeys: UtcDate => Future[Iterable[UniqueArrival]],
  ...
  override def receive: Receive = {
    ...
    case LookupHistoricSplits(um) =>
      queueLookups(
        um,
        maybeQueueHistoricSplitsLookupSubscriber,
        missingHistoricManifestArrivalKeys,
        "missing historic splits"
      )
  ...
  private def queueLookups(
      millis: Set[Long],
      subscriber: Option[ActorRef],
      lookup: UtcDate => Future[Iterable[UniqueArrival]],
      label: String
  )
      : Unit =
    Source(millis
