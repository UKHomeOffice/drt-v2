...
trait MinuteLookupsLike {
  ...
  def updatePassengerMinutes(queuesForDateAndTerminal: (LocalDate, Terminal) => Seq[Queue])
      : ((Terminal, UtcDate), MinutesContainer[PassengersMinute, TQM]) => Future[Set[TerminalUpdateRequest]] =
  ...
  def updateCrunchMinutes(
    ...
    (terminalDate: (Terminal, UtcDate), container: MinutesContainer[CrunchMinute, TQM]) => {
      ...
      val actor =
        system.actorOf(TerminalDayQueuesActor.props(Option(onUpdate), queuesForDateAndTerminal)(terminal, date, now))
      requestAndTerminateActor.ask(RequestAndTerminate(actor, container)).mapTo[Set[TerminalUpdateRequest]]
    }

  val updateStaffMinutes
      : ((Terminal, UtcDate), MinutesContainer[StaffMinute, TM]) => Future[Set[TerminalUpdateRequest]] =
  ...
  def queuesLoadsLookup(queuesForDateAndTerminal: (
      LocalDate,
      Terminal
  ) => Seq[Queue]): MinutesLookup[PassengersMinute, TQM] =
    lookup[PassengersMinute, TQM](
      TerminalDayQueueLoadsActor.props(queuesForDateAndTerminal),
      TerminalDayQueueLoadsActor.propsPointInTime(queuesForDateAndTerminal)
    )

  def queuesLookup(queuesForDateAndTerminal: (LocalDate, Terminal) => Seq[Queue]): MinutesLookup[CrunchMinute, TQM] =
    lookup[CrunchMinute, TQM](
      TerminalDayQueuesActor.props(None, queuesForDateAndTerminal),
      TerminalDayQueuesActor.propsPointInTime(queuesForDateAndTerminal)
    )
...
case class MinuteLookups(
  ...
  override val requestAndTerminateActor: ActorRef =
    system.actorOf(Props(new RequestAndTerminateActor()), "minutes-lookup-kill-actor")

  override val queueLoadsMinutesActor: ActorRef = system.actorOf(Props(new QueueLoadsMinutesActor(
    terminalsForDateRange,
    queuesLoadsLookup(queuesForDateAndTerminal),
    updatePassengerMinutes(queuesForDateAndTerminal)
  )))

  override val queueMinutesRouterActor: ActorRef = system.actorOf(Props(new QueueMinutesRouterActor(
    terminalsForDateRange,
    queuesLookup(queuesForDateAndTerminal),
    updateCrunchMinutes(updateLiveView, queuesForDateAndTerminal)
  )))

  override val staffMinutesRouterActor: ActorRef =
    system.actorOf(Props(new StaffMinutesRouterActor(terminalsForDateRange, staffLookup, updateStaffMinutes)))
}
