...
//  }
  ...
  "Given a Flight With Splits and a VoyageManifests with a matching arrival then I should get all the data with API nos plus the nationalities breakdown" >> {
    val resultStream = withActualApiExport
      .csvStream(Source(List((FlightsWithSplits(List(flightWithAllTypesOfAPISplit)), VoyageManifests(Set(VoyageManifest(DC, PortCode("AAA"), PortCode("AAA"), flightWithAllTypesOfAPISplit.apiFlight.VoyageNumber,
        flightWithAllTypesOfAPISplit.apiFlight.CarrierCode, ManifestDateOfArrival("2017-01-01"), ManifestTimeOfArrival("20:00"), List(PassengerInfoJson(
          None, Nationality("GBR"), EeaFlag("Y"), None, None, InTransit(false), None, Option(Nationality("GBR")), None)
        ))))))))
