...
class ProcessorsSpec extends Specification {
  "Given a single unit size of 1" >> {
    val processor = Processors(Iterable(1))
  ...
  private def expectCapacityForUnits(processor: Processors, units: Int, expected: Int): Fragment =
    s"The capacity for $units unit should be $expected" >> {
      processor.capacityForUnits(units) === expected
    }

  "Given two units of sizes 1, 2" >> {
    val processor = Processors(Iterable(1, 2))
  ...
  "Given three units of sizes 1, 2, 3" >> {
    val processor = Processors(Iterable(1, 2, 3))
