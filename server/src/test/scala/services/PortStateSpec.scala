...
class PortStateSpec extends CrunchTestLike {
  "Given an initial PortState with some pax loads " +
    ...
    "I should see the pax loads are unaffected" >> {
      ...
      offerAndWait(
        crunch.shiftsInput,
        UpdateShifts(Seq(StaffAssignment(
          "",
          T1,
          SDate(minute).addMinutes(-15).millisSinceEpoch,
          SDate(minute).addMinutes(15).millisSinceEpoch,
          1,
          None
        )))
  ...
  "Given 3 days of crunch minutes across 2 terminals and 2 queues " +
    ...
    "I should not see any data from the days either side" >> {
      val terminalQueues: Map[Terminal, Seq[Queue]] =
        Map(T1 -> Seq(Queues.EeaDesk, Queues.NonEeaDesk), T2 -> Seq(Queues.EeaDesk, Queues.NonEeaDesk))
    ...
    "Then the flight should be in the returned PortState" >> {
      val flight = ApiFlightWithSplits(
        ArrivalGenerator.live(
          iata = "BA0001",
          schDt = "2019-01-01T12:00",
          totalPax = Option(100)
        ).toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2019-01-02T00:01").millisSinceEpoch)),
        Set()
      )
      ...
      val windowedFlights =
        portState.window(SDate("2019-01-02T00:00"), SDate("2019-01-02T12:00"), paxFeedSourceOrder).flights.values.toSet
  ...
  "Given a PortState with a flight scheduled after next midnight and pax arriving before next midnight " +
    ...
    "Then the flight should be in the returned PortState" >> {
      val flight = ApiFlightWithSplits(
        ArrivalGenerator.live(
          iata = "BA0001",
          schDt = "2019-01-03T12:00",
          totalPax = Option(100)
        ).toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2019-01-02T14:00").millisSinceEpoch)),
        Set()
      )
      ...
      val windowedFlights =
        portState.window(SDate("2019-01-02T00:00"), SDate("2019-01-02T23:59"), paxFeedSourceOrder).flights.values.toSet
