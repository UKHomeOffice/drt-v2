...
class TestSnapshottingActor(
    probe: ActorRef,
    snapshotMessage: GeneratedMessage,
...
class SnapshottingSpec extends CrunchTestLike {
  "Given an actor extending the RecoveryActorLike trait " +
    ...
    "Then I should see the snapshot being saved" >> {
      ...
      val testActor =
        system.actorOf(Props(new TestSnapshottingActor(probe.ref, snapshotMessage, snapshotBytesThreshold, None)))
  ...
  "Given an actor extending the RecoveryActorLike trait and a snapshot interval of 10 " +
    ...
    "Then I should not see a snapshot being saved" >> {
      ...
      val testActor = system.actorOf(Props(new TestSnapshottingActor(
        probe.ref,
        snapshotMessage,
