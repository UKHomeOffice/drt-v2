...
case class TestShiftsAssignmentService(staff: Int) extends StaffAssignmentsLike {
  ...
  override def terminalStaffAt(
      terminal: Terminal,
      dateMillis: SDateLike,
      msToSDate: MillisSinceEpoch => SDateLike
  ): Int = staff
}
...
class StaffDeploymentSpec extends CrunchTestLike {
  ...
  "Given a set of CrunchMinutes representing a single terminal with 3 queues at one minute " +
    ...
    "Then I see the staff available distributed to the appropriate queues" >> {
      ...
      val staffSources = StaffSources(
        testShiftsService,
        testFixedPointsService,
        testStaffService,
        (_, _, _) => staffAvailable,
        sdateFromMillisLocal
      )
    ...
    "Given a shift with 10 staff and passengers split to 2 queues " +
      ...
      "Then I should see 1 EEA desk & 1 Non-EEA desks deployed - ie the min desks for each queue " >> {
        ...
        crunch.portStateTestProbe.fishForMessage(5.seconds) {
          case ps: PortState =>
            val minutesInOrder = ps.crunchMinutes.values.toList.filter(m =>
              shiftStart.millisSinceEpoch <= m.minute && m.minute <= shiftStart.addMinutes(4).millisSinceEpoch
            )
            val deployments =
              minutesInOrder.map(cm => (cm.queue, SDate(cm.minute), cm.deployedDesks.getOrElse(0))).toSet
