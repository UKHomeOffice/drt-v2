...
class PortStateMinutesSpec extends Specification {
    ...
    "Containing only new arrivals " >> {
      ...
      "To an empty PortState" >> {
        "Then I should see those flights in the PortState" >> {
          ...
          val expected = PortState(newFlightsWithSplits.flightsToUpdate.map(_.copy(lastUpdated = Option(now))), List(), List()).mutable
  ...
  "When I apply a DeskRecMinutes " >> {
    "Containing only new minutes " >> {
      ...
      val newCrunchMinutes = newDeskRecMinutes.minutes.toList.map(_.toUpdatedMinute(now))
      ...
      "To an existing PortState which already has some minutes" >> {
        "Then I should see the existing and new minutes in the PortState" >> {
          ...
          val existingCrunchMinutes = existingDeskRecMinutes.minutes.toList.map(_.toUpdatedMinute(0L))
  ...
  "When I apply a SimulationMinutes " >> {
    "Containing only new minutes " >> {
      ...
      val newCrunchMinutes = newSimulationMinutes.minutes.toList.map(_.toUpdatedMinute(now))
      ...
      "To an existing PortState which already has some minutes" >> {
        "Then I should see the existing and new minutes in the PortState" >> {
          ...
          val existingCrunchMinutes = existingSimulationMinutes.minutes.toList.map(_.toUpdatedMinute(0L))
          val existingPortState = PortState(List(), existingCrunchMinutes, List()).mutable
