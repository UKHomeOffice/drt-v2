...
class ArrivalsGraphStageEdiSpec extends CrunchTestLike {
  ...
  "An EDI Arrivals Graph Stage" should {
    ...
    "Reassign the terminal to A1 for an incoming arrival with A2 but with an A1 baggage belt" in {
      val live = ArrivalGenerator.live(
        iata = "BA1111",
        schDt = scheduled,
        origin = PortCode("JFK"),
        terminal = A2,
      ...
      val fcBase =
        ArrivalGenerator.forecast(iata = "BA1111", schDt = scheduled, origin = PortCode("JFK"), terminal = A2)
    ...
    "Reassign the terminal to A1 for an existing incoming arrival with A2 but with an A1 baggage belt, and remain in A1 with cirium updates" in {
      ...
      offerAndWait(
        crunch.ciriumArrivalsInput,
        ArrivalsFeedSuccess(Seq(live.copy(estimated = Option(SDate(scheduled).millisSinceEpoch))))
      )
    ...
    "Remove any existing duplicates in the PortState" in {
      ...
      val mergedA1 = ApiFlightWithSplits(
        withPcpTime(ArrivalGenerator.live(
          iata = "BA1111",
          schDt = scheduled,
          origin = PortCode("JFK"),
          terminal = A1,
    ...
    "Retain live arrival switch after ACL updates" in {
      ...
      val fcBaseNew =
        ArrivalGenerator.forecast(iata = "BA0001", schDt = scheduled, origin = PortCode("JFK"), terminal = A2)
    ...
    "Retain previous baggage id when live update has no baggage info" in {
      ...
      val updatedLive = ArrivalGenerator.live(
        iata = "BA1111",
        schDt = scheduled,
        origin = PortCode("JFK"),
        terminal = A2,
      ...
      crunch.portStateTestProbe.fishForMessage(1.seconds, s"looking for arrival at A1") {
        case ps: PortState =>
          ps.flights.values.map(a =>
            (
              a.apiFlight.flightCodeString,
              a.apiFlight.BaggageReclaimId,
              a.apiFlight.PassengerSources.get(LiveFeedSource).flatMap(_.actual)
            )
          ) == List(("BA1111", Option("1"), Option(100)))
      }
    ...
    "Retain live arrival switch after ACL updates" in {
      ...
      val live1a1 = ArrivalGenerator.live(
        iata = "BA0001",
        schDt = scheduled,
        origin = PortCode("JFK"),
        terminal = A2,
      ...
      val acl1a1 =
        ArrivalGenerator.forecast(iata = "BA0011", schDt = scheduled, origin = PortCode("JFK"), terminal = A2)
      ...
      val live2a2 = ArrivalGenerator.live(
        iata = "BA0002",
        schDt = scheduled,
        origin = PortCode("JFK"),
        terminal = A2,
      ...
      val live3a1 = ArrivalGenerator.live(
        iata = "BA0003",
        schDt = scheduled,
        origin = PortCode("JFK"),
        terminal = A2,
        baggageReclaimId = Option("2")
      )
      ...
      val cirium2a2 = ArrivalGenerator.live(
        iata = "BA0002",
        schDt = scheduled,
        origin = PortCode("JFK"),
        terminal = A2,
      ...
      val cirium3a1 = ArrivalGenerator.live(
        iata = "BA0003",
        schDt = scheduled,
        origin = PortCode("JFK"),
        terminal = A2,
      ...
      val forecast2a2 =
        ArrivalGenerator.forecast(iata = "BA0002", schDt = scheduled, origin = PortCode("JFK"), terminal = A2)
      val forecast3a1 =
        ArrivalGenerator.forecast(iata = "BA0003", schDt = scheduled, origin = PortCode("JFK"), terminal = A2)
