...
class ArrivalUpdatesHealthCheckSpec extends CrunchTestLike {
  ...
  "Given one flight due to land in the next 30 minutes and it has been updated in the past 30 minutes" >> {
    "the missing percentage for the next 30 minutes should be 0" >> {
      val flights = flightsStream(Seq(
        ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA0001", schDt = "2023-10-20T12:25").toArrival(LiveFeedSource), Set(), Option(myNow.addMinutes(-10).millisSinceEpoch))
      ))
  ...
  "Given two flights due to land in the next 30 minutes and only one has been updated in the past 30 minutes" >> {
    "the missing percentage for the next 30 minutes should be 0.5" >> {
      val flights = flightsStream(Seq(
        ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA0001", schDt = "2023-10-20T12:15").toArrival(LiveFeedSource), Set(), Option(myNow.addMinutes(-40).millisSinceEpoch)),
        ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA0011", schDt = "2023-10-20T12:25").toArrival(LiveFeedSource), Set(), Option(myNow.addMinutes(-10).millisSinceEpoch))
      ))
  ...
  "Given two flights due to land in the next 30 minutes and neither has been updated in the past 30 minutes" >> {
    "the missing percentage for the next 30 minutes should be 1" >> {
      val flights = flightsStream(Seq(
        ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA0001", schDt = "2023-10-20T12:15").toArrival(LiveFeedSource), Set(), Option(myNow.addMinutes(-40).millisSinceEpoch)),
        ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA0011", schDt = "2023-10-20T12:25").toArrival(LiveFeedSource), Set(), Option(myNow.addMinutes(-40).millisSinceEpoch)),
      ))
