...
class LandingTimesHealthCheckSpec extends CrunchTestLike {
      ...
      val flights = flightsStream(Seq(
        ApiFlightWithSplits(
          ArrivalGenerator.live(
            iata = "BA0001",
            actDt = "2023-10-20T11:35",
            schDt = "2023-10-20T11:35"
  ...
  "Given two flights due to land in the last 30 minutes and only one has a landing time" >> {
    "the received percentage for the last 30 minutes should be 0.5" >> {
      val flights = flightsStream(Seq(
        ApiFlightWithSplits(
          ArrivalGenerator.live(iata = "BA0001", schDt = "2023-10-20T11:35").toArrival(LiveFeedSource),
          Set(),
          None
        ),
        ApiFlightWithSplits(
          ArrivalGenerator.live(
            iata = "BA0011",
            actDt = "2023-10-20T11:45",
            schDt = "2023-10-20T11:45"
          ).toArrival(LiveFeedSource),
          Set(),
          None
        )
  ...
  "Given two flights due to land in the last 30 minutes and neither has a landing time" >> {
    "the received percentage for the last 30 minutes should be 0" >> {
      val flights = flightsStream(Seq(
        ...
        ApiFlightWithSplits(
          ArrivalGenerator.live(iata = "BA0011", schDt = "2023-10-20T11:45").toArrival(LiveFeedSource),
          Set(),
          None
        )
      ))
