...
class QueueDiversionSpec extends CrunchTestLike {
  ...
  "Concerning queue diversions" >> {
    ...
    val splits = Map(
      PaxTypeAndQueue(EeaMachineReadable, EeaDesk) -> deskRatios(EeaDesk),
      PaxTypeAndQueue(EeaMachineReadable, EGate) -> deskRatios(EGate)
    )

    val config = airportConfigForSplits(splits)
    val allGatesClosed = PortEgateBanksUpdates(Map(T1 -> EgateBanksUpdates(List(EgateBanksUpdate(
      0L,
      IndexedSeq(EgateBank(IndexedSeq(false, false, false, false, false)))
    )))))
    ...
    "Given an arrival" >> {
      ...
      "When the Egates and Eea desks are closed I should see all pax in the Non-Eea queue" >> {
        implicit val crunch: CrunchGraphInputsAndProbes = runCrunchGraph(TestConfig(
          airportConfig = config.copy(minMaxDesksByTerminalQueue24Hrs =
            config.minMaxDesksByTerminalQueue24Hrs.updated(
              T1,
              Map(
