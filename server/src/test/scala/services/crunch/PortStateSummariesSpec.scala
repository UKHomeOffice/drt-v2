...
class PortStateSummariesSpec extends Specification {
  "Given a port state with crunch minutes for 2 queues over 30 minutes " +
    ...
    val portState = CrunchApi.PortState(Map[UniqueArrival, ApiFlightWithSplits](), cmsMap, SortedMap[TM, StaffMinute]())
  ...
  "Given a port state with staff minutes for 2 queues over 30 minutes " +
    ...
    val portState = CrunchApi.PortState(Map[UniqueArrival, ApiFlightWithSplits](), SortedMap[TQM, CrunchMinute](), smsMap)
