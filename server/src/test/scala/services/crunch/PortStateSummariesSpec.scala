...
class PortStateSummariesSpec extends Specification {
  "Given a port state with crunch minutes for 2 queues over 30 minutes " +
    ...
    "Then I should see crunch minutes for all periods with the sum of any loads, and the max of any desks or wait times" >> {
      ...
      val periods = 4
      val periodSize = 15
      val summary: Map[MillisSinceEpoch, Map[Queue, CrunchMinute]] =
        portState.crunchSummary(SDate(0L), periods, periodSize, T1, queues)

      val expected = Map(
        0L -> Map(
          Queues.EeaDesk ->
            CrunchMinute(T1, Queues.EeaDesk, 0, 105, 105, 14, 14, Option(14), Option(14), Option(14), Option(14)),
          Queues.EGate ->
            CrunchMinute(T1, Queues.EGate, 0, 105, 105, 14, 14, Option(14), Option(14), Option(14), Option(14))
        ),
        15L * 60000 -> Map(
          Queues.EeaDesk -> CrunchMinute(
            T1,
            Queues.EeaDesk,
            15 * 60000,
            330,
          ...
          Queues.EGate ->
            CrunchMinute(T1, Queues.EGate, 15 * 60000, 330, 330, 29, 29, Option(29), Option(29), Option(29), Option(29))
        ),
        30L * 60000 -> Map(
  ...
  "Given a port state with crunch minutes with no deployed or actual desks & wait times " +
    ...
    "Then I should see crunch minutes with None for all the deployed and actual desks and wait times values" >> {
      ...
      val periodSize = 15
      val summary: Map[MillisSinceEpoch, Map[Queue, CrunchMinute]] =
        portState.crunchSummary(SDate(0L), periods, periodSize, terminal, queues)
  ...
  "staffPeriodSummary should return the highest available staff using minute as a tie breaker to be deterministic" >> {
    ...
    "Given 2 staff minutes where the second has the highest available staff" >> {
      ...
      val ps = PortState(List(), List(), List(m1, m2))
      ps.staffPeriodSummary(T1, SDate("2025-09-16T10:00").millisSinceEpoch, List(m1, m2)) ===
        m2.copy(minute = m1.minute)
    }
  ...
  "PortState " should {
    "correctly calculate UTC time days when creating a summary spanning a clock change" in {
      val summaries =
        PortState.empty.dailyCrunchSummary(SDate("2022-03-27", utcTimeZone), 7, T1, (_, _, _) => Seq(EeaDesk))
    ...
    "correctly calculate local time days when creating a summary spanning a clock change" in {
      val summaries =
        PortState.empty.dailyCrunchSummary(SDate("2022-03-27", europeLondonTimeZone), 7, T1, (_, _, _) => Seq(EeaDesk))
      val days = summaries.keys.toList.sorted.map(SDate(_))
