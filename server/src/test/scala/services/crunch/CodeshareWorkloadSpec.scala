...
class CodeshareWorkloadSpec extends CrunchTestLike {
  "Given 2 arrivals that are codeshares " +
    ...
    "I should see only pax loads from the highest pax arrival" >> {
      ...
      val arrival1 =
        ArrivalGenerator.live(iata = "BA0001", schDt = sch, totalPax = Option(15), origin = PortCode("AAA"))
      val arrival2 =
        ArrivalGenerator.live(iata = "AA0002", schDt = sch, totalPax = Option(10), origin = PortCode("AAA"))
      ...
      val updatedArrival2 =
        arrival2.copy(estimated = Option(schSdate.addMinutes(1).millisSinceEpoch), totalPax = Option(16))
      ...
      crunch.portStateTestProbe.fishForMessage(5.seconds) {
        case PortState(_, crunchMinutes, _) =>
          ...
          val minute2paxCorrect =
            crunchMinutes.get(TQM(T1, Queues.EeaDesk, schSdate.addMinutes(1).millisSinceEpoch)) match {
              case Some(minute) =>
                minute.paxLoad == 16
              case _ => false
            }
