...
class RunnableDeskRecsSpec extends CrunchTestLike {
  ...
  "Given a RunnableDescRecs with a mock PortStateActor and mock crunch " +
    ...
    val (millisToCrunchSourceActor: ActorRef, _) = RunnableDeskRecs(mockPortStateActor, DesksAndWaitsPortProvider(defaultAirportConfig, mockCrunch, pcpPaxFn), newBuffer, maxDesksProvider).run()
  ...
  "Given a flight with splits " +
    ...
    val (actor: ActorRef, _) = RunnableDeskRecs(mockPortStateActor, DesksAndWaitsPortProvider(testAirportConfig, mockCrunch, pcpPaxFn), newBuffer, maxDesksProvider).run()
