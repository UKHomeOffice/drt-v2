...
import uk.gov.homeoffice.drt.models.{ B5JPlusTypeAllocator, CrunchMinute, TQM }
...
class RunnableDeskRecsSpec extends CrunchTestLike {
  ...
  val noDelay = 10L
  val longDelay = 250L
  val historicSplits: Splits = Splits(
    Set(
  ...
  private def getDeskRecsGraph(
      mockPortStateActor: ActorRef,
    ...
    val desksAndWaitsProvider = PortDesksAndWaitsProvider(
      ...
      paxFeedSourceOrder,
      (_: LocalDate, q: Queue) => Future.successful(airportConfig.slaByQueue(q)),
      identity
    )
    ...
    val deskRecsProducer = DynamicRunnablePassengerLoads.crunchRequestsToQueueMinutes(
      ...
      dynamicQueueStatusProvider = () =>
        MockEgatesProvider.portProvider(airportConfig)().map { ep =>
    ...
    val crunchGraphSource =
      new SortedActorRefSource(TestProbe().ref, SortedSet.empty[TerminalUpdateRequest], "desk-recs")
  ...
  "Given a flight with splits, when I ask for the workload" >> {
    "Then I should see the workload associated with the best splits for that flight" >> {
      ...
      val arrival =
        ArrivalGenerator.arrival(iata = "BA0001", schDt = scheduled, totalPax = Option(25), feedSource = LiveFeedSource)
      ...
      portStateProbe.fishForMessage(2.seconds) {
        case minutes: MinutesContainer[PassengersMinute, TQM] =>
          val result = minutes.minutes.filterNot(_.toMinute.passengers.isEmpty).map(drm =>
            (drm.toMinute.queue, drm.toMinute.passengers.size, drm.minute)
          ).toSet
  ...
  "Given two flights with splits with overlapping pax " +
    ...
    "Then I should see the combined workload for those flights" >> {
      ...
      val flight = List(
        ApiFlightWithSplits(arrival, Set(historicSplits), None),
        ApiFlightWithSplits(arrival2, Set(historicSplits), None)
      )
  ...
  "Given a single flight with pax arriving at PCP at 00:15 and going to a single queue " +
    ...
    "Then the workload from noon should disappear and move to 00:05" >> {
      ...
      val arrival =
        ArrivalGenerator.live(iata = "BA0001", schDt = pcpOne, totalPax = Option(25)).toArrival(LiveFeedSource)
      ...
      mockPortStateActor !
        SetFlights(List(ApiFlightWithSplits(arrival.copy(PcpTime = Option(noonMillis)), Set(historicSplits), None)))
      ...
      mockPortStateActor !
        SetFlights(List(ApiFlightWithSplits(arrival.copy(PcpTime = Option(onePmMillis)), Set(historicSplits), None)))
  ...
  "Given an existing single flight with pax arriving at PCP at midnight " +
    ...
    "Then the workload in the PortState from noon should disappear and move to 00:30" >> {
      ...
      val flight1 = ApiFlightWithSplits(
        arrival.toArrival(LiveFeedSource).copy(PcpTime = Option(noonMillis)),
        Set(historicSplits),
        None
      )
  ...
  "Given a two flights with pax arriving at PCP at midnight " +
    ...
    "Then the workload in the PortState from noon should spread between 00:00 and 00:30" >> {
      ...
      val arrival =
        ArrivalGenerator.live(iata = "BA0001", schDt = noon, totalPax = Option(25), origin = PortCode("JFK"))
      val arrival2 =
        ArrivalGenerator.live(iata = "BA0002", schDt = noon, totalPax = Option(25), origin = PortCode("AAA"))
  ...
  "Given a flight with splits which is subsequently removed " +
    ...
    "Then I should see zero loads for the minutes affected by the flight in question" >> {
      ...
      val arrival =
        ArrivalGenerator.forecast(iata = "BA0001", schDt = noon, totalPax = Option(25), origin = PortCode("JFK"))
      val arrival2 =
        ArrivalGenerator.forecast(iata = "BA0002", schDt = noon, totalPax = Option(25), origin = PortCode("AAA"))
  ...
  "Given loads for a set of minutes within a day for 2 queues " +
    ...
    "Then I should see a full day's worth (1440) of crunch minutes per queue crunched - a total of 1440 * 2" >> {
      ...
      portStateProbe.fishForMessage(2.seconds) {
        case mins: MinutesContainer[CrunchMinute, TQM] => mins.minutes.size ===
            defaultAirportConfig.queuesByTerminal.head._2(T1).size * minsInADay
        case _ => false
      }
