...
import uk.gov.homeoffice.drt.models.{ B5JPlusTypeAllocator, CrunchMinute, TQM, UniqueArrivalKey }
...
object OptimiserMocks {
  ...
  def mockFlightsProvider(flights: List[ApiFlightWithSplits])
      : TerminalUpdateRequest => Future[Source[List[ApiFlightWithSplits], NotUsed]] =
    _ => Future.successful(Source(List(flights)))
}

case class MockManifestLookupService(
    bestAvailableManifests: Map[UniqueArrivalKey, Option[BestAvailableManifest]],
...
class RunnableDynamicDeskRecsSpec extends CrunchTestLike {
  ...
  val splitsCalculator: SplitsCalculator =
    manifests.queues.SplitsCalculator(ptqa, airportConfig.terminalPaxSplits, AdjustmentsNoop)
  ...
  def setupGraphAndCheckQueuePax(
      flight: ApiFlightWithSplits,
      expectedQueuePax: Map[(Terminal, Queue), Int]
  ): Any = {
    val probe = TestProbe()
    ...
    val maxDeskProviders =
      PortDeskLimits.flexed(airportConfig, MockEgatesProvider.terminalProvider(airportConfig), _ => dummyPaxForQueue)
    val queueMinutesProducer = DynamicRunnableDeskRecs.crunchRequestsToDeskRecs(
      ...
      loadsToQueueMinutes = (_, mins, _, _, _) =>
        Future.successful(DeskRecMinutes(mins.values.map(m =>
          DeskRecMinute(m.terminal, m.queue, m.minute, m.passengers.size, m.passengers.sum, 0, 0, None)
        ).toList)),
      setUpdatedAtForDay = (_, _, _) => Future.successful(Done)
    )

    val crunchGraphSource =
      new SortedActorRefSource(TestProbe().ref, SortedSet.empty[TerminalUpdateRequest], "passenger-loads")

    val (queue, _) =
      QueuedRequestProcessing.createGraph(crunchGraphSource, sink, queueMinutesProducer, "passenger-loads").run()
    queue ! request
  ...
  "Given an arrival with 100 pax " >> {
    val arrival =
      ArrivalGenerator.live("BA0001", schDt = s"2021-06-01T12:00", origin = PortCode("JFK"), totalPax = Option(100))
