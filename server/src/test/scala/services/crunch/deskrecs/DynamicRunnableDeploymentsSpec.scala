...
class DynamicRunnableDeploymentsSpec extends CrunchTestLike {
  ...
  val maxDesksProvider: Map[Terminal, TerminalDeskLimitsLike] =
    PortDeskLimits.flexed(airportConfig, egatesProvider, _ => dummyPaxForQueue)
  val mockCrunch: TryCrunchWholePax = CrunchMocks.mockCrunchWholePax

  val staffToDeskLimits: (Terminal, List[Int]) => FlexedTerminalDeskLimitsFromAvailableStaff =
    PortDeskLimits.flexedByAvailableStaff(airportConfig, egatesProvider, _ => dummyPaxForQueue)
  ...
  def setupGraphAndCheckQueuePax(
      minutes: MinutesContainer[PassengersMinute, TQM],
      expectedQueuePax: PartialFunction[Any, Boolean]
  ): Any = {
    val probe = TestProbe()
    ...
    "When I ask for deployments I should see 1440 minutes for each queue" >> {
      val expected: PartialFunction[Any, Boolean] = {
        case minutes: MinutesContainer[CrunchMinute, TQM] =>
          val minuteCountByQueue: Map[Queue, Int] =
            minutes.minutes.groupBy(_.toMinute.queue).view.mapValues(_.size).toMap
