...
class ForecastCrunchSpec extends CrunchTestLike {
  ...
  "Given a base flight with 21 pax" +
    ...
    "Then I should see 20 pax in the 1st minute and 1 in the second" >> {
      ...
      val baseArrival =
        ArrivalGenerator.forecast(schDt = baseScheduled, iata = "BA0001", terminal = T1, totalPax = Option(21))
      val baseFlights = List(baseArrival)
  ...
  "Given a forecast arrival with no matching base arrival " +
    ...
    "Then I should not see any arrivals" >> {
      ...
      val forecastArrival =
        ArrivalGenerator.forecast(schDt = forecastScheduled, iata = "BA0001", terminal = T1, totalPax = Option(21))
  ...
  "Given a base arrival with 21 pax, and a matching forecast arrival with 50 pax, 25 trans pax and a different format flight code " +
    ...
    "Then I should see the base arrival with the forecast's pax & transpax nos overlaid, but retaining the base's flight code" >> {
      ...
      val baseArrival =
        ArrivalGenerator.forecast(schDt = baseScheduled, iata = "BA0001", terminal = T1, totalPax = Option(21))
      val forecastArrival = ArrivalGenerator.forecast(
        schDt = forecastScheduled,
        iata = "BAW0001",
        terminal = T1,
        totalPax = Option(50),
        transPax = Option(25)
  ...
  "Given a base arrival with 21 pax, followed by a matching forecast arrival with 50 pax, and finally a live flight with zero pax " +
    ...
    "Then I should see the base arrival details with the forecast pax" >> {
      ...
      val baseArrival = ArrivalGenerator.forecast(
        schDt = baseScheduled,
        iata = "BA0001",
        terminal = T1,
        totalPax = Option(21),
      ...
      val liveArrival = ArrivalGenerator.live(
        schDt = liveScheduled,
        iata = "BA0001",
        terminal = T1,
        estDt = liveScheduled,
      ...
      crunch.portStateTestProbe.fishForMessage(1.seconds) {
        case PortState(fs, _, _) =>
          val crunchForecastArrivals =
            fs.values.map(_.apiFlight.bestPcpPaxEstimate(Seq(LiveFeedSource, ForecastFeedSource, AclFeedSource))).toSet
          crunchForecastArrivals == expectedForecastArrivals
      }
  ...
  "Given 2 base arrivals followed by 1 matching forecast arrival, and then the other matching forecast arrival " +
    ...
    "Then I should see the forecast pax & status details for both arrivals" >> {
      ...
      val baseArrival1 =
        ArrivalGenerator.forecast(schDt = baseScheduled, iata = "BA0001", terminal = T1, totalPax = Option(21))
      val baseArrival2 =
        ArrivalGenerator.forecast(schDt = baseScheduled, iata = "AA1110", terminal = T1, totalPax = Option(22))
      val forecastArrival1 =
        ArrivalGenerator.forecast(schDt = forecastScheduled, iata = "BAW0001", terminal = T1, totalPax = Option(51))
      val forecastArrival2 =
        ArrivalGenerator.forecast(schDt = forecastScheduled, iata = "AAW1110", terminal = T1, totalPax = Option(52))
  ...
  "Given an initial base arrivals of 1 flight, and and initial merged arrivals of 2 flights (from a port state that hadn't been updated) " +
    ...
    "Then I should only see arrivals that exist in the latest base arrivals" >> {
      ...
      val updatedBaseArrivals =
        List(ArrivalGenerator.forecast(schDt = scheduled, iata = "AA0099", terminal = T1, totalPax = Option(55)))
  ...
  "Given no initial arrivals " +
    ...
    "Then I should see the arrival in the port state with 90 passengers" >> {
      ...
      val arrival =
        ArrivalGenerator.forecast(schDt = scheduled, iata = "BA0001", terminal = T1, totalPax = Option(aclPax))
      val baseArrivals = List(arrival)

      val crunch = runCrunchGraph(TestConfig(
        now = () => SDate(scheduled),
        passengerAdjustments = arrivals =>
          Future.successful(arrivals
            .map {
              case a: LiveArrival => a.copy(
                  totalPax = a.totalPax.map(pax => (pax * paxDelta).toInt),
                  transPax = a.transPax.map(pax => (pax * paxDelta).toInt)
                )
              case a: ForecastArrival => a.copy(
                  totalPax = a.totalPax.map(pax => (pax * paxDelta).toInt),
                  transPax = a.transPax.map(pax => (pax * paxDelta).toInt)
                )
            })
  ...
  def interestingDeployments(cms: Map[TQM, CrunchMinute])
      : scala.Seq[(MillisSinceEpoch, Terminal, Queue, Option[Int])] = {
    cms.values
