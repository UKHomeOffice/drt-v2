...
import actors._
import actors.daily.{
  FlightUpdatesSupervisor,
  QueueUpdatesSupervisor,
  RequestAndTerminateActor,
  StaffUpdatesSupervisor
}
import actors.persistent.ManifestRouterActor
import actors.persistent.arrivals._
import actors.persistent.staffing.{
  FixedPointsActor,
  LegacyShiftAssignmentsActor,
  ShiftAssignmentsActor,
  StaffMovementsActor
}
...
import uk.gov.homeoffice.drt.arrivals.{ ApiFlightWithSplits, Arrival, FlightCode, UniqueArrival, VoyageNumber }
...
case class MockManifestLookupService(
    maybeBestManifest: Option[BestAvailableManifest] = None,
...
object MockEgatesProvider {
  ...
  def portProvider(airportConfig: AirportConfig): () => Future[PortEgateBanksUpdates] = () => {
    val portUpdates = PortEgateBanksUpdates(
      airportConfig.eGateBankSizes.view.mapValues(banks =>
        EgateBanksUpdates(List(EgateBanksUpdate(0L, EgateBank.fromAirportConfig(banks))))
      ).toMap
    )
  ...
  override def receive: Receive = {
    ...
    case tc: TestConfig =>
      ...
      val requestAndTerminateActor: ActorRef =
        system.actorOf(Props(new RequestAndTerminateActor()), "request-and-terminate-actor")
      val terminals: (LocalDate, LocalDate) => Seq[Terminal] =
        QueueConfig.terminalsForDateRange(tc.airportConfig.queuesByTerminal)

      def feedArrivalsRouter(
          source: FeedSource,
          partitionUpdates: PartialFunction[FeedArrivals, Map[(Terminal, UtcDate), FeedArrivals]],
          name: String
      ): ActorRef =
        system.actorOf(
          Props(new FeedArrivalsRouterActor(
            terminals,
      ...
      val forecastBaseFeedArrivalsActor: ActorRef = feedArrivalsRouter(
        AclFeedSource,
      ...
      val forecastFeedArrivalsActor: ActorRef =
        feedArrivalsRouter(ForecastFeedSource, partitionUpdates, "forecast-arrivals-actor")
      val liveBaseFeedArrivalsActor: ActorRef =
        feedArrivalsRouter(LiveBaseFeedSource, partitionUpdates, "live-base-arrivals-actor")
      ...
      val liveShiftsReadActor: ActorRef = system.actorOf(
        LegacyShiftAssignmentsActor.streamingUpdatesProps(
          LegacyShiftAssignmentsActor.persistenceId,
          journalType,
          tc.now
        ),
        name = "shifts-read-actor"
      )

      val liveShiftAssignmentsReadActor: ActorRef = system.actorOf(
        TestShiftsActor.streamingUpdatesProps(
          ShiftAssignmentsActor.persistenceId,
          journalType,
          tc.now
        ),
        name = "staff-assignments-read-actor"
      )
      val liveFixedPointsReadActor: ActorRef = system.actorOf(
        FixedPointsActor.streamingUpdatesProps(
          journalType,
          tc.now,
          tc.forecastMaxDays
        ),
        name = "fixed-points-read-actor"
      )
      val liveStaffMovementsReadActor: ActorRef = system.actorOf(
        StaffMovementsActor.streamingUpdatesProps(
          journalType
        ),
        name = "staff-movements-read-actor"
      )

      val shiftsSequentialWritesActor: ActorRef = system.actorOf(
        LegacyShiftAssignmentsActor.sequentialWritesProps(
          tc.now,
          startOfTheMonth(tc.now),
          requestAndTerminateActor,
          system
        ),
        "shifts-sequential-writes-actor"
      )
      val fixedPointsSequentialWritesActor: ActorRef = system.actorOf(
        FixedPointsActor.sequentialWritesProps(
          tc.now,
          requestAndTerminateActor,
          system
        ),
        "fixed-points-sequential-writes-actor"
      )
      val staffMovementsSequentialWritesActor: ActorRef = system.actorOf(
        StaffMovementsActor.sequentialWritesProps(
          tc.now,
          time48HoursAgo(tc.now),
          requestAndTerminateActor,
          system
        ),
        "staff-movements-sequential-writes-actor"
      )

      val manifestLookups = ManifestLookups(system, airportConfig.terminalsForDate)

      val manifestsRouterActor: ActorRef = system.actorOf(Props(new ManifestRouterActor(
        manifestLookups.manifestsByDayLookup,
        manifestLookups.updateManifests
      )))
      ...
      val (updateFlightsLiveView, update15MinuteQueueSlotsLiveView) = tc.maybeAggregatedDbTables match {
        ...
        case None =>
          (
            (_: Iterable[ApiFlightWithSplits], _: Iterable[UniqueArrival]) => Future.successful(()),
            (_: UtcDate, _: Iterable[CrunchMinute]) => Future.successful(())
          )
      }
      ...
      val flightLookups: FlightLookups =
        FlightLookups(system, tc.now, terminalsForDateRange, None, paxFeedSourceOrder, _ => None, updateFlightsLiveView)
      val flightsRouterActor: ActorRef = flightLookups.flightsRouterActor
      val minuteLookups: MinuteLookupsLike = MinuteLookups(
        tc.now,
        MilliTimes.oneDayMillis,
        terminalsForDateRange,
        queuesForDateAndTerminal,
      ...
      val queueUpdates = system.actorOf(
        Props(new QueueUpdatesSupervisor(
          tc.now,
          terminalsForDate,
          queueUpdatesProps(tc.now, InMemoryStreamingJournal)
        )),
        "updates-supervisor-queues"
      )
      val staffUpdates = system.actorOf(
        Props(new StaffUpdatesSupervisor(
          tc.now,
          terminalsForDate,
          staffUpdatesProps(tc.now, InMemoryStreamingJournal)
        )),
        "updates-supervisor-staff"
      )
      val flightUpdates = system.actorOf(
        Props(new FlightUpdatesSupervisor(
          tc.now,
          terminalsForDate,
          flightUpdatesProps(tc.now, InMemoryStreamingJournal)
        )),
        "updates-supervisor-flight"
      )
      val portStateActor = system.actorOf(
        Props(new PartitionedPortStateTestActor(
          portStateProbe.ref,
          flightsRouterActor,
          queuesActor,
          staffActor,
      ...
      val terminalEgatesProvider = tc.maybeEgatesProvider match {
        ...
        case Some(provider) =>
          (terminal: Terminal) =>
            provider().map(p =>
              p.updatesByTerminal.getOrElse(terminal, throw new Exception(s"No egates found for $terminal"))
            )
      }
      ...
      val portDeskRecs = PortDesksAndWaitsProvider(
        tc.airportConfig,
        tc.cruncher,
        FlightFilter.forPortConfig(tc.airportConfig),
        paxFeedSourceOrder,
        (_: LocalDate, q: Queue) => Future.successful(tc.airportConfig.slaByQueue(q)),
        uniqueArrivals
      )
      ...
      val staffToDeskLimits =
        PortDeskLimits.flexedByAvailableStaff(tc.airportConfig, terminalEgatesProvider, paxForQueue) _

      val ptqa = paxTypeQueueAllocator(
        tc.airportConfig.hasTransfer,
        TerminalQueueAllocator(tc.airportConfig.terminalPaxTypeQueueAllocation)
      )
      ...
      val startDeskRecs: () => (ActorRef, ActorRef, ActorRef, ActorRef, Iterable[UniqueKillSwitch]) = () => {
        ...
        val killSwitches = Iterable(
          mergeArrivalsKillSwitch,
          historicSplitsKillSwitch,
          historicPaxKillSwitch,
          deskRecsKillSwitch,
        ...
        (
          mergeArrivalsRequestActor,
          crunchRequestQueueActor,
          deskRecsRequestQueueActor,
          deploymentRequestActor,
          killSwitches
        )
      }

      val manifestsSource: Source[ManifestsFeedResponse, SourceQueueWithComplete[ManifestsFeedResponse]] =
        Source.queue[ManifestsFeedResponse](0, OverflowStrategy.backpressure)
      ...
      val manifestsLiveResponseSource: SourceQueueWithComplete[ManifestsFeedResponse] =
        manifestsSource.mapAsync(1)(persistManifests).toMat(Sink.ignore)(Keep.left).run()

      val liveArrivals: Source[ArrivalsFeedResponse, SourceQueueWithComplete[ArrivalsFeedResponse]] =
        Source.queue[ArrivalsFeedResponse](0, OverflowStrategy.backpressure)
      val liveBaseArrivals: Source[ArrivalsFeedResponse, SourceQueueWithComplete[ArrivalsFeedResponse]] =
        Source.queue[ArrivalsFeedResponse](0, OverflowStrategy.backpressure)
      val forecastArrivals: Source[ArrivalsFeedResponse, SourceQueueWithComplete[ArrivalsFeedResponse]] =
        Source.queue[ArrivalsFeedResponse](0, OverflowStrategy.backpressure)
      val forecastBaseArrivals: Source[ArrivalsFeedResponse, SourceQueueWithComplete[ArrivalsFeedResponse]] =
        Source.queue[ArrivalsFeedResponse](0, OverflowStrategy.backpressure)
      ...
      val crunchInputs = CrunchSystem(CrunchProps(
        ...
        system = system,
        updateFeedStatus =
          (feedSource: FeedSource, response: ArrivalsFeedResponse) => feedStatusWriteActors(feedSource) ! response
      ))
