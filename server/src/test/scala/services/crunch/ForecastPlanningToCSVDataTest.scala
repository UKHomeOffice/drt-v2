...
class ForecastPlanningToCSVDataTest extends Specification {
  ...
  "Given a ForecastPeriod which includes a period that spans a timezone change from BST to UTC " +
    ...
    val result = CSVData.rowHeadingsForPeriod(forecastPeriod).take(12)
  ...
  "Given a ForecastPeriod which includes a period that spans a timezone change from BST to UTC " +
    ...
    val forecastPeriodDays: ForecastPeriod = forecastPeriodStartingOnBSTToUTCChange(2)
  ...
  def forecastPeriodStartingOnBSTToUTCChange(daysToAdd: Int) = {
    val forecastStart = getLocalLastMidnight(SDate("2018-10-28T00:00:00Z"))
    ...
    val days = range.toList.groupBy(m => getLocalLastMidnight(SDate(m, Crunch.europeLondonTimeZone)).millisSinceEpoch)
