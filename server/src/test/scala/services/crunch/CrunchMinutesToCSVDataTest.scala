...
class CrunchMinutesToCSVDataTest extends Specification {
  ...
  "Given a set of crunch minutes for a terminal, we should receive a CSV for that terminals data" in new Context {
    ...
    val cms: SortedMap[TQM, CrunchMinute] = SortedMap[TQM, CrunchMinute]() ++ List(
    ...
    val staffMins: SortedMap[TM, StaffMinute] = SortedMap[TM, StaffMinute]() ++ List(
      StaffMinute(T1, startDateTime.millisSinceEpoch, 5, fixedPoints = 1, movements = -1)
    ).map(sm => (sm.key, sm))

    val result: String = CSVData.terminalMinutesToCsvData(cms, staffMins, Seq(Queues.EeaDesk, Queues.EGate, Queues.NonEeaDesk), startDateTime, startDateTime.addMinutes(14), 15)
  ...
  "Given a set of crunch minutes for a terminal, we should get the result back in 15 minute increments" in new Context {
    ...
    val result = CSVData.terminalMinutesToCsvData(cms, staffMins, Seq(Queues.EeaDesk, Queues.EGate, Queues.NonEeaDesk), startDateTime, startDateTime.addMinutes(29), 15)
