...
class CrunchFlightExclusionsSpec extends CrunchTestLike {
  ...
  "Given two flights, one with an invalid terminal " +
    ...
    val flights = Flights(List(
      ...
      ArrivalGenerator.arrival(iata = "FR8819", schDt = scheduled01, actPax = Option(10), terminal = "XXX")
    ))
  ...
  "Given two flights, one with a cancelled " +
    ...
    val flights = Flights(List(
      ArrivalGenerator.arrival(iata = "BA0001", schDt = scheduled00, actPax = Option(15), terminal = "T1"),
      ArrivalGenerator.arrival(iata = "FR8819", schDt = scheduled01, actPax = Option(10), terminal = "T1", status = "Cancelled")
    ))
