  ...
  "Given a set of forecast minutes and staff minutes for all terminals, " >> {
    "When I roll up for week per terminal " >> {
      "Then I should get the lowest number in each 15 minute block relevant to the particular terminal" >> {
        ...
        val queues =
          QueueConfig.queuesForDateAndTerminal(defaultAirportConfig.queuesByTerminal)(LocalDate(2025, 1, 1), T1)
        val ps =
          PortState(List(), (crunchMinutesT1 ++ crunchMinutesT2).toList, (staffMinutesT1 ++ staffMinutesT2).toList)
