...
import uk.gov.homeoffice.drt.arrivals.{CarrierCode, EventTypes, ForecastArrival, LiveArrival, VoyageNumber}
...
import uk.gov.homeoffice.drt.ports.{ApiFeedSource, HistoricApiFeedSource, PaxAge, PaxTypeAndQueue, PortCode, Queues}
...
class ApiPaxNosCrunchSpec extends CrunchTestLike {
  ...
  val flights = List(forecastArrival)
...
//  }
  ...
  "Given an historic manifests provider and a flight with no historic splits" >> {
    "Then the flight should have historic splits added to it" >> {
      val arrival = forecastArrival.copy(totalPax = Option(2))
      ...
      val crunch = runCrunchGraph(TestConfig(
        ...
        historicManifestLookup = Option(MockManifestLookupService(maybeBestManifest = Option(manifest), maybeManifestPaxCount = None))
      ))
      ...
      crunch.portStateTestProbe.fishForMessage(2.seconds) {
        case PortState(flights, _, _) =>
          ...
          println(s"maybeSplits: $maybeSplits")
          println(s"paxSources: $paxSources")
