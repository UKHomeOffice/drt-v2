...
class EgatesCapacityProviderSpec extends CrunchTestLike {
  "Given a capacity provider" >> {
    val threeBanks =
      IndexedSeq(EgateBank(IndexedSeq(true, true)), EgateBank(IndexedSeq(true, true)), EgateBank(IndexedSeq(true)))
    val oneBank = IndexedSeq(EgateBank(IndexedSeq(true, true)))
    val eventualUpdates =
      Future.successful(EgateBanksUpdates(List(EgateBanksUpdate(0L, threeBanks), EgateBanksUpdate(10L, oneBank))))
