...
object FixedTerminalDeskLimitsSpec {
  val dummyPaxForQueue: (NumericRange[Long], Queue) => Future[Seq[Int]] =
    (millis, _) => Future.successful(Seq.fill(millis.size)(0))
}
...
class FixedTerminalDeskLimitsSpec extends CrunchTestLike {
  ...
  "Given a fixed desk limits provider with one queue with a max of 10 desks " +
    ...
    "Then I should get 10 for every hour" >> {
      ...
      val limits = FixedTerminalDeskLimits(
        _ => Map(EeaDesk -> minDesks),
        Map(EeaDesk -> DeskCapacityProvider(maxDesks)),
        dummyPaxForQueue
      )
