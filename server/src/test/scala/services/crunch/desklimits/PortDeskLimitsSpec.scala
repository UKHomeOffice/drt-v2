  ...
  "Given a PortDeskLimits with an egates provider giving 3 banks followed by 1 bank from 10 milliseconds" >> {
    "When I ask for the max desks for milliseconds 9 & 10" >> {
      "I should see 3 max followed by 1 max" >> {
        val threeBanks =
          IndexedSeq(EgateBank(IndexedSeq(true)), EgateBank(IndexedSeq(true)), EgateBank(IndexedSeq(true)))
        val oneBankUpdate = IndexedSeq(EgateBank(IndexedSeq(true, true)))
        val eventualUpdates = Future.successful(EgateBanksUpdates(List(
          EgateBanksUpdate(0L, threeBanks),
          EgateBanksUpdate(10L, oneBankUpdate)
        )))
        ...
        maxDesks ===
          WorkloadProcessorsProvider(IndexedSeq(WorkloadProcessors(threeBanks), WorkloadProcessors(oneBankUpdate)))
      }
    }
  }
}
