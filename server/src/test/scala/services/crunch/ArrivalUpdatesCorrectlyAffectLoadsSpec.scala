...
class ArrivalUpdatesCorrectlyAffectLoadsSpec extends CrunchTestLike {
  val crunch: CrunchGraphInputsAndProbes = runCrunchGraph(TestConfig(
    ...
    airportConfig = defaultAirportConfig.copy(
      ...
      queuesByTerminal = SortedMap(LocalDate(2014, 1, 1) ->
        SortedMap(T1 -> Seq(Queues.EeaDesk, Queues.NonEeaDesk, Queues.EGate), T2 -> Seq()))
    )
  ))
  val arrivalOne: LiveArrival = ArrivalGenerator.live(
    iata = "BA0001",
    terminal = T1,
    origin = PortCode("JFK"),
  ...
  val arrivalTwo: LiveArrival = ArrivalGenerator.live(
    iata = "BA0002",
    terminal = T1,
    origin = PortCode("JFK"),
  ...
  "Given crunch inputs and an arrival" >> {
    ...
    "When I send a second arrival with a manifest" >> {
      ...
      val voyageManifests = ManifestsFeedSuccess(DqManifests(
        ...
        Set(
          ...
          manifestForArrival(
            arrivalTwo.toArrival(LiveFeedSource),
            manifestPax(110, euIdCard) ++ manifestPax(7, nonVisa)
          )
        )
      ))
    ...
    "When I send a second arrival with a manifest" >> {
      ...
      val voyageManifests = ManifestsFeedSuccess(DqManifests(
        ...
        Set(
          manifestForArrival(
            updatedArrivalOne.toArrival(LiveFeedSource),
            manifestPax(300, visa) ++ manifestPax(99, euPassport) ++ manifestPax(2, nonVisa)
          ),
          manifestForArrival(
            updatedArrivalTwo.toArrival(LiveFeedSource),
            manifestPax(30, euIdCard) ++ manifestPax(30, nonVisa) ++ manifestPax(116, euPassport)
          )
        )
      ))
  ...
  private def manifestForArrival(updatedArrival: Arrival, paxInfos: List[PassengerInfoJson]): VoyageManifest = {
    ...
    VoyageManifest(
      EventTypes.CI,
      PortCode("STN"),
      updatedArrival.Origin,
      updatedArrival.VoyageNumber,
      updatedArrival.CarrierCode,
      ManifestDateOfArrival(schDateTime.toISODateOnly),
      ManifestTimeOfArrival(schDateTime.toHoursAndMinutes),
      paxInfos
    )
  }
  ...
  private def paxLoadsAreCorrect(ps: PortState, arrivals: Seq[FeedArrival], queues: Seq[Queue]): Boolean = {
    ...
    val earliestPcpMinuteAcrossArrivals = Await.result(
      TestDefaults
        .setPcpFromBest(arrivals.map(_.toArrival(LiveFeedSource)).toList)
        .map(_.minBy(_.PcpTime.getOrElse(Long.MaxValue))),
      1.second
    ).PcpTime.getOrElse(Long.MaxValue)
