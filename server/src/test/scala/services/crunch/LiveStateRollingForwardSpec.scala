...
class LiveStateRollingForwardSpec extends CrunchTestLike {
  ...
  "Given a flight that applies to the few minutes after the live state window, ie in forecast state only " +
    ...
    "I should see the first update's flight after crossing midnight" >> {
      ...
      stateContainsArrivals(
        crunch.portStateTestProbe,
        Seq(futureArrival, futureArrival2).map(_.toArrival(LiveFeedSource))
      )
  ...
  "Given a flight that applies to the few minutes after the live state window, ie in forecast state only " +
    ...
    "I should see both flights after crossing midnight" >> {
      ...
      val futureArrival = ArrivalGenerator.live(
        iata = "BA0001",
        origin = PortCode("JFK"),
        schDt = fridayMidnight30,
        terminal = T1,
      ...
      val futureArrival2 = ArrivalGenerator.live(
        iata = "BA0002",
        origin = PortCode("JFK"),
        schDt = saturdayMidnight30,
        terminal = T1,
