...
class SplitsPredictorStage extends GraphStage[FlowShape[List[ApiFlightWithSplits], List[ApiFlightWithSplits]]] {
  val log = LoggerFactory.getLogger(getClass)

  val in = Inlet[List[ApiFlightWithSplits]]("SplitsPredictor.in")
  val out = Outlet[List[ApiFlightWithSplits]]("SplitsPredictor.out")

  val shape = FlowShape.of(in, out)

  var currentArrivals: Option[List[ApiFlightWithSplits]] = None
    ...
    new GraphStageLogic(shape) {
      ...
      setHandler(in, new InHandler {
        override def onPush(): Unit = {
          ...
          if (currentArrivals.isEmpty) {
            ...
            currentArrivals = Option(arrivals)
      ...
      setHandler(out, new OutHandler {
        override def onPull(): Unit = {
          log.info(s"onPull")
          currentArrivals match {
            case None =>
              log.info(s"Nothing")
              if(!hasBeenPulled(in)) pull(in)
            case Some(arrivalsToPush) =>
              log.info(s"Pushing $arrivalsToPush")
              push(out, arrivalsToPush)
          }
          if(!hasBeenPulled(in)) pull(in)
        }
...
object RunnablePredictor {
  ...
  def apply(arrivalsSource: Source[List[ApiFlightWithSplits], NotUsed],
            splitsPredictor: SplitsPredictorStage,
            sink: Sink[List[ApiFlightWithSplits], Future[Done]]
           ): RunnableGraph[(NotUsed, NotUsed)] = {

    import GraphDSL.Implicits._


    RunnableGraph.fromGraph(GraphDSL.create(arrivalsSource, splitsPredictor)((_, _)) { implicit builder =>
      (arrivals, predictor) =>


        arrivals ~> predictor ~> Sink.foreach(println)
...
class SplitsLearningSpec extends Specification {
  ...
  "I can send a set of arrivals to a graph stage and get back predicted splits" >> {
    implicit val actorSystem: ActorSystem = ActorSystem("splits")
    implicit val materializer: ActorMaterializer = ActorMaterializer()

    val arrival = ApiFlightWithSplits(ArrivalGenerator.apiFlight(iata = "BA0001", schDt = "2018-02-13"), splits = Set())

    val sink: Sink[List[ApiFlightWithSplits], Future[Done]] = Sink.foreach[List[ApiFlightWithSplits]](f => println(s"Sink received $f"))
    val predictorStage = new SplitsPredictorStage
    val arrivalsSource = Source.fromIterator(() => Iterator(List(arrival)))
    val runnable = RunnablePredictor(arrivalsSource, predictorStage, sink)

//    val (_, _, f): (NotUsed, NotUsed, Future[Done]) = runnable.run()
    runnable.run()
