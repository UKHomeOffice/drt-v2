...
class MockPredictionModelActor(
    now: () => SDateLike,
    category: ModelCategory,
    identifier: WithId
) extends PredictionModelActor(now, category, identifier, None) {
  private val model: RegressionModel =
    RegressionModel(Iterable(-4.491677337488966, 0.5758560689088016, 3.8006500547982798, 0.11517121378172734, 0.0), 0)
  ...
  state = Map(OffScheduleModelAndFeatures.targetName ->
    OffScheduleModelAndFeatures(model, features, examplesTrainedOn = 10, improvementPct = 25))
}

case class MockFlightPersistence()(implicit
  ...
  override val actorProvider: (ModelCategory, WithId, Option[Long]) => ActorRef =
    (modelCategory, identifier, _) =>
      system.actorOf(Props(new MockPredictionModelActor(() => SDate.now(), modelCategory, identifier)))
}

class ArrivalPredictionsSpec extends CrunchTestLike {
  val minutesOffScheduledThreshold = 45
  private val modelKeysForArrival: Arrival => Iterable[WithId] = (a: Arrival) =>
    Iterable(
  ...
  val scheduledStr: String = myNow.toISOString
  ...
  "Given an arrival and an actor containing a prediction model for that arrival" >> {
    "I should be able to update the arrival with an predicted touchdown time" >> {
      ...
      val maybePredictedTouchdown =
        Await.result(arrivalPredictions.applyPredictionsByKey(arrivalsMap, keysWithArrival), 5.second)
  ...
  "Given an ArrivalsDiff and an actor containing touchdown prediction models" >> {
    ...
    "predictions should not be updated if it's less than the threshold time since the last update" >> {
      val currentPredictions = Predictions(
        ...
        Map(OffScheduleModelAndFeatures.targetName -> -4)
      )
