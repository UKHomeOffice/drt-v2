...
class AclFeedSpec extends CrunchTestLike {
  ...
  "ACL Flights " >> {
    ...
    "Given some initial arrivals, no ACL arrivals and one live arrival " +
      ...
      val initialLive = SortedMap[UniqueArrivalWithOrigin, Arrival]() ++ List(ArrivalGenerator.arrival(iata = "BA0001", schDt = "2017-01-01T00:05Z", actPax = Option(99), status = ArrivalStatus("scheduled"))).map(a => (a.unique, a))
    ...
    "Given one ACL arrival followed by one live arrival and initial arrivals which don't match them " +
      ...
      val initialAcl = SortedMap[UniqueArrivalWithOrigin, Arrival]() ++ List(initialAcl1, initialAcl2).map(a => (a.unique, a))
      val initialLive = SortedMap[UniqueArrivalWithOrigin, Arrival]() ++ List(ArrivalGenerator.arrival(iata = "BA0003", schDt = "2017-01-01T00:03Z", actPax = Option(99), status = ArrivalStatus("scheduled"))).map(a => (a.unique, a))
