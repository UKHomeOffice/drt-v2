...
class FixedPointsServiceImplSpec extends TestKit(ActorSystem("test")) with AnyWordSpecLike with Matchers {
  ...
  "A FixedPointsServiceImpl" should {
    val service =
      FixedPointsServiceImpl(mockActor(liveProbe.ref), mockActor(writeProbe.ref), _ => mockActor(pitProbe.ref))
    val assignments = FixedPointAssignments(Seq(StaffAssignment(
      "assignment",
      T1,
