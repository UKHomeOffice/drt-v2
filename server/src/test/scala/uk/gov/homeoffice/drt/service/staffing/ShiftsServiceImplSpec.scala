...
class ShiftsServiceImplSpec extends TestKit(ActorSystem("test")) with AnyWordSpecLike with Matchers {
  ...
  "getActiveShifts" should {
    val service = LegacyShiftAssignmentsServiceImpl(
      mockActor(liveProbe.ref),
      mockActor(writeProbe.ref),
      _ => mockActor(pitProbe.ref)
    )
    ...
    "return a list of staff assignments for a given point in time" in {
      MockActor.response = assignments
      val result =
        service.shiftAssignmentsForDate(LocalDate(2024, 7, 1), Some(SDate("2024-07-01T05:00").millisSinceEpoch))
      result.futureValue should ===(assignments)
    ...
    "filter active shifts by date" in {
      ...
      val testShifts: Seq[Shift] = Seq(
        shift.copy(
          startDate = LocalDate(2024, 7, 1),
          endDate = Option(LocalDate(2024, 7, 30)),
          shiftName = "TEST1",
          startTime = "09:00",
          endTime = "15:00"
        ),
  ...
  "getActiveShiftsForViewRange" should {
    ...
    "filter shifts for daily view" in {
      val mockDao = mock[StaffShiftsDao]
      val service = ShiftsServiceImpl(mockDao)
      val shift1 = shift.copy(
        startDate = LocalDate(2024, 6, 1),
        endDate = Option(LocalDate(2024, 6, 10)),
        shiftName = "TEST1",
        startTime = "09:00",
        endTime = "15:00"
      )
      val shift2 = shift.copy(
        startDate = LocalDate(2024, 5, 25),
        endDate = Option(LocalDate(2024, 6, 4)),
        shiftName = "TEST2",
        startTime = "14:00",
        endTime = "18:00"
      )
      ...
      val result =
        Await.result(service.getActiveShiftsForViewRange("LHR", "T2", Some("daily"), Some("2024-06-05")), 1.seconds)
    ...
    "filter shifts for weekly view" in {
      ...
      val shift2 = shift.copy(
        startDate = LocalDate(2024, 5, 25),
        endDate = Option(LocalDate(2024, 6, 5)),
        shiftName = "TEST2",
        startTime = "14:00",
        endTime = "18:00"
      )
      val shift3 = shift.copy(
        startDate = LocalDate(2024, 6, 15),
        endDate = Option(LocalDate(2024, 6, 20)),
        shiftName = "TEST3",
        startTime = "10:00",
        endTime = "16:00"
      ...
      val result =
        Await.result(service.getActiveShiftsForViewRange("LHR", "T2", Some("weekly"), Some("2024-06-05")), 1.seconds)
    ...
    "filter shifts that has start of week for viewDate not before endDate of shift" in {
      ...
      val shift2 = shift.copy(
        startDate = LocalDate(2024, 5, 25),
        endDate = Option(endDateBeforeStartOfWeek),
        shiftName = "TEST2",
        startTime = "14:00",
        endTime = "18:00"
      )
    ...
    "filter shifts for month view" in {
      ...
      val shift3 = shift.copy(
        startDate = LocalDate(2024, 7, 1),
        endDate = Option(LocalDate(2024, 7, 10)),
        shiftName = "TEST3",
        startTime = "10:00",
        endTime = "16:00"
      )
