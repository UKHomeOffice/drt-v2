...
case class MockManifestLookupService(
    ...
    historicManifestPax: ManifestPaxCount
...
class ManifestProviderSpec extends CrunchTestLike {
  ...
  val manifestForArrival: BestAvailableManifest =
    BestAvailableManifest(VoyageManifestGenerator.manifestForArrival(arrival, List(euPassport)))
  val manifestHistoricForArrival: ManifestPaxCount = ManifestPaxCount(
    VoyageManifestGenerator.manifestForArrival(arrival, List(euPassport)),
    SplitSources.Historical
  )

  val mockLookupService: MockManifestLookupService =
    MockManifestLookupService(manifestForArrival, manifestHistoricForArrival)
  val mockCacheLookup: Arrival => Future[Option[ManifestLike]] = _ => Future.successful(None)
  val mockCacheStore: (Arrival, ManifestLike) => Future[Any] =
    (_: Arrival, _: ManifestLike) => Future.successful(StatusReply.Ack)
  val probe: TestProbe = TestProbe("manifests")

  "Given a mock lookup returning a BestAvailableManifest" >> {
    val lookup = OptimisationProviders.historicManifestsProvider(
      PortCode("STN"),
      mockLookupService,
      mockCacheLookup,
      mockCacheStore
