...
import uk.gov.homeoffice.drt.models.{
  ...
  EeaFlag,
  InTransit,
  ManifestDateOfArrival,
  ManifestPassengerProfile,
  ManifestTimeOfArrival,
  PassengerInfoJson,
  VoyageManifest
}
...
class SplitsCalculatorSpec extends CrunchTestLike {
  ...
  "A SplitsCalculator" should {
    "Not produce ApiPaxTypeAndQueueCounts for SplitRatios with a zero ratio value" in {
      ...
      result === Splits(
        Set(ApiPaxTypeAndQueueCount(EeaMachineReadable, NonEeaDesk, 100.0, None, None)),
        TerminalAverage,
        None,
        Percentage
      )
  ...
  "Concerning BestAvailableManifests" >> {
    ...
    val testArrival = live(
      iata = "SA0234",
      schDt = "2019-06-22T06:24:00Z",
      totalPax = Option(100),
      terminal = T2,
      origin = PortCode("USA")
    ...
    "When adjusting adult EGate use based on under age pax in API Data using an eGate split of 50%" >> {
      ...
      val splitsCalculator =
        SplitsCalculator(testPaxTypeAllocator, config.terminalPaxSplits, ChildEGateAdjustments(1.0))
    ...
    "When adjusting adult EGate use based on under age pax in API Data using an eGate split of 100%" >> {
      ...
      "Given 4 EEA adults and 1 EEA child with a 0.0 adjustment per child" >> {
        "Then I should expect 0 EEA Adults to Desk, 1 EEA child to desk and 4 EEA Adult to eGates" >> {
          ...
          val splitsCalculator =
            SplitsCalculator(testPaxTypeAllocator, config.terminalPaxSplits, ChildEGateAdjustments(0.0))
