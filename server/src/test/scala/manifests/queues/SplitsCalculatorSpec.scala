...
//            nationality = Nationality(CountryCodes.UK),
//            documentType = Option(DocumentType.Passport),
//            age = Option(PaxAge(35)),
...
//          ManifestPassengerProfile(Nationality(CountryCodes.UK), Option(DocumentType.Passport), Option(PaxAge(4)), Option(false))
//SSplits(Set(
// ApiPaxTypeAndQueueCount(EeaMachineReadable,EeaDesk,3.5,Some(Map(Nationality(GBR) -> 2.5))),
// ApiPaxTypeAndQueueCount(EeaBelowEGateAge,EeaDesk,1.0,Some(Map(Nationality(GBR) -> 1.0))),
// ApiPaxTypeAndQueueCount(EeaMachineReadable,EGate,1.5,Some(Map(Nationality(GBR) -> 2.5)))),AdvPaxInfo,None,PaxNumbers) !=
//Splits(Set(
// ApiPaxTypeAndQueueCount(EeaBelowEGateAge,EeaDesk,1.0,Some(Map(Nationality(GBR) -> 1.0))),
// ApiPaxTypeAndQueueCount(EeaMachineReadable,EeaDesk,3.0,Some(Map(Nationality(GBR) -> 2.0))),
// ApiPaxTypeAndQueueCount(EeaMachineReadable,EGate,1.0,Some(Map(Nationality(GBR) -> 2.0)))),Historical,None,PaxNumbers)
        val manifest = apiManifest(List(uk35yo, uk35yo, uk35yo, uk35yo, uk35yo, uk4yo))
        val expected = Splits(
          Set(
            ApiPaxTypeAndQueueCount(PaxTypes.EeaBelowEGateAge, Queues.EeaDesk, 1, Option(Map(Nationality(CountryCodes.UK) -> 1)), None),
            ApiPaxTypeAndQueueCount(PaxTypes.EeaBelowEGateAge, Queues.EeaDesk, 1, Option(Map(Nationality(CountryCodes.UK) -> 1)), None),
            ApiPaxTypeAndQueueCount(PaxTypes.EeaMachineReadable, Queues.EeaDesk, 3, Option(Map(Nationality(CountryCodes.UK) -> 2)), None),
            ApiPaxTypeAndQueueCount(PaxTypes.EeaMachineReadable, Queues.EGate, 1, Option(Map(Nationality(CountryCodes.UK) -> 2)), None)
          ),
          Historical,
          None,
          PaxNumbers
...
//Splits(Set(
      // ApiPaxTypeAndQueueCount(NonVisaNational,FastTrack,0.6,Some(Map(Nationality() -> 0.6))),
      // ApiPaxTypeAndQueueCount(NonVisaNational,NonEeaDesk,5.4,Some(Map(Nationality() -> 5.4)))),AdvPaxInfo,None,PaxNumbers) !=
      // Splits(Set(
      // ApiPaxTypeAndQueueCount(EeaBelowEGateAge,EeaDesk,1.0,Some(Map(Nationality(GBR) -> 1.0))),
      // ApiPaxTypeAndQueueCount(EeaMachineReadable,EeaDesk,3.0,Some(Map(Nationality(GBR) -> 2.0))),
      // ApiPaxTypeAndQueueCount(EeaMachineReadable,EGate,1.0,Some(Map(Nationality(GBR) -> 2.0)))),Historical,None,PaxNumbers)
