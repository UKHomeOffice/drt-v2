...
class ArrivalsScenarioSpec extends CrunchTestLike {
  ...
  val splitsCalculator: SplitsCalculator =
    SplitsCalculator(testPaxTypeAllocator, defaultAirportConfig.terminalPaxSplits, ChildEGateAdjustments(1.0))
  private val arrival: Arrival =
    ArrivalGenerator.arrival(schDt = "2021-03-08T00:00", totalPax = Option(100), feedSource = LiveFeedSource)
  ...
  val egateBanksProvider: () => Future[PortEgateBanksUpdates] =
    () =>
      Future.successful(PortEgateBanksUpdates(defaultAirportConfig.eGateBankSizes.map {
        case (terminal, banks) =>
          (terminal, EgateBanksUpdates(List(EgateBanksUpdate(0L, EgateBank.fromAirportConfig(banks)))))
      }))
  val terminalEgatesProvider: Terminal => Future[EgateBanksUpdates] =
    (terminal: Terminal) =>
      egateBanksProvider().map(_.updatesByTerminal.getOrElse(
        terminal,
        throw new Exception(s"No egates found for terminal $terminal")
      ))
