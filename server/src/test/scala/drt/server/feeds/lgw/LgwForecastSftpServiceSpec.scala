...
class LgwForecastSftpServiceSpec extends Specification {
  ...
  def withFiles(files: List[String])(test: SFTPClient => org.specs2.matcher.MatchResult[Option[String]])
  ...
  "latestFileName" should {
    ...
    "return the newest file with new format" in {
      withFiles(List(
        "LGWARRFORECAST-2024-06-01.csv",
        "LGWARRFORECAST-2024-06-02.csv",
        "LGWARRFORECAST-2024-06-03.csv"
      )) { sftp =>
    ...
    "return the newest file with old format" in {
      withFiles(List(
        "2024-06-01-LGWArrForecast.csv",
        "2024-06-02-LGWArrForecast.csv",
        "2024-06-03-LGWArrForecast.csv"
      )) { sftp =>
