...
object ShiftAssignmentsTest extends Specification {
  ...
  "Given existing shifts" >> {
    "update shift if shift exists before" >> {
      ...
      val existingShifts = ShiftAssignments(StaffAssignment(
        "Morning",
        T1,
        existingShiftStartTime,
        existingShiftEndTime,
        10,
        None
      ).splitIntoSlots(15))
      val expectedResult = Set(
    ...
    "should not take too long to apply update to 6 months of 15 minute existing shifts" >> {
      ...
      val updateShifts = Seq(
        StaffAssignment(
          "Morning",
          T1,
          SDate(s"2017-01-01T12:00").millisSinceEpoch,
          SDate(s"2017-01-01T18:59").millisSinceEpoch,
          10,
          None
        )
      )
