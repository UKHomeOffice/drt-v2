...
class IndexedByTerminalSpec extends SpecificationLike {
  "Given an empty PortStateMutable " >> {
    ...
    "When I add 2 arrivals, and purge updates up to a point after one of the arrival's updated time, and ask for updates since a point in time earlier than the remaining arrival's updated time " +
      ...
      val arrival1 = ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA001", schDt = "2019-01-01T00:00", origin = PortCode("JFK")), Set(), lastUpdated = Option(arrival1UpdateTime))
      val arrival2 = ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA001", schDt = "2019-01-01T05:00", origin = PortCode("JNB")), Set(), lastUpdated = Option(arrival2UpdateTime))
    ...
    "When I add 3 arrivals where the 3rd has a last updated time later than the first but earlier than 2nd, and I ask for updates since just before the 3rd arrival" +
      ...
      val arrival1 = ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA001", schDt = "2019-01-01T00:00", origin = PortCode("JFK")), Set(), lastUpdated = Option(arrival1UpdateTime))
      val arrival2 = ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA002", schDt = "2019-01-01T05:00", origin = PortCode("JNB")), Set(), lastUpdated = Option(arrival2UpdateTime))
      val arrival3 = ApiFlightWithSplits(ArrivalGenerator.arrival(iata = "BA003", schDt = "2019-01-01T07:00", origin = PortCode("AAA")), Set(), lastUpdated = Option(arrival3UpdateTime))
