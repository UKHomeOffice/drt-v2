...
class QueueMinutesRouterActorSpec extends CrunchTestLike {
  ...
  def lookupWithData(crunchMinutes: MinutesContainer[CrunchMinute, TQM]): MinutesLookup[CrunchMinute, TQM] =
    (_: (Terminal, UtcDate), _: Option[MillisSinceEpoch]) => Future(Option(crunchMinutes))

  val crunchMinute: CrunchMinute =
    CrunchMinute(terminal, queue, date.millisSinceEpoch, 1, 2, 3, 4, None, None, None, None)
  ...
  "When I send two sets of PassengerMinutes to a QueueMinutesActor and query it" >> {
    ...
    "I should see all minutes in the updates callback" >> {
      ...
      val crunchMinute1 =
        CrunchMinute(terminal, queue, date.millisSinceEpoch, 3, 3, 1, 1, None, None, None, None, None, None, None)
      val crunchMinute2 = CrunchMinute(
        terminal,
        queue,
        date.addMinutes(1).millisSinceEpoch,
        4,
        3,
        1,
        1,
      ...
      val newCrunchMinute2 = CrunchMinute(
        terminal,
        queue,
        date.addMinutes(1).millisSinceEpoch,
        5,
      ...
      val crunchMinute3 = CrunchMinute(
        terminal,
        queue,
        date.addMinutes(2).millisSinceEpoch,
        6,
        3,
        1,
        1,
  ...
  "When I ask for CrunchMinutes" >> {
    "Given a lookups with no data" >> {
      "I should get None" >> {
        val cmActor: ActorRef = system.actorOf(Props(new QueueMinutesRouterActor(
          (_, _) => Seq(terminal),
          lookupWithData(minutesContainer),
          noopUpdates
        )))
        val eventualResult = cmActor.ask(GetStateForTerminalDateRange(
  ...
  "When I ask for crunch minutes in the range 10:00 to 10:59" >> {
    ...
    "Given a lookup with minutes 09:59 and 10:00 & " >> {
      val crunchMinuteOutSideRange1: CrunchMinute =
        CrunchMinute(terminal, queue, SDate("2020-01-01T09:59").millisSinceEpoch, 1, 2, 3, 4, None, None, None, None)
      val crunchMinuteOutSideRange2: CrunchMinute =
        CrunchMinute(terminal, queue, SDate("2020-01-01T11:00").millisSinceEpoch, 1, 2, 3, 4, None, None, None, None)
      val crunchMinuteInsideRange1: CrunchMinute =
        CrunchMinute(terminal, queue, SDate("2020-01-01T10:00").millisSinceEpoch, 1, 2, 3, 4, None, None, None, None)
      val crunchMinuteInsideRange2: CrunchMinute =
        CrunchMinute(terminal, queue, SDate("2020-01-01T10:59").millisSinceEpoch, 1, 2, 3, 4, None, None, None, None)
      val minutes =
        Seq(crunchMinuteInsideRange1, crunchMinuteInsideRange2, crunchMinuteOutSideRange1, crunchMinuteOutSideRange2)
      val minutesState: MinutesContainer[CrunchMinute, TQM] = MinutesContainer(minutes)

      "I should get the one minute back" >> {
        val cmActor: ActorRef = system.actorOf(Props(new QueueMinutesRouterActor(
          (_, _) => Seq(terminal),
          lookupWithData(minutesState),
          noopUpdates
        )))
        val eventualResult = cmActor.ask(GetStateForTerminalDateRange(
