...
class FlightsRouterActorSpec extends CrunchTestLike {
  ...
  val flightWithSplits: ApiFlightWithSplits =
    ArrivalGenerator.flightWithSplitsForDayAndTerminal(date, T1, LiveFeedSource)
  ...
  val noopUpdates: (
      Option[ActorRef],
      Option[ActorRef]
  ) => ((Terminal, UtcDate), FlightUpdates) => Future[Set[TerminalUpdateRequest]] =
  ...
  "Concerning visibility of flights (scheduled & pcp range)" >> {
    ...
    "Given a flight scheduled before the range and a PCP time within the range" >> {
      val fws1 = ApiFlightWithSplits(
        ArrivalGenerator.live(schDt =
          "2020-09-22T23:00"
        ).toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2020-09-23T00:30").millisSinceEpoch)),
        Set()
      )
  ...
  "Concerning persistence of flights" >> {
    "Given a router, I should see updates sent to it are persisted" >> {
      val lookups = FlightLookups(
        system,
        myNow,
        terminalsForDateRange = (_, _) => Seq(T1),
        None,
        paxFeedSourceOrder,
        _ => None,
      ...
      val requestForFlights =
        GetFlights(SDate(scheduled).millisSinceEpoch, SDate(scheduled).addHours(6).millisSinceEpoch)
    ...
    "A flights router actor" should {
      ...
      val queues: SortedMap[LocalDate, Map[Terminal, Seq[Queue]]] =
        SortedMap(LocalDate(2014, 1, 1) -> Map(T1 -> Seq.empty[Queue], T2 -> Seq.empty[Queue]))
      ...
      "Add red list pax to an existing arrival" in {
        ...
        val eventualFlights =
          (flightsRouter ? GetFlightsForTerminalDateRange(
            redListNow.getLocalLastMidnight.millisSinceEpoch,
            redListNow.getLocalNextMidnight.millisSinceEpoch,
            T1
          )).flatMap {
            case source: Source[(UtcDate, FlightsWithSplits), NotUsed] =>
              source.runFold(FlightsWithSplits.empty)(_ ++ _._2)
          }
      ...
      "Add red list pax counts to the appropriate arrivals" in {
        ...
        val arrivalT1 =
          ArrivalGenerator.live(iata = "BA0001", terminal = T1, schDt = scheduled).toArrival(LiveFeedSource)
        val arrivalT2 =
          ArrivalGenerator.live(iata = "AB1234", terminal = T2, schDt = scheduled2).toArrival(LiveFeedSource)
        ...
        arrivals.contains(arrivalT1.copy(RedListPax = Option(redListPax.size))) &&
        arrivals.contains(arrivalT2.copy(RedListPax = Option(redListPax2.size)))
      }
  ...
  "Concerning multi-terminal queries" >> {
    ...
    val t21015 = ApiFlightWithSplits(
      ArrivalGenerator.live(iata = "BA0001", origin = PortCode("JFK"), terminal = T2)
        .toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2021-07-10T15:00").millisSinceEpoch)),
      Set()
    )
    val t21013 = ApiFlightWithSplits(
      ArrivalGenerator.live(iata = "BA0002", origin = PortCode("JFK"), terminal = T2)
        .toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2021-07-10T13:00").millisSinceEpoch)),
      Set()
    )
    val t31015 = ApiFlightWithSplits(
      ArrivalGenerator.live(iata = "BA0003", origin = PortCode("JFK"), terminal = T3)
        .toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2021-07-10T15:00").millisSinceEpoch)),
      Set()
    )
    val t31013 = ApiFlightWithSplits(
      ArrivalGenerator.live(iata = "BA0004", origin = PortCode("JFK"), terminal = T3)
        .toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2021-07-10T13:00").millisSinceEpoch)),
      Set()
    )
    val t21115 = ApiFlightWithSplits(
      ArrivalGenerator.live(iata = "BA0005", origin = PortCode("JFK"), terminal = T2)
        .toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2021-07-10T15:00").millisSinceEpoch)),
      Set()
    )
    val t21113 = ApiFlightWithSplits(
      ArrivalGenerator.live(iata = "BA0006", origin = PortCode("JFK"), terminal = T2)
        .toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2021-07-10T13:00").millisSinceEpoch)),
      Set()
    )
    val t31115 = ApiFlightWithSplits(
      ArrivalGenerator.live(iata = "BA0007", origin = PortCode("JFK"), terminal = T3)
        .toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2021-07-10T15:00").millisSinceEpoch)),
      Set()
    )
    val t31113 = ApiFlightWithSplits(
      ArrivalGenerator.live(iata = "BA0008", origin = PortCode("JFK"), terminal = T3)
        .toArrival(LiveFeedSource).copy(PcpTime = Option(SDate("2021-07-10T13:00").millisSinceEpoch)),
      Set()
    )
    ...
    "sortedSourceForIterables" should {
      "produce a FlightsWithSplits for each date, with flights from all terminals sorted by pcp time & voyage number" in {
        val flightsByDayAndTerminalProvider
            : Option[MillisSinceEpoch] => UtcDate => Terminal => Future[FlightsWithSplits] =
          _ => flightsForDayAndTerminal

        val flightsStream = FlightsRouterActor.multiTerminalFlightsByDaySource(flightsByDayAndTerminalProvider)(
          SDate(UtcDate(2021, 7, 10)),
          SDate(UtcDate(2021, 7, 11)),
          terminals,
          None,
          paxFeedSourceOrder
        )
        ...
        result === Seq(
          (UtcDate(2021, 7, 10), FlightsWithSplits(Seq(t21013, t31013, t21015, t31015))),
          (UtcDate(2021, 7, 11), FlightsWithSplits(Seq(t21113, t31113, t21115, t31115)))
        )
      }
    }
