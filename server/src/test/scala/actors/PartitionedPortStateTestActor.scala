...
class PartitionedPortStateTestActor(
    probe: ActorRef,
    ...
    case ps: PortState =>
      ...
      if (ps.flights.nonEmpty) {
        flightsActor.ask(ArrivalsDiff(ps.flights.map(_._2.apiFlight), Seq()))
          .flatMap(_ =>
            flightsActor.ask(SplitsForArrivals(ps.flights.map {
        ...
        case mc: MinutesContainer[_, _] =>
          ...
          mc.minutes.headOption match {
            ...
            case Some(minuteLike) if minuteLike.toMinute.isInstanceOf[StaffMinute] =>
              actor.ask(GetStateForDateRange(
                minuteMillis.min,
                minuteMillis.max
  ...
  private def minutesAffectedByDiff(
      arrivalsDiff: ArrivalsDiff,
      removals: Iterable[UniqueArrival]
