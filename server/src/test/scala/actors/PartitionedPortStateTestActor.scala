    ...
    actor.ask(message).foreach { _ =>
      message match {
        ...
        case flightsWithSplitsDiff@FlightsWithSplitsDiff(_, _) if flightsWithSplitsDiff.nonEmpty =>
          updateFlights(actor, flightsWithSplitsDiff.arrivalsToRemove.collect { case ua: UniqueArrival => ua }, flightsWithSplitsDiff.updateMinutes.min, flightsWithSplitsDiff.updateMinutes.max)
