...
import uk.gov.homeoffice.drt.egates.{
  DeleteEgateBanksUpdates,
  EgateBank,
  EgateBanksUpdate,
  EgateBanksUpdates,
...
class EgateBanksUpdatesMessageConversionSpec extends Specification {
  "Given a SetEgateBanksUpdate" >> {
    "I should be able to serialise it and deserialise it without any data loss" >> {
      val setEgateBanksUpdate = SetEgateBanksUpdate(
        T1,
        100L,
        EgateBanksUpdate(150L, IndexedSeq(EgateBank(IndexedSeq(true, false, true, false))))
      )
  ...
  "Given a PortEgateBanksUpdates" >> {
    "I should be able to serialise it and deserialise it without any data loss" >> {
      val portEgateBanksUpdates = PortEgateBanksUpdates(Map(
        T1 -> EgateBanksUpdates(List(EgateBanksUpdate(
          100L,
          IndexedSeq(EgateBank(IndexedSeq(true, false, true)), EgateBank(IndexedSeq(false, true, false)))
        ))),
        T2 -> EgateBanksUpdates(List(EgateBanksUpdate(
          150L,
          IndexedSeq(EgateBank(IndexedSeq(false, false, true)), EgateBank(IndexedSeq(true, true, false)))
        )))
      ))
