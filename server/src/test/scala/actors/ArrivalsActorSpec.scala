...
class ArrivalsActorSpec extends Specification {
  ...
  private def arrivalsToKeysArrivals(arrivals: List[Arrival]): Seq[(UniqueArrival, Arrival)] =
    arrivals.map(a => (a.unique, a))
}
