...
import actors.PartitionedPortStateActor.{GetFlightsForTerminalEffectingRange, GetStateForDateRange, GetStateForTerminalDateRange, GetUpdatesSince}
...
class FlightsStateActorResponsesSpec extends CrunchTestLike {
  ...
  val messagesAndResponseTypes: Map[Any, (PartialFunction[Any, Result], Any)] = Map(
    ...
    GetFlightsForTerminalEffectingRange(0L, 1L, T1) -> (({ case _: FlightsWithSplits => success }, classOf[FlightsWithSplits])),
