...
import actors.PartitionedPortStateActor.{GetFlightsForTerminalDateRange, GetStateForDateRange, GetStateForTerminalDateRange, GetUpdatesSince}
...
class FlightsStateActorResponsesSpec extends CrunchTestLike {
  ...
  val messagesAndResponseTypes: Map[Any, (PartialFunction[Any, Result], Any)] = Map(
    ...
    GetFlightsForTerminalDateRange(0L, 1L, T1) -> (({ case _: FlightsWithSplits => success }, classOf[FlightsWithSplits])),
