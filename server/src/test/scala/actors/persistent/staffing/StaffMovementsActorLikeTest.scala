...
class StaffMovementsActorLikeTest extends AnyWordSpec with Matchers {
  "eventToState" should {
    "add incoming staff movements to the existing state" in {
      ...
      val newMovement =
        StaffMovementMessage(Option("T1"), Option("reason"), Option(1234567890L), Option(1), Option("uuid2"))
      ...
      updatedState shouldEqual
        StaffMovementsState(StaffMovements(Seq(existingMovement, staffMovementMessageToStaffMovement(newMovement))))
    }

    "ignore any staff movement events with a UUID that already exists (because these are duplicates)" in {
      val existingMovement = StaffMovement(T1, "reason", 1234567890L, 1, "uuid1", None, None)
      val newMovement =
        StaffMovementMessage(Option("T1"), Option("reason"), Option(1234567890L), Option(1), Option("uuid1"))
