...
class FlightUpdatesAndRemovalsSpec extends Specification {
  ...
  private val arrival1 =
    ArrivalGenerator.live(iata = "BA0001", totalPax = Option(10), schDt = "2022-05-01T00:25").toArrival(LiveFeedSource)
  ...
  private val arrival2 =
    ArrivalGenerator.live(iata = "BA0002", totalPax = Option(10), schDt = "2022-05-01T12:40").toArrival(LiveFeedSource)
  private val arrival3 =
    ArrivalGenerator.live(iata = "BA0003", totalPax = Option(10), schDt = "2022-05-01T20:15").toArrival(LiveFeedSource)
  "Concerning FlightsWithSplitsDiffs" >> {
    "Given an empty FlightUpdatesAndRemovals" >> {
      "When I add an update it should retain it in its updates Map" >> {
        val updatesToAdd = FlightUpdatesAndRemovals(
          Map(1L -> ArrivalsDiff(Seq(arrival1), Seq())),
          Map(1L -> SplitsForArrivals(Map(arrival1.unique -> splits(10, TerminalAverage))))
        )
    ...
    "Given a FlightUpdatesAndRemovals containing arrivals and removals" >> {
      "When I ask for updates since 1500 it should return flights and removals with update times later than 1500" >> {
        ...
        updatesAndRemovals.updatesSince(update1500) === FlightUpdatesAndRemovals(
          Map(
            update2000 -> ArrivalsDiff(Seq(arrival3), Seq(arrival2.unique))
          ),
          Map()
    ...
    "Given a FlightUpdatesAndRemovals containing 2 arrivals" >> {
      "When I apply a SplitsForArrivals with an API split for the first arrival" >> {
        "Then I should get that updated flight when I ask for updates" >> {
          ...
          updatesAndRemovals.add(diff, update2000).updatesSince(update1500) === FlightUpdatesAndRemovals(
            Map(),
            Map(
