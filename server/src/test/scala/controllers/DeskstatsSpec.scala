...
class DeskstatsSpec extends Specification {
  ...
  "Deskstats " >> {
    ...
    "Can parse data from blackjack CSV content " >> {
      ...
      val expected = Map(
        "T2" -> Map(
          Queues.EeaDesk -> Map(1498771800000L -> DeskStat(Some(2), Some(9))),
          Queues.NonEeaDesk -> Map(1498771800000L -> DeskStat(Some(16), None)),
          Queues.FastTrack -> Map(1498771800000L -> DeskStat(Some(2), Some(0)))))
