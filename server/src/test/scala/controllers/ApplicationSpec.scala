...
class ApplicationSpec extends CrunchTestLike {
  ...
  trait Context extends Scope {
    ...
    implicit val config: Configuration = play.api.Configuration.from(Map("portCode" -> "test", "dq.s3.bucket" -> "bucket", "googleTrackingCode"-> "", "virus-scanner-url" -> "", "crunch.forecast.max_days" -> "3"))
