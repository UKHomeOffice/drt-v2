...
class HealthCheckControllerSpec extends PlaySpec {
  ...
  val flights: Seq[(UtcDate, FlightsWithSplits)] = Seq(
    (
      UtcDate(2024, 6, 26),
      FlightsWithSplits(Seq(
        ApiFlightWithSplits(
          ArrivalGenerator.live(iata = "BA0001", schDt = "2024-06-26T11:30").toArrival(LiveFeedSource),
          Set(),
          lastUpdated = Option(SDate("2024-06-26T05:40").millisSinceEpoch)
        ),
        ApiFlightWithSplits(
          ArrivalGenerator.live(
            iata = "BA0005",
            schDt = "2024-06-26T11:35",
            actDt = "2024-06-26T11:40"
          ).toArrival(LiveFeedSource),
          Set(splits),
          lastUpdated = Option(SDate("2024-06-26T11:50").millisSinceEpoch)
        ),
        ApiFlightWithSplits(
          ArrivalGenerator.live(iata = "BA0011", schDt = "2024-06-26T12:30").toArrival(LiveFeedSource),
          Set(),
          lastUpdated = Option(SDate("2024-06-26T05:40").millisSinceEpoch)
        ),
        ApiFlightWithSplits(
          ArrivalGenerator.live(
            iata = "BA0015",
            schDt = "2024-06-26T12:35",
            actDt = "2024-06-26T12:45"
          ).toArrival(LiveFeedSource),
          Set(splits),
          lastUpdated = Option(SDate("2024-06-26T11:50").millisSinceEpoch)
        )
  ...
  val minutes: Seq[(UtcDate, MinutesContainer[CrunchMinute, TQM])] = Seq(
    (
      UtcDate(2024, 6, 16),
      MinutesContainer(Seq(
        CrunchMinute(
          T1,
          EeaDesk,
          SDate("2024-06-26T12:30").millisSinceEpoch,
          0d,
          0d,
          0,
          0,
  ...
  private def newDrtInterface(
      flights: Seq[(UtcDate, FlightsWithSplits)],
      minutes: Seq[(UtcDate, MinutesContainer[CrunchMinute, TQM])]
  ): DrtSystemInterface = {
    ...
    new TestDrtSystem(Lhr.config, mod.drtParameters, now) {
      ...
      override lazy val applicationService: ApplicationService = new ApplicationService(
        ...
        override lazy val flightsProvider: FlightsProvider =
          FlightsProvider(system.actorOf(Props(new MockFlightsRouterActor(flights))))(timeout)
      }

      override lazy val actorService = new TestActorService(
        journalType,
        airportConfig,
        now,
