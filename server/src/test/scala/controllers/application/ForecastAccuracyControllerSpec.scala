...
import play.api.test.Helpers.{ contentAsString, contentType, status, OK }
...
class ForecastAccuracyControllerSpec extends PlaySpec with BeforeAndAfter {
  ...
  "ForecastAccuracyController" should {
    ...
    val controller: ForecastAccuracyController =
      forecastAccuracyController(forecastTotalPax, mlFeedPax, liveFeedPax, mlPredCapPct, flights)
    "get forecast accuracy percentage" in {
      ...
      val request = FakeRequest(
        method = "GET",
        uri = "",
        headers = Headers(("X-Forwarded-Groups", "TEST")),
      ...
      contentAsString(result) must ===(
        s"""{"localDate":{"year":2023,"month":1,"day":1},"pax":[["uk.gov.homeoffice.drt.ports.Terminals.T1",{"1":[20],"14":[20],"3":[20],"30":[20],"7":[20]}]]}""".stripMargin
      )
    }
  ...
  private def forecastAccuracyController(
      forecastTotalPax: Int,
      mlFeedPax: Int,
      liveFeedPax: Int,
      mlPred: Int,
    ...
    val module: DrtModule = new TestDrtModule() {
      ...
      override lazy val provideDrtSystemInterface: TestDrtSystem =
        new TestDrtSystem(Test.config, drtParameters, now)(mat, ec, system, timeout) {
          lazy override val feedService: ProdFeedService = new ProdFeedService(
            journalType,
            airportConfig,
            now,
            params,
            ...
            flightLookups,
            manifestLookups,
            actorService.requestAndTerminateActor,
            drtParameters.forecastMaxDays,
            SDate("2024-04-03")
          )(this.system, ec, timeout) {
            ...
            override val forecastArrivals: (LocalDate, SDateLike) => Future[Map[Terminal, Seq[Arrival]]] = (_, _) =>
              Future.successful(
                Map(T1 -> Seq(ArrivalGenerator.live(
                  iata = "BA0001",
                  schDt = "2023-10-20T12:25",
                  terminal = T1
                ).toArrival(ForecastFeedSource).copy(
            ...
            override val actualArrivals: LocalDate => Future[Map[Terminal, Seq[Arrival]]] = _ =>
              Future.successful(
                Map(T1 -> Seq(ArrivalGenerator.live(
                  iata = "BA0001",
                  schDt = "2023-10-20T12:25",
          ...
          lazy override val actorService: ActorsServiceLike = new TestActorService(
            journalType,
            airportConfig,
            now,
  ...
  case class MockModelPersistence(pax: Int) extends ModelPersistence {
    override def getModels(
      ...
      _ =>
        Future.successful(Models(Map("some-model-id" -> new ArrivalModelAndFeatures {
    ...
    override val persist
        : (PredictionModelActor.WithId, Int, Any, FeaturesWithOneToManyValues, Int, Double, String) => Future[Done] =
