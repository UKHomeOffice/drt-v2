...
class SummariesControllerSpec extends PlaySpec with BeforeAndAfterEach {
  ...
  def generateMinutes(
    ...
    (start.millisSinceEpoch to end.millisSinceEpoch by 60000L)
      ...
      .map {
        case (terminal, queue, millis) =>
          CrunchMinute(terminal, queue, millis, paxPerHour, 0d, 0, 0, None, None, None, None, None, None, None)
      }
  }
  ...
  "populatePassengersForDate" should {
    "create hourly entries from existing crunch minutes for the date requested" in {
      ...
      val minutes = MinutesContainer(generateMinutes(
        SDate("2024-05-31T23:00"),
        SDate("2024-06-01T22:59"),
        Seq(T3, T2),
        queues,
        queuePaxPerMinute
      ))
      ...
      val result = controller
        .populatePassengersForDate("2024-06-01", "2024-06-01")
      ...
      rows must ===((0 to 23).map { hour =>
        (
          SDate("2024-06-01", europeLondonTimeZone).addHours(hour).millisSinceEpoch,
          Map(EeaDesk -> queuePaxPerHour, NonEeaDesk -> queuePaxPerHour)
        )
      }.toMap)
    }
  }
  ...
  "exportPassengersByTerminalForDateRangeApi" should {
    ...
    "generate a csv with the correct headers for the given port" in {
      ...
      contentAsString(result) must
        ===(s"Heathrow,LHR,$capacity,$portPaxPerDay,0,$terminalPaxPerDay,0,$terminalPaxPerDay,0\n")
    }
    "generate a daily breakdown csv with the correct headers for the given port" in {
      val controller: SummariesController = populateForDate(LocalDate(2024, 6, 1), LocalDate(2024, 6, 1), terminals)
      val csvRequest =
        FakeRequest(method = "GET", uri = "?granularity=daily", headers = acceptHeader, body = AnyContentAsEmpty)
    ...
    "generate an hourly breakdown csv with the correct headers for the given port" in {
      val controller: SummariesController = populateForDate(LocalDate(2024, 6, 1), LocalDate(2024, 6, 1), terminals)
      val csvRequest =
        FakeRequest(method = "GET", uri = "?granularity=hourly", headers = acceptHeader, body = AnyContentAsEmpty)
      val result = controller.exportPassengersByPortForDateRangeApi("2024-06-01", "2024-06-02").apply(csvRequest)
      ...
      contentAsString(result) must ===(
        (0 to 23).map { hour =>
          ...
          f"2024-06-01T$hour%02d:00:00+01:00,Heathrow,LHR,$capacity,${portPaxPerDay / 24},0,${terminalPaxPerDay /
              24},0,${terminalPaxPerDay / 24},0\n"
        }.mkString
      )
    }
    "generate a csv with the correct headers for the given terminal" in {
      ...
      val csvRequest = FakeRequest(method = "GET", uri = "", headers = acceptHeader, body = AnyContentAsEmpty)
      val result =
        controller.exportPassengersByTerminalForDateRangeApi("2024-06-01", "2024-06-01", "T3").apply(csvRequest)

      status(result) must ===(OK)
      contentType(result) must ===(Some("text/csv"))
      contentAsString(result) must
        ===(s"Heathrow,LHR,T3,0,${queuePaxPerDay * queues.size},0,$queuePaxPerDay,0,$queuePaxPerDay,0\n")
    }
    "generate a daily breakdown csv with the correct headers for the given terminal" in {
      ...
      val csvRequest =
        FakeRequest(method = "GET", uri = "?granularity=daily", headers = acceptHeader, body = AnyContentAsEmpty)
      val result =
        controller.exportPassengersByTerminalForDateRangeApi("2024-06-01", "2024-06-02", "T3").apply(csvRequest)
  ...
  private def populateForDate(
      startDate: LocalDate,
      endDate: LocalDate,
      terminals: Seq[Terminal]
  ): SummariesController = {
    val startSDate = SDate(startDate)
    val endSDate = SDate(endDate).addDays(1).addMinutes(-1)
    val drtInterface = newDrtInterface
    ...
    val row = CapacityHourlyRow(
      drtInterface.airportConfig.portCode.iata,
      T1.toString,
      startSDate.toLocalDate.toISOString,
      12,
      capacity,
      new Timestamp(0L)
    )
    ...
    val request = FakeRequest(
      method = "PUT",
      uri = "",
      headers = Headers(("X-Forwarded-Groups", "super-admin,LHR")),
      body = AnyContentAsEmpty
    )
    Await.ready(
      controller.populatePassengersForDate(startDate.toISOString, endDate.toISOString).apply(request),
      2.second
    )
    controller
  }
