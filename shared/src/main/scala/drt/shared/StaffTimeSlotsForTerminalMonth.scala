...
object StaffTimeSlotsForTerminalMonth {
  ...
  def staffTimeSlotSeqToStaffTimeSlots(
      monthStart: SDateLike,
      staffDays: Seq[Seq[Int]],
      terminal: Terminal,
      slotMinutes: Int
  ): Seq[StaffTimeSlot] = for {
  ...
  def slotStart(month: SDateLike, slotMinutes: Int, timeSlotIndex: Int, dayIndex: Int): SDateLike =
    month.addDays(dayIndex).addMinutes(timeSlotIndex * slotMinutes)
}
