...
object CodeShares {
  private def uniqueArrivalsWithCodeShares[T](
  ...
  def uniqueFlightsWithCodeShares(
      paxFeedSourceOrder: List[FeedSource],
      exceptions: Set[FlightCode]
  ): Seq[ApiFlightWithSplits] => Seq[(ApiFlightWithSplits, Seq[String])] = {
    val fn: (Seq[ApiFlightWithSplits], ApiFlightWithSplits => Boolean, ApiFlightWithSplits => Arrival) => Seq[(
        ApiFlightWithSplits,
        Seq[String]
    )] =
