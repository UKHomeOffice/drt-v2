...
object Forecast {
  ...
  def handleBSTToUTC(forecastPeriodDays: Map[MillisSinceEpoch, Seq[ForecastTimeSlot]])
      : Map[MillisSinceEpoch, Seq[Option[ForecastTimeSlot]]] = {
    val epochToMaybeSlots: Map[MillisSinceEpoch, Seq[Option[ForecastTimeSlot]]] =
      forecastPeriodDays.mapValues(_.map(Option(_))).toMap
