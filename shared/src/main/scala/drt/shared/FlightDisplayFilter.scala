...
sealed trait FlightDisplayFilter {
  val forTerminalIncludingIncomingDiversions
      : (Iterable[ApiFlightWithSplits], Terminal) => Iterable[ApiFlightWithSplits] =
    (flights: Iterable[ApiFlightWithSplits], terminal: Terminal) =>
      flights.filter { fws =>
...
case class LhrFlightDisplayFilter(
    redListUpdates: RedListUpdates,
    isRedListOrigin: (PortCode, MillisSinceEpoch, RedListUpdates) => Boolean,
    terminalTypes: LhrTerminalTypes
) extends FlightDisplayFilter {
  override val filterIncludingIncomingDivertedRedListFlights: (ApiFlightWithSplits, Terminal) => Boolean =
    (fws: ApiFlightWithSplits, terminal: Terminal) =>
  ...
  private def isIncomingDivertedRedListPaxFlight(
  ...
  private def isOutgoingRedListPaxFlight(
      fws: ApiFlightWithSplits,
      terminal: Terminal,
      redListUpdates: RedListUpdates
  ) = {
