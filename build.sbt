...
lazy val drtv2 = (project in file("."))
  .settings(
    name := drtv2Name,
    // Allow scalafmt on the root project to include sbt build-definition Scala files.
    Compile / unmanagedSourceDirectories += baseDirectory.value / "project",
    Test / unmanagedSourceDirectories += baseDirectory.value / "project"
  )
...
addCommandAlias("scalafmtAll", ";project drtv2;all scalafmtSbt scalafmt Test/scalafmt;project server")
